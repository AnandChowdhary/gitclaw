#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-memory-status slash action queues provider-visible memory status and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-memory-status-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-memory-status-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-memory-status slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-memory-status-e2e-${timestamp}"
ingest_message_id="memory-status-ingest-${timestamp}"
notify_message_id="memory-status-notify-${timestamp}"
memory_status_id="memory-status-${timestamp}"
account_id="telegram-memory-status-account-NOECHO_CHANNEL_MEMORY_STATUS_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_MEMORY_STATUS_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_MEMORY_STATUS_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_MEMORY_STATUS_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_MEMORY_STATUS_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_MEMORY_STATUS_CONTEXT_V1"
search_phrase="channel memory status unique search fixture phrase"
...
memory_status_notification_count() {
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-memory-status slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-memory-status slash E2E.
...
memory_status_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels memory-status --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --status-id ${memory_status_id}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$memory_status_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel memory-status action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel memory-status action receipt"
memory_status_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "loaded_memory_entries: \`" \
  "omitted_memory_entries: \`" \
  "memory_files: \`" \
  "long_term_memory_present: \`true\`" \
  "long_term_memory_loaded: \`true\`" \
  "dated_memory_notes: \`" \
  "canonical_dated_memory_notes: \`" \
  ...
  "llm_e2e_required_after_channel_memory_status_action_change: \`true\`"; do
  grep -Fq "$expected" <<<"$memory_status_receipt" || die "channel memory-status receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$memory_status_id" ".gitclaw/MEMORY.md" ".gitclaw/memory/" "GITCLAW_CHANNEL_MEMORY_STATUS_CONTEXT_V1"; do
  if grep -Fq "$leaked" <<<"$memory_status_receipt"; then
    die "channel memory-status receipt leaked ${leaked}"
  fi
done

[[ "$(memory_status_notification_count)" == "1" ]] || die "channel memory-status did not queue exactly one memory-status notification"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$notification_bodies" || die "memory-status notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$memory_status_id" "GITCLAW_CHANNEL_MEMORY_STATUS_CONTEXT_V1" ".gitclaw/MEMORY.md" ".gitclaw/memory/"; do
  if grep -Fq "$leaked" <<<"$notification_bodies"; then
    die "memory-status notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing memory-status notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$memory_status_id" "GITCLAW_CHANNEL_MEMORY_STATUS_CONTEXT_V1" ".gitclaw/MEMORY.md" ".gitclaw/memory/" "MEMORY.md"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels recall-status --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --status-id ${memory_status_id}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel memory-status action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel memory-status receipt"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$duplicate_receipt" || die "duplicate channel memory-status receipt missing ${expected}"
done
[[ "$(memory_status_notification_count)" == "1" ]] || die "duplicate channel memory-status queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$memory_status_id" ".gitclaw/MEMORY.md" ".gitclaw/memory/" "GITCLAW_CHANNEL_MEMORY_STATUS_CONTEXT_V1"; do
  if grep -Fq "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel memory-status receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel memory-status thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_MEMORY_STATUS_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, memory-status ids, memory file paths, memory bodies, issue bodies, comment bodies, prompts, sessions, backup payloads, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel memory-status model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel memory-status follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel memory-status search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel memory-status follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel memory-status follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel memory-status follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel memory-status follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel memory-status follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel memory-status follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$memory_status_id" "$account_id"; do
  if grep -Fq "$leaked" <<<"$model_comment"; then
    die "model channel memory-status follow-up leaked ${leaked}"
  fi
done
