...
title="@gitclaw /skills e2e ${timestamp}"
body="Live skills-report E2E.

Please list the local GitClaw skill inventory and select the repo-reader skill.
This should produce a deterministic skills report without calling a model."
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "skills-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one skills report comment"
...
if grep -Fq "GITCLAW_SKILL_CONTEXT_V1" <<<"$comments"; then
  die "skills report leaked full skill body token"
fi
