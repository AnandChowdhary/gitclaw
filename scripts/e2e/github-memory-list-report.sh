...
token="GITCLAW_MEMORY_LIST_E2E_${timestamp}"
title="@gitclaw /memory list e2e ${timestamp}"
body="Live memory-list E2E.

Hidden memory list body token: ${token}
This should produce a deterministic memory list report without a model call."
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "memory-list-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one memory list report comment"
...
for expected in \
  ...
  'sha256_12='; do
  grep -Fq -- "$expected" <<<"$comments" || die "memory list report missing ${expected}"
done

for leaked in "$token" "Durable memory token" "GITCLAW_MEMORY_CONTEXT_V1" "Hidden memory list body token"; do
  if grep -Fq "$leaked" <<<"$comments"; then
    die "memory list report leaked body token ${leaked}"
  fi
done
