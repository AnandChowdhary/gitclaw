...
log() {
  echo "workspace-risk-report-e2e: $*" >&2
}
...
hidden_token="GITCLAW_WORKSPACE_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_WORKSPACE_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /workspace risk e2e ${timestamp}"
body="@gitclaw /workspace risk

Live workspace-risk E2E.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "workspace-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one workspace risk report comment"
...
for expected in \
  ...
  'current_issue_workspace_request=`true`' \
  'issue_body_scanned=`false`' \
  'comment_bodies_scanned=`false`' \
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "workspace risk report missing ${expected}"
done

for leaked in "$hidden_token" "GITCLAW_WORKSPACE_CONTEXT_V1" "This declarative workspace record" "repository checkout used by GitClaw" "$search_phrase"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "workspace risk report leaked ${leaked}"
  fi
done
