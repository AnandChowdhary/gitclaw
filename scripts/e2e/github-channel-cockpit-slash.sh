#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel cockpit queues provider-visible status boards and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-cockpit-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-cockpit-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-cockpit slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-cockpit-e2e-${timestamp}"
ingest_message_id="cockpit-ingest-${timestamp}"
notify_message_id="cockpit-notify-${timestamp}"
cockpit_id="cockpit-${timestamp}"
cockpit_lane="research"
cockpit_note="Keep the board skimmable"
account_id="telegram-cockpit-account-NOECHO_CHANNEL_COCKPIT_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_COCKPIT_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_COCKPIT_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_COCKPIT_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_COCKPIT_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_COCKPIT_CONTEXT_V1"
search_phrase="channel cockpit unique search fixture phrase"
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-cockpit slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-cockpit slash E2E.
...
cockpit_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels cockpit ${cockpit_lane} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --cockpit-id ${cockpit_id}
Note: ${cockpit_note}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$cockpit_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel cockpit action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel cockpit action receipt"
cockpit_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "llm_e2e_required_after_channel_cockpit_action_change: \`true\`"; do
  grep -Fq "$expected" <<<"$cockpit_receipt" || die "channel cockpit receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$cockpit_id" "$cockpit_lane" "$cockpit_note" "$expected_token" "OpenClaw" "Hermes" "Evidence [ready]"; do
  if grep -Fqi "$leaked" <<<"$cockpit_receipt"; then
    die "channel cockpit receipt leaked ${leaked}"
  fi
done

[[ "$(cockpit_notification_count)" == "1" ]] || die "channel cockpit did not queue exactly one notification"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$notification_bodies" || die "cockpit notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$cockpit_id" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$notification_bodies"; then
    die "cockpit notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing cockpit notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$cockpit_id" "$cockpit_lane" "$cockpit_note" "$expected_token" "OpenClaw" "Evidence [ready]"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels dashboard ${cockpit_lane} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --cockpit-id ${cockpit_id}
Note: ${cockpit_note}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel cockpit action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel cockpit receipt"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$duplicate_receipt" || die "duplicate channel cockpit receipt missing ${expected}"
done
[[ "$(cockpit_notification_count)" == "1" ]] || die "duplicate channel cockpit queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$cockpit_id" "$cockpit_lane" "$cockpit_note" "$expected_token" "OpenClaw" "Evidence [ready]"; do
  if grep -Fqi "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel cockpit receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel cockpit thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_COCKPIT_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, cockpit ids, lanes, notes, gauges, command text, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel cockpit model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel cockpit follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel cockpit search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel cockpit follow-up marker did not use configured GitHub models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel cockpit follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel cockpit follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel cockpit follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel cockpit follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel cockpit follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$cockpit_id" "$cockpit_note" "$account_id"; do
  if grep -Fq "$leaked" <<<"$model_comment"; then
    die "model channel cockpit follow-up leaked ${leaked}"
  fi
done
