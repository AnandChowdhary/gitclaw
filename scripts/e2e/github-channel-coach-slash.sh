#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-coach slash action queues provider-visible next-move cards and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-coach-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-coach-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-coach slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-coach-e2e-${timestamp}"
ingest_message_id="coach-ingest-${timestamp}"
notify_message_id="coach-notify-${timestamp}"
coach_id="coach-${timestamp}"
coach_lane="skills"
coach_note="Use live posture"
account_id="telegram-coach-account-NOECHO_CHANNEL_COACH_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_COACH_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_COACH_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_COACH_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_COACH_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_COACH_CONTEXT_V1"
search_phrase="channel coach unique search fixture phrase"
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-coach slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-coach slash E2E.
...
coach_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels coach ${coach_lane} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --coach-id ${coach_id}
Note: ${coach_note}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$coach_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel coach action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel coach action receipt"
coach_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
...
[[ "$(coach_notification_count)" == "1" ]] || die "channel coach did not queue exactly one notification"
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels advise ${coach_lane} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --coach-id ${coach_id}
Note: ${coach_note}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel coach action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel coach receipt"
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel coach thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_COACH_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, coach ids, coach lanes, coach notes, coach recommendations, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel coach model follow-up"
...
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel coach follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel coach follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel coach follow-up marker missing prompt-visible tools"
