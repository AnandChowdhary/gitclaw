...
hidden_token="GITCLAW_SESSION_TRAJECTORY_HIDDEN_${timestamp}"
comment_token="GITCLAW_SESSION_TRAJECTORY_COMMENT_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw session trajectory e2e ${timestamp}"
body="Live session trajectory E2E.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "session-trajectory e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
Please export the body-free assistant-turn trajectory manifest.
...
for expected in \
  ...
  'run_metadata_turns: `1`' \
  'unique_run_id_hashes: `1`' \
  'context_documents_total:' \
  'selected_skills_total: `1`' \
  'tool_outputs_total:' \
  ...
  'skills=`repo-reader`' \
  'tools=`gitclaw.list_files, gitclaw.search_files, gitclaw.skill_index`' \
  'usage_present=`true`' \
  ...
  grep -Fq "$expected" <<<"$session_comment" || die "session trajectory report missing ${expected}"
done
