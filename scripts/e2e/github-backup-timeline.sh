...
token="GITCLAW_BACKUP_TIMELINE_E2E_${timestamp}"
followup_token="GITCLAW_BACKUP_TIMELINE_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /backup timeline e2e ${timestamp}"
body="Live backup timeline E2E.

Hidden backup timeline token: ${token}
This should produce a deterministic backup report, then the fetched backup branch should render a body-free chronological backup timeline."
...
cleanup() {
  ...
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "backup-timeline e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
for expected in \
  ...
  "requested_local_command: \`gitclaw backup timeline --root .gitclaw/backups --repo ${repo} --limit 20\`" \
...
for leaked in "$token" "$title"; do
  if grep -Fq "$leaked" <<<"$comments"; then
    die "backup report leaked issue-side timeline input ${leaked}"
  fi
done
...
for _ in {1..60}; do
  ...
  if [[ -f "$index_path" ]] &&
    ...
    timeline_output="$(go run ./cmd/gitclaw backup timeline --root "${backup_checkout}/.gitclaw/backups" --repo "$repo" --limit 5)"
    for expected in \
      ...
      'raw_bodies_included: `false`' \
      'llm_e2e_required_after_backup_timeline_change: `true`' \
      "### Timeline Points" \
      "issue=#${issue_number}" \
      "path=\`${issue_path}\`" \
      'gap_seconds_since_previous=' \
      'payload_sha256_12=' \
      'assistant_turn_comments=' \
      'title_sha256_12='; do
      grep -Fq "$expected" <<<"$timeline_output" || die "backup timeline missing ${expected}"
    done
    if grep -Fq "$token" <<<"$timeline_output"; then
      die "backup timeline leaked issue body token"
    fi
    if grep -Fq "$title" <<<"$timeline_output"; then
      die "backup timeline leaked issue title"
    fi
    break
  fi
  sleep 5
...
if [[ -z "${timeline_output:-}" ]]; then
  die "backup timeline did not observe issue #${issue_number} in ${backup_branch}"
fi
