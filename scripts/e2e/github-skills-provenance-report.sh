...
log() {
  echo "skills-provenance-report-e2e: $*" >&2
}
...
hidden_token="GITCLAW_SKILL_PROVENANCE_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_SKILL_PROVENANCE_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /skills provenance e2e ${timestamp}"
body="@gitclaw /skills provenance

Live skills-provenance E2E.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "skills-provenance-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one skills provenance report comment"
...
for expected in \
  ...
  "### Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$provenance_comment" || die "skills provenance report missing ${expected}"
done

for leaked in "$hidden_token" "GITCLAW_SKILL_CONTEXT_V1" "Use GitClaw's read-only repository context" "Add skill provenance"; do
  if grep -Fq "$leaked" <<<"$provenance_comment"; then
    die "skills provenance report leaked ${leaked}"
  fi
done
