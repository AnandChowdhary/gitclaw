...
followup_hidden_token="NOECHO_MIGRATION_PLAN_FOLLOWUP_${timestamp}"
followup_expected_token="GITCLAW_MIGRATION_PLAN_FOLLOWUP_CONTEXT_V1"
followup_search_phrase="gclaw-migration-plan-followup-needle"
...
url="$(jq -r '.url' <<<"$run_json")"
log "migration plan report verified for issue #${issue_number}: ${url}"
...
Do not answer with the migration report token, issue title, issue number, migration source, or any token from this issue/comments.
