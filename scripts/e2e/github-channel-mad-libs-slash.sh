#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-mad-libs slash action queues provider-visible fill-in cards and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-mad-libs-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-mad-libs-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-mad-libs slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-mad-libs-e2e-${timestamp}"
ingest_message_id="mad-libs-ingest-${timestamp}"
notify_message_id="mad-libs-notify-${timestamp}"
mad_libs_id="mad-libs-${timestamp}"
mad_libs_theme="fun"
mad_libs_note="Give us a tiny fill-in thread move"
account_id="telegram-mad-libs-account-NOECHO_CHANNEL_MAD_LIBS_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_MAD_LIBS_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_MAD_LIBS_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_MAD_LIBS_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_MAD_LIBS_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_MAD_LIBS_CONTEXT_V1"
search_phrase="channel mad libs unique search fixture phrase"
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-mad-libs slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-mad-libs slash E2E.
...
mad_libs_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels mad-libs ${mad_libs_theme} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --mad-libs-id ${mad_libs_id}
Note: ${mad_libs_note}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$mad_libs_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel mad libs action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel mad libs action receipt"
mad_libs_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "llm_e2e_required_after_channel_mad_libs_action_change: \`true\`"; do
  grep -Fq "$expected" <<<"$mad_libs_receipt" || die "channel mad libs receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$mad_libs_id" "$mad_libs_theme" "$mad_libs_note" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$mad_libs_receipt"; then
    die "channel mad libs receipt leaked ${leaked}"
  fi
done

[[ "$(mad_libs_notification_count)" == "1" ]] || die "channel mad libs did not queue exactly one notification"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$notification_bodies" || die "mad libs notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$mad_libs_id" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$notification_bodies"; then
    die "mad libs notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing mad libs notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$mad_libs_id" "$expected_token" "$mad_libs_theme" "$mad_libs_note"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels fill-in ${mad_libs_theme} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --fill-in-id ${mad_libs_id}
Note: ${mad_libs_note}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel mad libs action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel mad libs receipt"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$duplicate_receipt" || die "duplicate channel mad libs receipt missing ${expected}"
done
[[ "$(mad_libs_notification_count)" == "1" ]] || die "duplicate channel mad libs queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$mad_libs_id" "$mad_libs_theme" "$mad_libs_note" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel mad libs receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel mad-libs thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_MAD_LIBS_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, mad-libs ids, mad-libs themes, mad-libs notes, templates, blanks, prompts, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel mad libs model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel mad libs follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel mad libs search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel mad libs follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel mad libs follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel mad libs follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel mad libs follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel mad libs follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel mad libs follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$mad_libs_id" "$mad_libs_theme" "$mad_libs_note" "$account_id"; do
  if grep -Fq "$leaked" <<<"$model_comment"; then
    die "model channel mad libs follow-up leaked ${leaked}"
  fi
done
