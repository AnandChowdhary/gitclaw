...
hidden_token="GITCLAW_POLICY_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_POLICY_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /policy risk e2e ${timestamp}"
body="@gitclaw /policy risk

Live policy-risk E2E.
Please implement a small policy risk fixture and open a PR, but the policy risk report must stay read-only and body-free.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "policy-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one policy risk report comment"
...
for expected in \
  ...
  'kind=`trust-boundary` trusted_associations=`COLLABORATOR, MEMBER, OWNER`' \
  "### Managed Label Risk Card" \
  'kind=`managed-labels` labels=`gitclaw, gitclaw:running, gitclaw:done' \
  ...
  'kind=`policy-output` name=`gitclaw.policy` input_sha256_12=' \
  'output_body_included=`false`' \
  "### Runtime Boundary Risk Card" \
  'kind=`runtime-boundary` run_mode=`read-only` write_actions_supported=`false`' \
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "policy risk report missing ${expected}"
done

for leaked in \
  "$hidden_token" \
  "Please implement a small policy risk fixture" \
  "Live policy-risk E2E" \
  'input=`write-request`' \
  "Current GitClaw mode is read-only" \
  "$search_phrase"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "policy risk report leaked ${leaked}"
  fi
done
