...
hidden_token="GITCLAW_NODES_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_NODES_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /nodes risk e2e ${timestamp}"
body="@gitclaw /nodes risk
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "nodes-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
for expected in \
  ...
  'llm_e2e_required_after_node_risk_change: `true`' \
  "### Node Policy Risk Card" \
  'kind=`node-policy` path=`.gitclaw/NODES.md`' \
  'risk_findings=`0`' \
  'risk_codes=`none`' \
  "### Node Spec Risk Cards" \
  'kind=`node-spec` name=`github-actions-runner` path=`.gitclaw/nodes/github-actions-runner.md`' \
  ...
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "nodes risk report missing ${expected}"
done

for leaked in "$hidden_token" "GITCLAW_NODES_CONTEXT_V1" "This declarative node record" "Node policy verification token" "bounded repository search fixture phrase"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "nodes risk report leaked ${leaked}"
  fi
done
