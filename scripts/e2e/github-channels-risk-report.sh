...
log() {
  echo "channels-risk-report-e2e: $*" >&2
}
...
hidden_token="GITCLAW_CHANNELS_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_CHANNELS_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /channels risk e2e ${timestamp}"
body="@gitclaw /channels risk

Live channels-risk E2E.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channels-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one channels risk report comment"
...
for expected in \
  ...
  'kind=`channel-message` none' \
  'risk_findings=`0`' \
  'risk_codes=`none`' \
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "channels risk report missing ${expected}"
done
...
for leaked in "$hidden_token" "module github.com/AnandChowdhary/gitclaw" "GITCLAW_SEARCH_CONTEXT_V1" "bounded repository search fixture phrase"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "channels risk report leaked ${leaked}"
  fi
done
