#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-skill-spotlight queues deterministic skill cards and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-skill-spotlight-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-skill-spotlight-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-skill-spotlight slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-skill-spotlight-e2e-${timestamp}"
ingest_message_id="skill-spotlight-ingest-${timestamp}"
notify_message_id="skill-spotlight-notify-${timestamp}"
skill_spotlight_id="skill-spotlight-${timestamp}"
skill_focus="repo-reader"
account_id="telegram-skill-spotlight-account-NOECHO_CHANNEL_SKILL_SPOTLIGHT_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_SKILL_SPOTLIGHT_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_SKILL_SPOTLIGHT_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_SKILL_SPOTLIGHT_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_SKILL_SPOTLIGHT_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_SKILL_SPOTLIGHT_CONTEXT_V1"
search_phrase="channel skill spotlight unique search fixture phrase"
...
skill_spotlight_notification_count() {
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-skill-spotlight slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-skill-spotlight slash E2E.
...
skill_spotlight_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels skill-spotlight ${skill_focus} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --spotlight-id ${skill_spotlight_id}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$skill_spotlight_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel skill-spotlight action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel skill-spotlight action receipt"
skill_spotlight_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "llm_e2e_required_after_channel_skill_spotlight_change: \`true\`"; do
  grep -Fq -- "$expected" <<<"$skill_spotlight_receipt" || die "channel skill-spotlight receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$skill_spotlight_id" "$skill_focus" ".gitclaw/SKILLS" "Use GitClaw's read-only repository context" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$skill_spotlight_receipt"; then
    die "channel skill-spotlight receipt leaked ${leaked}"
  fi
done

[[ "$(skill_spotlight_notification_count)" == "1" ]] || die "channel skill-spotlight did not queue exactly one notification"
...
for expected in \
  ...
  grep -Fq -- "$expected" <<<"$notification_bodies" || die "skill-spotlight notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$skill_spotlight_id" "Use GitClaw's read-only repository context" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$notification_bodies"; then
    die "skill-spotlight notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing skill-spotlight notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$skill_spotlight_id" "$skill_focus" "$expected_token" ".gitclaw/SKILLS"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels skill-draw ${skill_focus} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --spotlight-id ${skill_spotlight_id}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel skill-spotlight action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel skill-spotlight receipt"
...
for expected in \
  ...
  grep -Fq -- "$expected" <<<"$duplicate_receipt" || die "duplicate channel skill-spotlight receipt missing ${expected}"
done
[[ "$(skill_spotlight_notification_count)" == "1" ]] || die "duplicate channel skill-spotlight queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$skill_spotlight_id" "$skill_focus" ".gitclaw/SKILLS" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel skill-spotlight receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel skill-spotlight thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_SKILL_SPOTLIGHT_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, spotlight ids, issue numbers, skill names, skill paths, focus text, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel skill-spotlight model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel skill-spotlight follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq -- "$expected_token" <<<"$model_comment" || die "assistant did not include channel skill-spotlight search_files token ${expected_token}"
if ! grep -Fq -- 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq -- 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel skill-spotlight follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq -- 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel skill-spotlight follow-up marker missing prompt context hash"
grep -Fq -- 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel skill-spotlight follow-up marker missing selected repo-reader skill"
grep -Fq -- 'tools="' <<<"$model_comment" || die "assistant channel skill-spotlight follow-up marker missing prompt-visible tools"
grep -Fq -- 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel skill-spotlight follow-up marker did not prove search_files was prompt-visible"
grep -Fq -- 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel skill-spotlight follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$skill_spotlight_id" "$account_id"; do
  if grep -Fq -- "$leaked" <<<"$model_comment"; then
    die "model channel skill-spotlight follow-up leaked ${leaked}"
  fi
done
