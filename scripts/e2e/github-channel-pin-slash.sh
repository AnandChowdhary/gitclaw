#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-pin slash action queues the default provider pin reaction.
set -euo pipefail

log() {
  echo "channel-pin-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-pin-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-pin slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-pin-e2e-${timestamp}"
ingest_message_id="pin-ingest-${timestamp}"
reaction="pushpin"
account_id="telegram-pin-account-NOECHO_CHANNEL_PIN_ACCOUNT_${timestamp}"
external_message_id="telegram-pin-delivered-NOECHO_CHANNEL_PIN_EXTERNAL_${timestamp}"
gateway_run_id="channel-pin-slash-e2e-${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_PIN_INGEST_${timestamp}"
source_hidden_token="NOECHO_CHANNEL_PIN_SOURCE_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_PIN_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_PIN_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_PIN_CONTEXT_V1"
search_phrase="channel pin unique search fixture phrase"
account_hash="$(sha256_12 "$account_id")"
external_hash="$(sha256_12 "$external_message_id")"
pin_message_hash="$(sha256_12 "${ingest_message_id}|${reaction}")"
issue_number=""
state_issue=""
  ...
  for number in "$issue_number" "$state_issue"; do
    if [[ -n "${number:-}" && "$number" != "null" ]]; then
      ...
      if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
        gh issue close "$number" --repo "$repo" --comment "channel-pin slash e2e cleanup" >/dev/null 2>&1 || true
      fi
    fi
  done
  rm -rf "$lock_dir"
...
Mirrored Telegram thread for channel-pin slash E2E.
...
pin_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels pin --message-id ${ingest_message_id}

Queue only a provider pin.
Do not include this source hidden token in the receipt: ${source_hidden_token}" >/dev/null

wait_for_issue_comment_run "$pin_started_at" >/dev/null || die "timed out waiting for channel pin action"
wait_for_assistant_count 2 || die "expected channel pin action receipt"
pin_receipt="$(latest_assistant_comment)"
for expected in \
  "GitClaw Channel Reaction Action" \
  "Generated without a model call" \
  'model="gitclaw/channels"' \
  "requested_channel_command: \`/channels pin\`" \
  ...
  grep -Fq "$expected" <<<"$pin_receipt" || die "channel pin receipt missing ${expected}"
done
for leaked in "$source_hidden_token" "$ingest_hidden_token" "$ingest_message_id" "$thread_id" "$reaction"; do
  if grep -Fq "$leaked" <<<"$pin_receipt"; then
    die "channel pin receipt leaked ${leaked}"
  fi
done

[[ "$(pin_comment_count)" == "1" ]] || die "channel pin did not queue exactly one reaction comment"
source_comment_id="$(sed -n 's/.*reaction_comment_id: `\([0-9][0-9]*\)`.*/\1/p' <<<"$pin_receipt" | head -n 1)"
[[ -n "$source_comment_id" && "$source_comment_id" != "null" ]] || die "could not resolve pin source comment id"
...
grep -Fq 'reaction="pushpin"' <<<"$(jq -r '[.comments[].body] | join("\n")' <<<"$issue_json")" || die "channel issue missing default pushpin reaction marker"
...
jq -e --arg hash "$pin_message_hash" '.messages[] | select(.kind == "channel-reaction" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing pin hash ${pin_message_hash}"
...
wait_for_issue_comment_run "$duplicate_started_at" >/dev/null || die "timed out waiting for duplicate channel pin action"
wait_for_assistant_count 3 || die "expected duplicate channel pin receipt"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$duplicate_receipt" || die "duplicate channel pin receipt missing ${expected}"
done
[[ "$(pin_comment_count)" == "1" ]] || die "duplicate channel pin queued another reaction comment"
for leaked in "$duplicate_hidden_token" "$ingest_message_id" "$thread_id" "$reaction"; do
  if grep -Fq "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel pin receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel pin thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_PIN_\`.
...
model_run_json="$(wait_for_issue_comment_run "$model_started_at")" || die "timed out waiting for channel pin model follow-up"
wait_for_assistant_count 4 || die "expected model-backed channel pin follow-up"
model_comment="$(latest_assistant_comment)"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel pin search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel pin follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel pin follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel pin follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel pin follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel pin follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel pin follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$source_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$account_id" "$external_message_id"; do
  if grep -Fq "$leaked" <<<"$model_comment"; then
    die "model channel pin follow-up leaked ${leaked}"
  fi
done
