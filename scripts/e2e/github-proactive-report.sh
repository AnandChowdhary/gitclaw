...
token="NOECHO_PROACTIVE_REPORT_${timestamp}"
followup_hidden_token="NOECHO_PROACTIVE_REPORT_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_PROACTIVE_REPORT_CONTEXT_V1"
search_phrase="proactive report unique search fixture phrase"
...
if grep -Fq "$expected_token" <<<"$comments" || grep -Fq "$search_phrase" <<<"$comments"; then
  die "proactive report leaked follow-up fixture context"
fi

url="$(jq -r '.url' <<<"$run_json")"
log "proactive report verified for issue #${issue_number}: ${url}"
