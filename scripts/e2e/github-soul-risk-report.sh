...
hidden_token="GITCLAW_SOUL_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_SOUL_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /soul risk e2e ${timestamp}"
body="@gitclaw /soul risk
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "soul-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one soul risk report comment"
...
for expected in \
  ...
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "soul risk report missing ${expected}"
done
...
for leaked in "$hidden_token" "GitClaw remembers durable product decisions" "GitClaw is a repo-native GitHub issue assistant"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "soul risk report leaked ${leaked}"
  fi
done
