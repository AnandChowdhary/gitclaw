...
log() {
  echo "profile-risk-report-e2e: $*" >&2
}
...
hidden_token="GITCLAW_PROFILE_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_PROFILE_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /profile risk e2e ${timestamp}"
body="@gitclaw /profile risk

Live profile-risk E2E.
Use repo-reader while inspecting the profile risk surface.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "profile-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one profile risk report comment"
...
for expected in \
  ...
  'kind=`profile-document` path=`.gitclaw/IDENTITY.md` category=`identity` required=`true`' \
  'kind=`profile-document` path=`.gitclaw/USER.md` category=`user` required=`true`' \
  'kind=`profile-document` path=`.gitclaw/TOOLS.md` category=`tools` required=`true`' \
  'kind=`profile-document` path=`.gitclaw/MEMORY.md` category=`memory` required=`true`' \
  'kind=`profile-document` path=`.gitclaw/HEARTBEAT.md` category=`heartbeat` required=`true`' \
  'kind=`profile-document` path=`.gitclaw/WORKSPACE.md` category=`workspace` required=`false`' \
  'kind=`profile-document` path=`.gitclaw/memory/2026-05-29.md` category=`memory-note` required=`false`' \
  "### Skill Profile Risk Cards" \
  'kind=`profile-skill` name=`repo-reader` enabled=`true` selected=`true`' \
  'body_scanned=`false`' \
  "### Current Profile Request Risk Card" \
  'current_issue_profile_request=`true`' \
  'issue_body_scanned=`false`' \
  'comment_bodies_scanned=`false`' \
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "profile risk report missing ${expected}"
done

for leaked in "$hidden_token" "Use repo-reader while inspecting the profile risk surface" "GITCLAW_MEMORY_CONTEXT_V1" "GitClaw is a repo-native GitHub issue assistant" "$search_phrase"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "profile risk report leaked ${leaked}"
  fi
done
