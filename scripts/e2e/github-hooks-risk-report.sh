...
hidden_token="GITCLAW_HOOKS_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_HOOKS_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /hooks risk e2e ${timestamp}"
body="@gitclaw /hooks risk
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "hooks-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
for expected in \
  ...
  'llm_e2e_required_after_hook_risk_change: `true`' \
  "### Hook Policy Risk Card" \
  'kind=`hook-policy` path=`.gitclaw/HOOKS.md`' \
  'risk_findings=`0`' \
  'risk_codes=`none`' \
  ...
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "hooks risk report missing ${expected}"
done

for leaked in "$hidden_token" "GITCLAW_HOOKS_CONTEXT_V1" "This declarative hook records" "Hook policy verification token" "bounded repository search fixture phrase"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "hooks risk report leaked ${leaked}"
  fi
done
