...
token="NOECHO_TOOLS_LIST_REPORT_${timestamp}"
followup_hidden_token="NOECHO_TOOLS_LIST_REPORT_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_TOOLS_LIST_REPORT_CONTEXT_V1"
search_phrase="tools list report unique search fixture phrase"
title="@gitclaw /tools list e2e ${timestamp}"
body="Live tools-list E2E.

Please inspect \`go.mod\` and search for \`bounded repository search fixture phrase\`.
Hidden tools list body token: ${token}
This should produce a deterministic tools list report without calling a model."
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "tools-list-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one tools list report comment"
...
for expected in \
  ...
  'input=`go.mod`' \
  "sha256_12="; do
  grep -Fq -- "$expected" <<<"$comments" || die "tools list report missing ${expected}"
done

for leaked in "$token" "module github.com/AnandChowdhary/gitclaw" "GITCLAW_SEARCH_CONTEXT_V1" "Hidden tools list body token"; do
  if grep -Fq "$leaked" <<<"$comments"; then
    die "tools list report leaked ${leaked}"
  fi
done

wait_for_done_status || die "expected gitclaw:done without running/error"
url="$(jq -r '.url' <<<"$run_json")"
log "tools list report verified for issue #${issue_number}: ${url}"
...
Do not answer with the tools-list alias nonce, tool count, issue title, issue number, tool name, validation status, or any token from this issue/comments.
