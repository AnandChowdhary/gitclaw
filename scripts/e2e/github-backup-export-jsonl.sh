...
log() {
  echo "backup-export-jsonl-e2e: $*" >&2
}
...
token="NOECHO_BACKUP_EXPORT_JSONL_${timestamp}"
followup_hidden_token="NOECHO_BACKUP_EXPORT_JSONL_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_BACKUP_EXPORT_JSONL_CONTEXT_V1"
search_phrase="backup export jsonl unique search fixture phrase"
title="@gitclaw /backup export-jsonl e2e ${timestamp}"
body="Live backup export-jsonl E2E.

Hidden backup export token: ${token}
This should produce a deterministic backup report, then the fetched backup branch should export this transcript as JSONL."
...
cleanup() {
  ...
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "backup export-jsonl e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
if grep -Fq "$expected_token" <<<"$comments" || grep -Fq "$search_phrase" <<<"$comments"; then
  die "backup export report leaked follow-up fixture context"
fi
...
for _ in {1..60}; do
  ...
  if [[ -f "$index_path" ]] &&
    ...
    export_output="$(go run ./cmd/gitclaw backup export-jsonl --root "${backup_checkout}/.gitclaw/backups" --repo "$repo" --issue "$issue_number")"
    line_count="$(wc -l <<<"$export_output" | tr -d ' ')"
    [[ "$line_count" == "2" ]] || die "export produced ${line_count} JSONL lines, want 2"

    jq -e --arg repo "$repo" --argjson number "$issue_number" --arg token "$token" '
      select(.schema == "gitclaw.backup.transcript.v1")
      | select(.repo == $repo)
      | select(.issue_number == $number)
      | select(.sequence == 1)
      | select(.source == "issue")
      | select(.role == "user")
      | select(.body | contains($token))
    ' <<<"$(head -n 1 <<<"$export_output")" >/dev/null || die "first JSONL record missing issue transcript token"

    jq -e --argjson number "$issue_number" '
      select(.schema == "gitclaw.backup.transcript.v1")
      | select(.issue_number == $number)
      | select(.sequence == 2)
      | select(.source | startswith("comment:"))
      | select(.role == "assistant")
      | select(.body | contains("GitClaw Backup Report"))
      | select(.body_sha256_12 | length == 12)
    ' <<<"$(tail -n 1 <<<"$export_output")" >/dev/null || die "second JSONL record missing assistant backup report"
...
[[ -n "${url:-}" ]] || die "backup export did not observe issue #${issue_number} in ${backup_branch}"
