...
token="NOECHO_CHECKPOINTS_REPORT_${timestamp}"
followup_hidden_token="NOECHO_CHECKPOINTS_REPORT_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHECKPOINTS_REPORT_CONTEXT_V1"
search_phrase="checkpoints report unique search fixture phrase"
...
url="$(jq -r '.url' <<<"$run_json")"
log "checkpoints report verified for issue #${issue_number}: ${url}"
...
Do not answer with the rollback nonce, issue title, issue number, commit hash, backup branch, or any token from this issue/comments.
