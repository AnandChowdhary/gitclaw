...
token="GITCLAW_SOUL_VERIFY_E2E_${timestamp}"
title="@gitclaw /soul verify e2e ${timestamp}"
body="Live soul-verify E2E.

Hidden soul verify body token: ${token}
This should produce a deterministic soul trust envelope without calling a model or dumping context bodies."
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "soul-verify-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one soul verify report comment"
...
for expected in \
  ...
  'code=`profile_export_verification_not_configured`'; do
  grep -Fq -- "$expected" <<<"$comments" || die "soul verify report missing ${expected}"
done

for leaked in "$token" "Hidden soul verify body token" "GitClaw remembers durable product decisions" "GitClaw is a repo-native GitHub issue assistant"; do
  if grep -Fq "$leaked" <<<"$comments"; then
    die "soul verify report leaked ${leaked}"
  fi
done
