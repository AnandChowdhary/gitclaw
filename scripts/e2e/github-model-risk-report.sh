...
hidden_token="GITCLAW_MODEL_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_MODEL_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /models risk e2e ${timestamp}"
body="@gitclaw /models risk
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "model-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one model risk report comment"
...
for expected in \
  ...
  'current_issue_model_request=`true`' \
  'issue_body_scanned=`false`' \
  'comment_bodies_scanned=`false`' \
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "model risk report missing ${expected}"
done
...
for leaked in "$hidden_token" "Use repo-reader after the deterministic report" "$search_phrase"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "model risk report leaked ${leaked}"
  fi
done
