...
hidden_token="GITCLAW_ORDERS_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_ORDERS_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /orders risk e2e ${timestamp}"
body="@gitclaw /orders risk

Live standing-orders risk E2E.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "orders-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one standing-orders risk report comment"
...
for expected in \
  ...
  'agent_authored_order_mutation_supported: `false`' \
  ...
  'path=`.gitclaw/STANDING_ORDERS.md`' \
  'risk_findings=`0`' \
  'risk_codes=`none`' \
  "### Program Risk Cards" \
  'program=`01`' \
  ...
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "standing-orders risk report missing ${expected}"
done

for leaked in "$hidden_token" "GITCLAW_STANDING_ORDERS_CONTEXT_V1" "Repository Stewardship" "Commit and push each completed feature batch"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "standing-orders risk report leaked ${leaked}"
  fi
done
