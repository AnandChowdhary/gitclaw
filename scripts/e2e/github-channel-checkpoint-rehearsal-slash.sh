#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel rehearse-checkpoint creates a checkpoint rollback rehearsal issue and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-checkpoint-rehearsal-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-checkpoint-rehearsal-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel checkpoint rehearsal slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-checkpoint-rehearsal-e2e-${timestamp}"
ingest_message_id="checkpoint-rehearsal-ingest-${timestamp}"
rehearsal_id="channel-checkpoint-rehearsal-${timestamp}"
notify_message_id="checkpoint-rehearsal-notify-${timestamp}"
target_ref="HEAD~1"
target_ref_hash="$(sha256_12 "$target_ref")"
account_id="telegram-checkpoint-rehearsal-account-NOECHO_CHANNEL_CHECKPOINT_REHEARSAL_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_CHECKPOINT_REHEARSAL_INGEST_${timestamp}"
source_hidden_token="NOECHO_CHANNEL_CHECKPOINT_REHEARSAL_SOURCE_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_CHECKPOINT_REHEARSAL_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_CHECKPOINT_REHEARSAL_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_CHECKPOINT_REHEARSAL_CONTEXT_V1"
search_phrase="channel checkpoint rehearsal unique search fixture phrase"
notify_message_hash="$(sha256_12 "$notify_message_id")"
issue_number=""
rehearsal_issue_number=""
issue_title="GitClaw ${channel} thread ${thread_id}"
rehearsal_issue_title="GitClaw checkpoint rehearsal: ${rehearsal_id}"
...
checkpoint_rehearsal_notification_count() {
  ...
  for number in "$issue_number" "$rehearsal_issue_number"; do
    if [[ -n "${number:-}" && "$number" != "null" ]]; then
      ...
      if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
        gh issue close "$number" --repo "$repo" --comment "channel checkpoint rehearsal slash e2e cleanup" >/dev/null 2>&1 || true
      fi
    fi
  done
  rm -rf "$lock_dir"
...
Mirrored Telegram thread for channel checkpoint rehearsal slash E2E.
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels rehearse-checkpoint --target ${target_ref} --id ${rehearsal_id} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id}

Please open a channel-origin checkpoint rollback rehearsal for the mirrored thread.
Do not include this hidden source token: ${source_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$request_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel checkpoint rehearsal action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel checkpoint rehearsal action receipt"
...
for expected in \
  ...
  "target_ref_sha256_12: \`${target_ref_hash}\`" \
  "target_allowed: \`true\`" \
  ...
  "llm_e2e_required_after_channel_checkpoint_rehearsal_change: \`true\`"; do
  grep -Fq "$expected" <<<"$request_receipt" || die "channel checkpoint rehearsal receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$source_hidden_token" "$rehearsal_id" "$target_ref" "$thread_id" "$ingest_message_id" "$notify_message_id" "Please open a channel-origin checkpoint rollback rehearsal"; do
  if grep -Fq "$leaked" <<<"$request_receipt"; then
    die "channel checkpoint rehearsal receipt leaked ${leaked}"
  fi
done

rehearsal_issue_number="$(sed -n 's/.*rehearsal_issue: `#\([0-9][0-9]*\)`.*/\1/p' <<<"$request_receipt" | head -n 1)"
[[ -n "$rehearsal_issue_number" && "$rehearsal_issue_number" != "null" ]] || die "could not resolve checkpoint rehearsal issue number"
log "channel checkpoint rehearsal created rehearsal issue #${rehearsal_issue_number}"

[[ "$(checkpoint_rehearsal_notification_count)" == "1" ]] || die "channel checkpoint rehearsal did not queue exactly one rehearsal-link outbound comment"
...
[[ "$rehearsal_title" == "$rehearsal_issue_title" ]] || die "unexpected checkpoint rehearsal issue title: ${rehearsal_title}"
grep -Fxq "gitclaw" <<<"$rehearsal_labels" || die "checkpoint rehearsal issue missing gitclaw label"
for expected in \
  ...
  grep -Fq "$expected" <<<"$rehearsal_body" || die "checkpoint rehearsal issue body missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$source_hidden_token" "Please open a channel-origin checkpoint rollback rehearsal"; do
  if grep -Fq "$leaked" <<<"$rehearsal_body"; then
    die "checkpoint rehearsal issue body leaked ${leaked}"
  fi
done
...
grep -Fq "gitclaw:channel-outbound" <<<"$outbound_bodies" || die "channel issue missing checkpoint rehearsal outbound marker"
grep -Fq "GitClaw channel checkpoint rehearsal" <<<"$outbound_bodies" || die "channel issue missing visible checkpoint rehearsal notification"
grep -Fq "Target ref hash: ${target_ref_hash}" <<<"$outbound_bodies" || die "channel issue missing target hash in notification"
for leaked in "$source_hidden_token" "$rehearsal_id" "$target_ref"; do
  if grep -Fq "$leaked" <<<"$outbound_bodies"; then
    die "channel checkpoint rehearsal notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing checkpoint rehearsal outbound hash ${notify_message_hash}"
for leaked in "$rehearsal_id" "$target_ref" "$source_hidden_token" "$account_id" "$ingest_hidden_token"; do
...
wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel checkpoint rehearsal action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel checkpoint rehearsal receipt"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$duplicate_receipt" || die "duplicate channel checkpoint rehearsal receipt missing ${expected}"
done
[[ "$(checkpoint_rehearsal_notification_count)" == "1" ]] || die "duplicate channel checkpoint rehearsal queued another rehearsal-link notification"
for leaked in "$duplicate_hidden_token" "$rehearsal_id" "$target_ref" "$thread_id" "$ingest_message_id" "$notify_message_id"; do
  if grep -Fq "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel checkpoint rehearsal receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$rehearsal_issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel-created checkpoint rollback rehearsal and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_CHECKPOINT_REHEARSAL_\`.
...
Do not include rehearsal ids, target refs, message ids, thread ids, account hashes, issue numbers, branch names, source bodies, raw diffs, file bodies, or restore commands.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$rehearsal_issue_title")" || die "timed out waiting for channel checkpoint rehearsal model follow-up"
wait_for_assistant_count_for_issue "$rehearsal_issue_number" 1 || die "expected model-backed channel checkpoint rehearsal follow-up"
model_comment="$(latest_assistant_comment_for_issue "$rehearsal_issue_number")"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel checkpoint rehearsal search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel checkpoint rehearsal follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel checkpoint rehearsal follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel checkpoint rehearsal follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel checkpoint rehearsal follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel checkpoint rehearsal follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel checkpoint rehearsal follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$source_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$account_id" "$rehearsal_id" "$target_ref" "$notify_message_id"; do
  if grep -Fq "$leaked" <<<"$model_comment"; then
    die "model channel checkpoint rehearsal follow-up leaked ${leaked}"
  fi
done
