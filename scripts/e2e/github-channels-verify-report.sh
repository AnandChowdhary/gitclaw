...
log() {
  echo "channels-verify-report-e2e: $*" >&2
}
...
token="NOECHO_CHANNELS_VERIFY_${timestamp}"
followup_hidden_token="NOECHO_CHANNELS_VERIFY_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNELS_VERIFY_CONTEXT_V1"
search_phrase="channels verify unique search fixture phrase"
title="@gitclaw /channels verify e2e ${timestamp}"
body="Live channels verify E2E.

Hidden channels verify token: ${token}
This should produce the deterministic channel bridge verification report."
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channels-verify-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one channels verify report comment"
comments="$(assistant_comments)"

for expected in \
  'model="gitclaw/channels"' \
  "GitClaw Channel Verify Report" \
  "Generated without a model call" \
  'channel_verify_status: `ok`' \
  ...
  'delivery_workflow_path: `.github/workflows/gitclaw-channel-delivery.yml`' \
...
if grep -Fq "$token" <<<"$comments"; then
  die "channels verify report leaked hidden token"
fi
if grep -Fq "$expected_token" <<<"$comments" || grep -Fq "$search_phrase" <<<"$comments"; then
  die "channels verify report leaked follow-up fixture context"
fi

url="$(jq -r '.url' <<<"$run_json")"
log "channels verify report verified for issue #${issue_number}: ${url}"
