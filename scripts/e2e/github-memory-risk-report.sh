...
hidden_token="GITCLAW_MEMORY_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_MEMORY_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /memory risk e2e ${timestamp}"
body="@gitclaw /memory risk

Live memory-risk E2E.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "memory-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one memory risk report comment"
...
for expected in \
  ...
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "memory risk report missing ${expected}"
done
...
for leaked in "$hidden_token" "Durable memory token" "GITCLAW_MEMORY_CONTEXT_V1" "GitClaw remembers durable product decisions" "bounded repository search fixture phrase"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "memory risk report leaked ${leaked}"
  fi
done
