...
log() {
  echo "tools-verify-report-e2e: $*" >&2
}
...
token="NOECHO_TOOLS_VERIFY_REPORT_${timestamp}"
followup_hidden_token="NOECHO_TOOLS_VERIFY_REPORT_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_TOOLS_VERIFY_REPORT_CONTEXT_V1"
search_phrase="tools verify report unique search fixture phrase"
title="@gitclaw /tools verify e2e ${timestamp}"
body="Live tools-verify E2E.

Inspect go.mod and search for \"bounded repository search fixture phrase\".
Hidden tools verify body token: ${token}
This should produce a deterministic tool trust envelope without dumping tool inputs, outputs, or issue bodies."
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "tools-verify-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one tools verify report comment"
...
for expected in \
  ...
  'code=`runtime_permission_verification_static_only`'; do
  grep -Fq -- "$expected" <<<"$comments" || die "tools verify report missing ${expected}"
done

leaks=(
  "$token"
  "Hidden tools verify body token"
...
for leaked in "${leaks[@]}"; do
  if grep -Fq "$leaked" <<<"$comments"; then
    die "tools verify report leaked ${leaked}"
  fi
done

wait_for_done_status || die "expected gitclaw:done without running/error"
url="$(jq -r '.url' <<<"$run_json")"
log "tools verify report verified for issue #${issue_number}: ${url}"
...
Do not answer with the tools-verify nonce, trust-card count, issue title, issue number, tool name, verification status, or any token from this issue/comments.
