#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-availability slash action queues provider-visible presence cards.
set -euo pipefail

log() {
  echo "channel-availability-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-availability-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-availability slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-availability-e2e-${timestamp}"
ingest_message_id="availability-ingest-${timestamp}"
notify_message_id="availability-notify-${timestamp}"
availability_id="availability-${timestamp}"
account_id="telegram-availability-account-NOECHO_CHANNEL_AVAILABILITY_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_AVAILABILITY_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_AVAILABILITY_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_AVAILABILITY_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_AVAILABILITY_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_AVAILABILITY_CONTEXT_V1"
search_phrase="channel availability unique search fixture phrase"
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-availability slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-availability slash E2E.
...
availability_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels availability --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --availability-id ${availability_id} --state available
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$availability_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel availability action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel availability action receipt"
availability_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "llm_e2e_required_after_channel_availability_action_change: \`true\`"; do
  grep -Fq "$expected" <<<"$availability_receipt" || die "channel availability receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$availability_id"; do
  if grep -Fq "$leaked" <<<"$availability_receipt"; then
    die "channel availability receipt leaked ${leaked}"
  fi
done

[[ "$(availability_notification_count)" == "1" ]] || die "channel availability did not queue exactly one availability notification"
...
for expected in \
  ...
  "@gitclaw /channels reminder --message-id <id>"; do
  grep -Fq "$expected" <<<"$notification_bodies" || die "availability notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$availability_id"; do
  if grep -Fq "$leaked" <<<"$notification_bodies"; then
    die "availability notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing availability notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$availability_id"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels online --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --availability-id ${availability_id} --state available
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel availability action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel availability receipt"
...
for expected in \
  ...
  "provider_api_call_performed: \`false\`" \
  "repository_mutation_performed: \`false\`" \
  "workflow_mutation_performed: \`false\`"; do
  grep -Fq "$expected" <<<"$duplicate_receipt" || die "duplicate channel availability receipt missing ${expected}"
done
[[ "$(availability_notification_count)" == "1" ]] || die "duplicate channel availability queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$availability_id"; do
  if grep -Fq "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel availability receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel availability thread and use the repo-reader skill.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, availability ids, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel availability model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel availability follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel availability search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel availability follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel availability follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel availability follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel availability follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel availability follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel availability follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$availability_id" "$account_id"; do
  if grep -Fq "$leaked" <<<"$model_comment"; then
    die "model channel availability follow-up leaked ${leaked}"
  fi
done
