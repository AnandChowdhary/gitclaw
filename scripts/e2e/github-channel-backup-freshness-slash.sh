#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel backup-freshness queues archive freshness cards and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-backup-freshness-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-backup-freshness-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel backup-freshness slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-backup-freshness-e2e-${timestamp}"
ingest_message_id="backup-freshness-ingest-${timestamp}"
notify_message_id="backup-freshness-notify-${timestamp}"
freshness_id="channel-backup-freshness-${timestamp}"
max_age_hours="8760"
account_id="telegram-backup-freshness-account-NOECHO_CHANNEL_BACKUP_FRESHNESS_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_BACKUP_FRESHNESS_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_BACKUP_FRESHNESS_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_BACKUP_FRESHNESS_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_BACKUP_FRESHNESS_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_BACKUP_FRESHNESS_CONTEXT_V1"
search_phrase="channel backup freshness unique search fixture phrase"
...
backup_freshness_notification_count() {
...
cleanup() {
  ...
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel backup-freshness slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel backup-freshness slash E2E.
...
for _ in {1..60}; do
  ...
  if [[ -f "$index_path" ]] &&
    ...
    freshness_output="$(go run ./cmd/gitclaw backup freshness --root "${backup_checkout}/.gitclaw/backups" --repo "$repo" --max-age-hours "$max_age_hours")"
    break
  fi
  sleep 5
done

if [[ -z "$freshness_output" ]]; then
  die "channel backup-freshness did not observe source issue #${issue_number} in ${backup_branch}"
fi
for expected in \
  "GitClaw Backup Freshness Report" \
  "repository: \`${repo}\`" \
  ...
  'llm_e2e_required_after_backup_freshness_change: `true`'; do
  grep -Fq "$expected" <<<"$freshness_output" || die "local fetched backup-freshness output missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$issue_title"; do
  if grep -Fq "$leaked" <<<"$freshness_output"; then
    die "local fetched backup-freshness output leaked ${leaked}"
  fi
done

freshness_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels backup-freshness --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --freshness-id ${freshness_id} --max-age-hours ${max_age_hours}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$freshness_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel backup-freshness action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel backup-freshness action receipt"
freshness_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "llm_e2e_required_after_channel_backup_freshness_change: \`true\`"; do
  grep -Fq "$expected" <<<"$freshness_receipt" || die "channel backup-freshness receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$freshness_id" "$expected_token" "$repo_key"; do
  if grep -Fq "$leaked" <<<"$freshness_receipt"; then
    die "channel backup-freshness receipt leaked ${leaked}"
  fi
done

[[ "$(backup_freshness_notification_count)" == "1" ]] || die "channel backup-freshness did not queue exactly one notification"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$notification_bodies" || die "backup-freshness notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$freshness_id" "$expected_token" "$issue_title"; do
  if grep -Fq "$leaked" <<<"$notification_bodies"; then
    die "backup-freshness notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing backup-freshness notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$freshness_id" "$expected_token"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels archive-health --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --freshness-id ${freshness_id} --max-age-hours ${max_age_hours}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel backup-freshness action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel backup-freshness receipt"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$duplicate_receipt" || die "duplicate channel backup-freshness receipt missing ${expected}"
done
[[ "$(backup_freshness_notification_count)" == "1" ]] || die "duplicate channel backup-freshness queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$freshness_id" "$expected_token" "$repo_key"; do
  if grep -Fq "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel backup-freshness receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel backup-freshness thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_BACKUP_FRESHNESS_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, freshness ids, backup paths, backup branches, freshness gates, archive timestamps, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel backup-freshness model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel backup-freshness follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel backup-freshness search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel backup-freshness follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel backup-freshness follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel backup-freshness follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel backup-freshness follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel backup-freshness follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel backup-freshness follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$freshness_id" "$account_id"; do
  if grep -Fq "$leaked" <<<"$model_comment"; then
    die "model channel backup-freshness follow-up leaked ${leaked}"
  fi
done
