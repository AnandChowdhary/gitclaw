...
hidden_token="GITCLAW_SESSION_TOOLS_HIDDEN_${timestamp}"
comment_token="GITCLAW_SESSION_TOOLS_COMMENT_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw session tools e2e ${timestamp}"
body="Live session tools E2E.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "session-tools e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
Please audit assistant-turn tool provenance.
...
for expected in \
  ...
  "### Tool Ledger" \
  'tool=`gitclaw.search_files` prompt_visible_turns=`1` model_backed_turns=`1` deterministic_turns=`0`' \
  "### Tool Turn Evidence" \
  ...
  grep -Fq "$expected" <<<"$session_comment" || die "session tools report missing ${expected}"
done

for leaked in "$hidden_token" "$comment_token" "$search_phrase" "Please audit assistant-turn tool provenance"; do
  if grep -Fq "$leaked" <<<"$session_comment"; then
    die "session tools report leaked ${leaked}"
  fi
done
