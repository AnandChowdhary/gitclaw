...
hidden_token="GITCLAW_TOOLS_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_TOOLS_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /tools risk e2e ${timestamp}"
body="@gitclaw /tools risk
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "tools-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one tools risk report comment"
...
for expected in \
  ...
  "### Tool Risk Cards" \
  'kind=`contract` name=`gitclaw.search_files`' \
  'kind=`guidance` path=`.gitclaw/TOOLS.md`' \
  'kind=`active-output` name=`gitclaw.list_files`' \
  'risk_findings=`0`' \
  'risk_codes=`none`' \
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "tools risk report missing ${expected}"
done

for leaked in "$hidden_token" "module github.com/AnandChowdhary/gitclaw" "GITCLAW_SEARCH_CONTEXT_V1" "bounded repository search fixture phrase"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "tools risk report leaked ${leaked}"
  fi
done
