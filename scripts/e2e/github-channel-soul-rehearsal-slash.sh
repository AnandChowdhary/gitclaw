#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel rehearse-soul creates a soul rehearsal issue and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-soul-rehearsal-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-soul-rehearsal-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel soul rehearsal slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-soul-rehearsal-e2e-${timestamp}"
ingest_message_id="soul-rehearsal-ingest-${timestamp}"
rehearsal_id="channel-soul-rehearsal-${timestamp}"
notify_message_id="soul-rehearsal-notify-${timestamp}"
account_id="telegram-soul-rehearsal-account-NOECHO_CHANNEL_SOUL_REHEARSAL_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_SOUL_REHEARSAL_INGEST_${timestamp}"
source_hidden_token="NOECHO_CHANNEL_SOUL_REHEARSAL_SOURCE_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_SOUL_REHEARSAL_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_SOUL_REHEARSAL_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_SOUL_REHEARSAL_CONTEXT_V1"
search_phrase="channel soul rehearsal unique search fixture phrase"
notify_message_hash="$(sha256_12 "$notify_message_id")"
issue_number=""
rehearsal_issue_number=""
issue_title="GitClaw ${channel} thread ${thread_id}"
rehearsal_issue_title="GitClaw soul rehearsal: .gitclaw/SOUL.md (${rehearsal_id})"
...
soul_rehearsal_notification_count() {
  ...
  for number in "$issue_number" "$rehearsal_issue_number"; do
    if [[ -n "${number:-}" && "$number" != "null" ]]; then
      ...
      if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
        gh issue close "$number" --repo "$repo" --comment "channel soul rehearsal slash e2e cleanup" >/dev/null 2>&1 || true
      fi
    fi
  done
  rm -rf "$lock_dir"
...
Mirrored Telegram thread for channel soul rehearsal slash E2E.
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels rehearse-soul --target soul --id ${rehearsal_id} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id}

Please rehearse a channel-origin current-soul request.
Do not include this hidden source token: ${source_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$request_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel soul rehearsal action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel soul rehearsal action receipt"
...
for expected in \
  ...
  "llm_e2e_required_after_channel_soul_rehearsal_change: \`true\`"; do
  grep -Fq "$expected" <<<"$request_receipt" || die "channel soul rehearsal receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$source_hidden_token" "$rehearsal_id" "$thread_id" "$ingest_message_id" "$notify_message_id" "Please rehearse a channel-origin"; do
  if grep -Fq "$leaked" <<<"$request_receipt"; then
    die "channel soul rehearsal receipt leaked ${leaked}"
  fi
done

rehearsal_issue_number="$(sed -n 's/.*rehearsal_issue: `#\([0-9][0-9]*\)`.*/\1/p' <<<"$request_receipt" | head -n 1)"
[[ -n "$rehearsal_issue_number" && "$rehearsal_issue_number" != "null" ]] || die "could not resolve soul rehearsal issue number"
log "channel soul rehearsal created rehearsal issue #${rehearsal_issue_number}"

[[ "$(soul_rehearsal_notification_count)" == "1" ]] || die "channel soul rehearsal did not queue exactly one rehearsal-link outbound comment"
...
[[ "$rehearsal_title" == "$rehearsal_issue_title" ]] || die "unexpected soul rehearsal issue title: ${rehearsal_title}"
grep -Fxq "gitclaw" <<<"$rehearsal_labels" || die "soul rehearsal issue missing gitclaw label"
for expected in \
...
grep -Fq "gitclaw:channel-outbound" <<<"$outbound_bodies" || die "channel issue missing soul rehearsal outbound marker"
grep -Fq "GitClaw channel soul rehearsal" <<<"$outbound_bodies" || die "channel issue missing visible soul rehearsal notification"
grep -Fq "Target: .gitclaw/SOUL.md" <<<"$outbound_bodies" || die "channel issue missing target in notification"
if grep -Fq "$source_hidden_token" <<<"$outbound_bodies"; then
  die "channel soul rehearsal notification leaked source token"
fi
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing soul rehearsal outbound hash ${notify_message_hash}"
...
wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel soul rehearsal action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel soul rehearsal receipt"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$duplicate_receipt" || die "duplicate channel soul rehearsal receipt missing ${expected}"
done
[[ "$(soul_rehearsal_notification_count)" == "1" ]] || die "duplicate channel soul rehearsal queued another rehearsal-link notification"
for leaked in "$duplicate_hidden_token" "$rehearsal_id" "$thread_id" "$ingest_message_id" "$notify_message_id"; do
  if grep -Fq "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel soul rehearsal receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$rehearsal_issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel-created current-soul rehearsal and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_SOUL_REHEARSAL_\`.
...
Do not include rehearsal ids, message ids, thread ids, account hashes, issue numbers, source bodies, target bodies, or candidate soul text.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$rehearsal_issue_title")" || die "timed out waiting for channel soul rehearsal model follow-up"
wait_for_assistant_count_for_issue "$rehearsal_issue_number" 1 || die "expected model-backed channel soul rehearsal follow-up"
model_comment="$(latest_assistant_comment_for_issue "$rehearsal_issue_number")"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel soul rehearsal search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel soul rehearsal follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel soul rehearsal follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel soul rehearsal follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel soul rehearsal follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel soul rehearsal follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel soul rehearsal follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$source_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$account_id" "$rehearsal_id" "$notify_message_id"; do
  if grep -Fq "$leaked" <<<"$model_comment"; then
    die "model channel soul rehearsal follow-up leaked ${leaked}"
  fi
done
