#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-haiku slash action queues provider-visible deterministic poem cards and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-haiku-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-haiku-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-haiku slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-haiku-e2e-${timestamp}"
ingest_message_id="haiku-ingest-${timestamp}"
notify_message_id="haiku-notify-${timestamp}"
haiku_id="haiku-${timestamp}"
haiku_theme="launch"
haiku_note="Release handoff is steady"
account_id="telegram-haiku-account-NOECHO_CHANNEL_HAIKU_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_HAIKU_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_HAIKU_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_HAIKU_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_HAIKU_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_HAIKU_CONTEXT_V1"
search_phrase="channel haiku unique search fixture phrase"
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-haiku slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-haiku slash E2E.
...
haiku_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels haiku ${haiku_theme} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --haiku-id ${haiku_id}
Note: ${haiku_note}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$haiku_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel haiku action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel haiku action receipt"
haiku_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "llm_e2e_required_after_channel_haiku_action_change: \`true\`"; do
  grep -Fq "$expected" <<<"$haiku_receipt" || die "channel haiku receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$haiku_id" "$haiku_theme" "$haiku_note" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$haiku_receipt"; then
    die "channel haiku receipt leaked ${leaked}"
  fi
done

[[ "$(haiku_notification_count)" == "1" ]] || die "channel haiku did not queue exactly one notification"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$notification_bodies" || die "haiku notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$haiku_id" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$notification_bodies"; then
    die "haiku notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing haiku notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$haiku_id" "$expected_token" "$haiku_theme" "$haiku_note"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels poem ${haiku_theme} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --haiku-id ${haiku_id}
Note: ${haiku_note}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel haiku action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel haiku receipt"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$duplicate_receipt" || die "duplicate channel haiku receipt missing ${expected}"
done
[[ "$(haiku_notification_count)" == "1" ]] || die "duplicate channel haiku queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$haiku_id" "$haiku_theme" "$haiku_note" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel haiku receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel haiku thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_HAIKU_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, haiku ids, haiku themes, haiku notes, poem lines, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel haiku model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel haiku follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel haiku search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel haiku follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel haiku follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel haiku follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel haiku follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel haiku follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel haiku follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$haiku_id" "$haiku_theme" "$haiku_note" "$account_id"; do
  if grep -Fq "$leaked" <<<"$model_comment"; then
    die "model channel haiku follow-up leaked ${leaked}"
  fi
done
