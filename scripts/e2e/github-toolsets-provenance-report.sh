...
log() {
  echo "toolsets-provenance-report-e2e: $*" >&2
}
...
hidden_token="NOECHO_TOOLSET_PROVENANCE_SENTINEL_${timestamp}"
followup_hidden_token="NOECHO_TOOLSET_PROVENANCE_FOLLOWUP_SENTINEL_${timestamp}"
expected_token="GITCLAW_TOOLSET_PROVENANCE_CONTEXT_V1"
search_phrase="toolset provenance unique search fixture phrase"
title="@gitclaw /tools toolsets provenance e2e ${timestamp}"
body="@gitclaw /tools toolsets provenance

Live toolset provenance E2E. Please keep the provenance report body-free.
Do not include this hidden toolset provenance sentinel: ${hidden_token}"

local_report="$(go run ./cmd/gitclaw tools toolsets provenance)"
for expected in \
  ...
  '### Findings' \
  "- none"; do
  grep -Fq -- "$expected" <<<"$local_report" || die "local toolset provenance report missing ${expected}"
done
...
for leaked in "Read-only repository context tools" "Prefer bounded repository search" "$expected_token" "$search_phrase"; do
  if grep -Fq "$leaked" <<<"$local_report"; then
    die "local toolset provenance report leaked ${leaked}"
  fi
done
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "toolsets-provenance-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one toolset provenance report comment"
...
for expected in \
  ...
  "### Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$provenance_comment" || die "toolset provenance report missing ${expected}"
done

for leaked in "$hidden_token" "Live toolset provenance E2E" "Read-only repository context tools" "Prefer bounded repository search" "$expected_token" "$search_phrase"; do
  if grep -Fq "$leaked" <<<"$provenance_comment"; then
    die "toolset provenance report leaked ${leaked}"
  fi
done
