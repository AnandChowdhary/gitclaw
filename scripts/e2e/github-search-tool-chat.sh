...
hidden_token="GITCLAW_SEARCH_TOOL_CHAT_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
followup_hidden_token="NOECHO_SEARCH_TOOL_FOLLOWUP_${timestamp}"
followup_expected_token="GITCLAW_SEARCH_TOOL_FOLLOWUP_CONTEXT_V1"
followup_search_phrase="gclaw-searchtool-followup-needle"
title="@gitclaw search tool chat e2e ${timestamp}"
body="Live search-tool chat E2E.

Search the repository for \`${search_phrase}\`.
Reply with the exact GITCLAW_SEARCH token from the matching repository search result line.
Do not use @file or @folder references.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "search-tool-chat e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one search tool chat comment"
comments="$(assistant_comments)"

grep -Fq "$expected_token" <<<"$comments" || die "assistant did not include search_files token ${expected_token}"
...
if grep -Fq "@file" <<<"$comments" || grep -Fq "@folder" <<<"$comments"; then
  die "assistant echoed explicit context references instead of using search tool context"
fi

wait_for_done_status || die "expected gitclaw:done without running/error"
url="$(jq -r '.url' <<<"$run_json")"
log "search tool turn verified for issue #${issue_number}: ${url}"
...
Do not use @file or @folder references.
Do not answer with the first search token, issue title, issue number, or any token from this issue/comments.
...
if grep -Fq "@file" <<<"$model_comment" || grep -Fq "@folder" <<<"$model_comment"; then
  die "model follow-up echoed explicit context references instead of using search tool context"
fi
