...
token="NOECHO_PROMPT_REPORT_${timestamp}"
followup_hidden_token="NOECHO_PROMPT_REPORT_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_PROMPT_REPORT_CONTEXT_V1"
search_phrase="prompt report unique search fixture phrase"
noise=""
for _ in $(seq 1 1800); do
  noise+="prompt-report-noise "
done
title="@gitclaw /prompt e2e ${timestamp}"
body="Live prompt-report E2E.

Inspect \`go.mod\`, use the repo-reader skill, and search for \"bounded repository search fixture phrase\".
Hidden prompt report body token: ${token}
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "prompt-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one prompt report comment"
...
for expected in \
  ...
  'llm_e2e_required_after_prompt_report_change: `true`' \
  "### Prompt Inputs" \
  "### Context Files" \
  "### Selected Skills" \
  "### Tool Outputs" \
  ...
  'input=`go.mod`' \
  "sha256_12="; do
  grep -Fq "$expected" <<<"$comments" || die "prompt report missing ${expected}"
done
...
for leaked in \
  ...
  "prompt-report-noise"; do
  if grep -Fq "$leaked" <<<"$comments"; then
    die "prompt report leaked ${leaked}"
  fi
done

wait_for_done_status || die "expected gitclaw:done without running/error"
url="$(jq -r '.url' <<<"$run_json")"
log "prompt report verified for issue #${issue_number}: ${url}"
...
Do not answer with the prompt report nonce, prompt hash, issue title, issue number, prompt bytes, or any token from this issue/comments.
