#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-tool-map slash action queues provider-visible safe tool sequence cards and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-tool-map-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-tool-map-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-tool-map slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-tool-map-e2e-${timestamp}"
ingest_message_id="tool-map-ingest-${timestamp}"
notify_message_id="tool-map-notify-${timestamp}"
tool_map_id="tool-map-${timestamp}"
requested_tool="search_files"
tool_map_note="Keep tool execution reviewed"
account_id="telegram-tool-map-account-NOECHO_CHANNEL_TOOL_MAP_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_TOOL_MAP_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_TOOL_MAP_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_TOOL_MAP_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_TOOL_MAP_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_TOOL_MAP_CONTEXT_V1"
search_phrase="channel tool map unique search fixture phrase"
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-tool-map slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-tool-map slash E2E.
...
tool_map_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels tool-map ${requested_tool} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --map-id ${tool_map_id}
Note: ${tool_map_note}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$tool_map_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel tool map action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel tool map action receipt"
tool_map_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "llm_e2e_required_after_channel_tool_map_action_change: \`true\`"; do
  grep -Fq -- "$expected" <<<"$tool_map_receipt" || die "channel tool map receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$tool_map_id" "$requested_tool" "$tool_map_note" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$tool_map_receipt"; then
    die "channel tool map receipt leaked ${leaked}"
  fi
done

[[ "$(tool_map_notification_count)" == "1" ]] || die "channel tool map did not queue exactly one notification"
...
for expected in \
  ...
  "\`/channels tool-info ${requested_tool} --message-id <id> --notify-message-id <id>\`" \
  "\`/channels approval-plan ${requested_tool} --id <approval-plan-id> --message-id <id> --notify-message-id <id>\`" \
  "\`/channels rehearse-tool ${requested_tool} --id <rehearsal-id> --message-id <id> --notify-message-id <id>\`" \
  "\`/channels request-run ${requested_tool} --id <request-id> --message-id <id> --notify-message-id <id>\`" \
  "Note: ${tool_map_note}" \
  "Note hash: " \
  "Tool map hash: " \
  "Tool step hash: " \
  ...
  grep -Fq -- "$expected" <<<"$notification_bodies" || die "tool_map notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$tool_map_id" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$notification_bodies"; then
    die "tool_map notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing tool_map notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$tool_map_id" "$expected_token" "$requested_tool" "$tool_map_note"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels tool-path ${requested_tool} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --map-id ${tool_map_id}
Note: ${tool_map_note}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel tool map action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel tool map receipt"
...
for expected in \
  ...
  grep -Fq -- "$expected" <<<"$duplicate_receipt" || die "duplicate channel tool map receipt missing ${expected}"
done
[[ "$(tool_map_notification_count)" == "1" ]] || die "duplicate channel tool map queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$tool_map_id" "$requested_tool" "$tool_map_note" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel tool map receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel tool map thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_TOOL_MAP_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, tool map ids, tool map notes, requested tool names, tool steps, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel tool map model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel tool map follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq -- "$expected_token" <<<"$model_comment" || die "assistant did not include channel tool map search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel tool map follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel tool map follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel tool map follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel tool map follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel tool map follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel tool map follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$tool_map_id" "$tool_map_note" "$account_id"; do
  if grep -Fq -- "$leaked" <<<"$model_comment"; then
    die "model channel tool map follow-up leaked ${leaked}"
  fi
done
