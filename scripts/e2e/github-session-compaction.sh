...
hidden_token="GITCLAW_SESSION_COMPACTION_HIDDEN_${timestamp}"
comment_token="GITCLAW_SESSION_COMPACTION_COMMENT_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw session compaction e2e ${timestamp}"
body="Live session compaction E2E.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "session-compaction e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
Please audit the body-free session compaction readiness for this conversation.
...
wait_for_assistant_count 2 || die "expected session compaction report as second assistant comment"
...
for expected in \
  ...
  'agent_compaction_recommended: `false`' \
  ...
  'message=`03` role=`user`' \
  'in_current_prompt_pack=`true`' \
  'omitted_by_limit=`false`' \
  'truncated_by_limit=`false`' \
  'compaction_action=`keep-body-out-of-report`' \
  'body_included=`false`' \
  "### Compaction Gates" \
  'agent_compaction_gate=`pass`' \
  ...
  grep -Fq "$expected" <<<"$session_comment" || die "session compaction report missing ${expected}"
done

for leaked in "$hidden_token" "$comment_token" "$search_phrase" "Please audit the body-free session compaction readiness" "https://github.com/${repo}/actions/runs/"; do
  if grep -Fq "$leaked" <<<"$session_report_body"; then
    die "session compaction report leaked ${leaked}"
  fi
done
