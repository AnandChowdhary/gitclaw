...
hidden_token="GITCLAW_BACKUP_COVERAGE_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_BACKUP_COVERAGE_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /backup coverage e2e ${timestamp}"
body="@gitclaw /backup coverage

Live backup-coverage E2E.
The issue-visible report should record coverage intent, then the local fetched backup branch should prove this issue is indexed and readable.
...
cleanup() {
  ...
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "backup-coverage e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
coverage_run_json="$(wait_for_run issues "$issue_started_at")" || die "timed out waiting for issues workflow run"
wait_for_assistant_count 1 || die "expected one backup coverage report comment"
...
for expected in \
  ...
  "requested_local_command: \`gitclaw backup coverage --root .gitclaw/backups --repo ${repo} --issue ${issue_number}\`" \
  'raw_bodies_included: `false`'; do
  grep -Fq "$expected" <<<"$coverage_comment" || die "backup coverage issue report missing ${expected}"
done

for leaked in "$hidden_token" "The issue-visible report should record coverage intent"; do
  if grep -Fq "$leaked" <<<"$coverage_comment"; then
    die "backup coverage issue report leaked ${leaked}"
  fi
done
...
for _ in {1..60}; do
  ...
  if [[ -f "$index_path" ]] &&
    ...
    for expected in \
      ...
      'issue_backup_path_canonical: `true`' \
      ...
      'llm_e2e_required_after_backup_coverage_change: `true`' \
      'index_entry=`present`' \
      'mutation_performed=`false`'; do
      grep -Fq "$expected" <<<"$coverage_output" || die "backup coverage output missing ${expected}"
    done
    if grep -Fq "$hidden_token" <<<"$coverage_output" || grep -Fq "$title" <<<"$coverage_output"; then
      die "backup coverage output leaked raw issue data"
    fi
    break
  fi
  sleep 5
...
if [[ -z "${coverage_output:-}" ]]; then
  die "backup coverage did not observe issue #${issue_number} in ${backup_branch}"
fi
...
log "passed for issue #${issue_number}: ${coverage_url} (model follow-up: ${model_url})"
