#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel constellation queues provider-visible capability star maps and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-constellation-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-constellation-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-constellation slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-constellation-e2e-${timestamp}"
ingest_message_id="constellation-ingest-${timestamp}"
notify_message_id="constellation-notify-${timestamp}"
constellation_id="constellation-${timestamp}"
constellation_lane="research"
constellation_note="Keep the star map playful"
account_id="telegram-constellation-account-NOECHO_CHANNEL_CONSTELLATION_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_CONSTELLATION_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_CONSTELLATION_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_CONSTELLATION_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_CONSTELLATION_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_CONSTELLATION_CONTEXT_V1"
search_phrase="channel constellation unique search fixture phrase"
...
constellation_notification_count() {
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-constellation slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-constellation slash E2E.
...
constellation_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels constellation ${constellation_lane} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --constellation-id ${constellation_id}
Note: ${constellation_note}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$constellation_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel constellation action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel constellation action receipt"
constellation_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "llm_e2e_required_after_channel_constellation_action_change: \`true\`"; do
  grep -Fq "$expected" <<<"$constellation_receipt" || die "channel constellation receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$constellation_id" "$constellation_lane" "$constellation_note" "$expected_token" "OpenClaw" "Hermes" "Research catalog"; do
  if grep -Fqi "$leaked" <<<"$constellation_receipt"; then
    die "channel constellation receipt leaked ${leaked}"
  fi
done

[[ "$(constellation_notification_count)" == "1" ]] || die "channel constellation did not queue exactly one notification"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$notification_bodies" || die "constellation notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$constellation_id" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$notification_bodies"; then
    die "constellation notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing constellation notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$constellation_id" "$constellation_lane" "$constellation_note" "$expected_token" "OpenClaw" "Research catalog"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels star-map ${constellation_lane} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --constellation-id ${constellation_id}
Note: ${constellation_note}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel constellation action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel constellation receipt"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$duplicate_receipt" || die "duplicate channel constellation receipt missing ${expected}"
done
[[ "$(constellation_notification_count)" == "1" ]] || die "duplicate channel constellation queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$constellation_id" "$constellation_lane" "$constellation_note" "$expected_token" "OpenClaw" "Research catalog"; do
  if grep -Fqi "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel constellation receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel constellation thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_CONSTELLATION_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, constellation ids, lanes, notes, star text, command text, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel constellation model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel constellation follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel constellation search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel constellation follow-up marker did not use configured GitHub models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel constellation follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel constellation follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel constellation follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel constellation follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel constellation follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$constellation_id" "$constellation_note" "$account_id"; do
  if grep -Fq "$leaked" <<<"$model_comment"; then
    die "model channel constellation follow-up leaked ${leaked}"
  fi
done
