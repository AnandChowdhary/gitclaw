...
log() {
  echo "runs-history-report-e2e: $*" >&2
}
...
hidden_token="GITCLAW_RUNS_HISTORY_HIDDEN_${timestamp}"
history_hidden_token="GITCLAW_RUNS_HISTORY_COMMENT_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_RUNS_HISTORY_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw run history seed e2e ${timestamp}"
body="Live runs-history E2E.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "runs-history-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
history_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /runs history

Please show the body-free run history.
Hidden history request token: ${history_hidden_token}" >/dev/null

history_run_json="$(wait_for_run issue_comment "$history_started_at")" || die "timed out waiting for runs history issue_comment workflow run"
wait_for_assistant_count 2 || die "expected runs history report as second assistant comment"
history_comment="$(latest_assistant_comment)"
...
for expected in \
  ...
  'turns_missing_prompt_provenance: `0`' \
  'unique_run_ids: `1`' \
  'model_names: `openai/' \
  ...
  "raw assistant replies, prompts, tool outputs, issue bodies, and workflow payloads are excluded"; do
  grep -Fq -- "$expected" <<<"$history_comment" || die "runs history report missing ${expected}"
done

for leaked in "$hidden_token" "$history_hidden_token" "$expected_token" "$search_phrase" "Reply with the exact GITCLAW_SEARCH token" "Please show the body-free run history"; do
  if grep -Fq "$leaked" <<<"$history_comment"; then
    die "runs history report leaked ${leaked}"
  fi
done
...
history_url="$(jq -r '.url' <<<"$history_run_json")"
followup_url="$(jq -r '.url' <<<"$followup_run_json")"
log "passed for issue #${issue_number}: seed=${seed_url} history=${history_url} followup=${followup_url}"
