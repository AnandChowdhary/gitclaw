...
token="GITCLAW_SKILLS_LIST_E2E_${timestamp}"
title="@gitclaw /skills list e2e ${timestamp}"
body="Live skills-list E2E.

Please list the local GitClaw skill inventory and select the repo-reader skill.
Hidden skills list body token: ${token}
This should produce a deterministic skills list report without calling a model."
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "skills-list-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one skills list report comment"
...
for expected in \
  ...
  grep -Fq -- "$expected" <<<"$comments" || die "skills list report missing ${expected}"
done

for leaked in "$token" "GITCLAW_SKILL_CONTEXT_V1" "Hidden skills list body token"; do
  if grep -Fq "$leaked" <<<"$comments"; then
    die "skills list report leaked ${leaked}"
  fi
done
