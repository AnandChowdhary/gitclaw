#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-bundle-map slash action queues provider-visible safe skill-bundle sequence cards and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-bundle-map-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-bundle-map-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-bundle-map slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-bundle-map-e2e-${timestamp}"
ingest_message_id="bundle-map-ingest-${timestamp}"
notify_message_id="bundle-map-notify-${timestamp}"
bundle_map_id="bundle-map-${timestamp}"
requested_bundle="repo-context"
bundle_map_note="Keep bundle usage reviewed"
account_id="telegram-bundle-map-account-NOECHO_CHANNEL_BUNDLE_MAP_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_BUNDLE_MAP_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_BUNDLE_MAP_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_BUNDLE_MAP_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_BUNDLE_MAP_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_BUNDLE_MAP_CONTEXT_V1"
search_phrase="channel bundle map unique search fixture phrase"
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-bundle-map slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-bundle-map slash E2E.
...
bundle_map_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels bundle-map ${requested_bundle} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --map-id ${bundle_map_id}
Note: ${bundle_map_note}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$bundle_map_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel bundle map action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel bundle map action receipt"
bundle_map_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "missing_bundle_skills: \`" \
  "bundles_with_instruction: \`" \
  "bundles_with_parse_errors: \`0\`" \
  "bundles_with_risk_findings: \`0\`" \
  "selected_bundle_skill_refs: \`1\`" \
  "selected_bundle_resolved_skills: \`1\`" \
  "selected_bundle_missing_skills: \`0\`" \
  ...
  "llm_e2e_required_after_channel_bundle_map_action_change: \`true\`"; do
  grep -Fq -- "$expected" <<<"$bundle_map_receipt" || die "channel bundle map receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$bundle_map_id" "$requested_bundle" "$bundle_map_note" "GITCLAW_SKILL_CONTEXT_V1" "Use GitClaw's read-only repository context" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$bundle_map_receipt"; then
    die "channel bundle map receipt leaked ${leaked}"
  fi
done

[[ "$(bundle_map_notification_count)" == "1" ]] || die "channel bundle map did not queue exactly one notification"
...
for expected in \
  ...
  "Requested bundle: ${requested_bundle}" \
  "Available bundles: " \
  "Matched bundles: 1" \
  "Selected bundles for this turn: " \
  "Bundle skill refs: " \
  "Resolved bundle skills: " \
  "Missing bundle skills: " \
  "Bundles with instruction: " \
  "Bundle parse errors: 0" \
  "Bundle risk findings: 0" \
  "Matched bundle: ${requested_bundle}" \
  "Bundle skills: repo-reader" \
  "Resolved skills: repo-reader" \
  "Missing skills: none" \
  "Instruction present: true" \
  "Bundle sequence:" \
  "\`/channels profile --message-id <id> --notify-message-id <id>\`" \
  "\`/bundles info ${requested_bundle}\`" \
  "\`/bundles risk ${requested_bundle}\`" \
  "\`/channels skill-map repo-reader --message-id <id> --notify-message-id <id>\`" \
  "\`/bundles rehearse ${requested_bundle} --id <rehearsal-id>\`" \
  ...
  grep -Fq -- "$expected" <<<"$notification_bodies" || die "bundle_map notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$bundle_map_id" "GITCLAW_SKILL_CONTEXT_V1" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$notification_bodies"; then
    die "bundle_map notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing bundle_map notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$bundle_map_id" "$expected_token" "$requested_bundle" "$bundle_map_note" "GITCLAW_SKILL_CONTEXT_V1"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels bundle-path ${requested_bundle} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --map-id ${bundle_map_id}
Note: ${bundle_map_note}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel bundle map action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel bundle map receipt"
...
for expected in \
  ...
  grep -Fq -- "$expected" <<<"$duplicate_receipt" || die "duplicate channel bundle map receipt missing ${expected}"
done
[[ "$(bundle_map_notification_count)" == "1" ]] || die "duplicate channel bundle map queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$bundle_map_id" "$requested_bundle" "$bundle_map_note" "GITCLAW_SKILL_CONTEXT_V1" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel bundle map receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel bundle map thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_BUNDLE_MAP_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, bundle map ids, bundle map notes, requested bundle names, bundle map steps, bundle bodies, skill bodies, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel bundle map model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel bundle map follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq -- "$expected_token" <<<"$model_comment" || die "assistant did not include channel bundle map search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel bundle map follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel bundle map follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel bundle map follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel bundle map follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel bundle map follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel bundle map follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$bundle_map_id" "$bundle_map_note" "$requested_bundle" "$account_id" "GITCLAW_SKILL_CONTEXT_V1"; do
  if grep -Fq -- "$leaked" <<<"$model_comment"; then
    die "model channel bundle map follow-up leaked ${leaked}"
  fi
done
