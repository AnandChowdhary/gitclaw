...
log() {
  echo "memory-snapshot-report-e2e: $*" >&2
}
...
hidden_token="NOECHO_MEMORY_SNAPSHOT_${timestamp}"
followup_hidden_token="NOECHO_MEMORY_SNAPSHOT_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_MEMORY_SNAPSHOT_CONTEXT_V1"
search_phrase="memory snapshot unique search fixture phrase"
title="@gitclaw /memory snapshot e2e ${timestamp}"
body="@gitclaw /memory snapshot

Live memory-snapshot E2E.
Do not include this hidden issue token: ${hidden_token}"

local_report="$(go run ./cmd/gitclaw memory snapshot)"
for expected in \
  ...
  grep -Fq -- "$expected" <<<"$local_report" || die "local memory snapshot report missing ${expected}"
done
...
for leaked in "$expected_token" "$search_phrase" "Durable memory context token" "GITCLAW_MEMORY_CONTEXT_V1" "GitClaw remembers durable product decisions" "Keep persistent state"; do
  if grep -Fq "$leaked" <<<"$local_report"; then
    die "local memory snapshot report leaked ${leaked}"
  fi
done
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "memory-snapshot-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one memory snapshot report comment"
...
for expected in \
  ...
  'raw_body_gate=`hash_only`' \
  'snapshot_hash_gate=`composite-sha256_12`'; do
  grep -Fq -- "$expected" <<<"$snapshot_comment" || die "memory snapshot report missing ${expected}"
done

for leaked in "$hidden_token" "$expected_token" "$search_phrase" "Live memory-snapshot E2E" "Durable memory context token" "GITCLAW_MEMORY_CONTEXT_V1" "GitClaw remembers durable product decisions" "Keep persistent state"; do
  if grep -Fq "$leaked" <<<"$snapshot_comment"; then
    die "memory snapshot report leaked ${leaked}"
  fi
done
