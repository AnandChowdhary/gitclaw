#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel backup-spotlight draws from the real backup branch and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-backup-spotlight-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-backup-spotlight-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel backup-spotlight slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-backup-spotlight-e2e-${timestamp}"
ingest_message_id="backup-spotlight-ingest-${timestamp}"
notify_message_id="backup-spotlight-notify-${timestamp}"
backup_spotlight_id="channel-backup-spotlight-${timestamp}"
backup_anchor="channel-backup-spotlight-anchor-${timestamp}"
account_id="telegram-backup-spotlight-account-NOECHO_CHANNEL_BACKUP_SPOTLIGHT_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_BACKUP_SPOTLIGHT_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_BACKUP_SPOTLIGHT_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_BACKUP_SPOTLIGHT_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_BACKUP_SPOTLIGHT_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_BACKUP_SPOTLIGHT_CONTEXT_V1"
search_phrase="channel backup spotlight unique search fixture phrase"
...
backup_spotlight_notification_count() {
...
cleanup() {
  ...
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel backup-spotlight slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel backup-spotlight slash E2E.
Archive spotlight seed: ${backup_anchor}
...
if [[ -z "$backup_search_output" ]]; then
  die "channel backup-spotlight did not observe source issue #${issue_number} in ${backup_branch}"
fi
for expected in \
...
spotlight_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels backup-spotlight ${backup_anchor} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --spotlight-id ${backup_spotlight_id}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$spotlight_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel backup-spotlight action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel backup-spotlight action receipt"
spotlight_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "llm_e2e_required_after_channel_backup_spotlight_change: \`true\`"; do
  grep -Fq "$expected" <<<"$spotlight_receipt" || die "channel backup-spotlight receipt missing ${expected}"
done
grep -Eq 'candidate_backups: `([1-9][0-9]*)`' <<<"$spotlight_receipt" || die "channel backup-spotlight receipt did not report candidates"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$backup_anchor" "$thread_id" "$ingest_message_id" "$notify_message_id" "$backup_spotlight_id" "$expected_token" "$repo_key" "$issue_path"; do
  if grep -Fq "$leaked" <<<"$spotlight_receipt"; then
    die "channel backup-spotlight receipt leaked ${leaked}"
  fi
done

[[ "$(backup_spotlight_notification_count)" == "1" ]] || die "channel backup-spotlight did not queue exactly one notification"
...
for expected in \
  ...
  "title_sha256_12=" \
  "Try next:" \
  "@gitclaw /channels backup-info #${issue_number}" \
  ...
  grep -Fq "$expected" <<<"$notification_bodies" || die "backup-spotlight notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$backup_anchor" "$backup_spotlight_id" "$expected_token" "$issue_title"; do
  if grep -Fq "$leaked" <<<"$notification_bodies"; then
    die "backup-spotlight notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing backup-spotlight notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$backup_anchor" "$backup_spotlight_id" "$expected_token" "$issue_path"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels backup-draw ${backup_anchor} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --spotlight-id ${backup_spotlight_id}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel backup-spotlight action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel backup-spotlight receipt"
...
for expected in \
  ...
  "repository_mutation_performed: \`false\`" \
  "backup_branch_write_performed: \`false\`" \
  "restore_mutation_performed: \`false\`"; do
  grep -Fq "$expected" <<<"$duplicate_receipt" || die "duplicate channel backup-spotlight receipt missing ${expected}"
done
[[ "$(backup_spotlight_notification_count)" == "1" ]] || die "duplicate channel backup-spotlight queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$backup_spotlight_id" "$backup_anchor" "$expected_token" "$repo_key" "$issue_path"; do
  if grep -Fq "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel backup-spotlight receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel backup-spotlight thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_BACKUP_SPOTLIGHT_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, spotlight ids, issue numbers, backup paths, backup branches, archive spotlight anchors, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel backup-spotlight model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel backup-spotlight follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel backup-spotlight search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel backup-spotlight follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel backup-spotlight follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel backup-spotlight follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel backup-spotlight follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel backup-spotlight follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel backup-spotlight follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$backup_spotlight_id" "$account_id" "$backup_anchor"; do
  if grep -Fq "$leaked" <<<"$model_comment"; then
    die "model channel backup-spotlight follow-up leaked ${leaked}"
  fi
done
