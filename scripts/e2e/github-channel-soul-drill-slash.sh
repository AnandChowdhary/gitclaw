#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-soul-drill queues a body-free high-authority context metadata card and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-soul-drill-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-soul-drill-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-soul-drill slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-soul-drill-e2e-${timestamp}"
ingest_message_id="soul-drill-ingest-${timestamp}"
notify_message_id="soul-drill-notify-${timestamp}"
soul_drill_id="soul-drill-${timestamp}"
soul_focus="SOUL"
account_id="telegram-soul-drill-account-NOECHO_CHANNEL_SOUL_DRILL_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_SOUL_DRILL_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_SOUL_DRILL_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_SOUL_DRILL_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_SOUL_DRILL_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_SOUL_DRILL_CONTEXT_V1"
search_phrase="channel soul drill unique search fixture phrase"
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-soul-drill slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-soul-drill slash E2E.
...
soul_drill_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels soul-drill ${soul_focus} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --drill-id ${soul_drill_id}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$soul_drill_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel soul-drill action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel soul-drill action receipt"
soul_drill_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "llm_e2e_required_after_channel_soul_drill_change: \`true\`"; do
  grep -Fq "$expected" <<<"$soul_drill_receipt" || die "channel soul-drill receipt missing ${expected}"
done
grep -Eq 'available_soul_files: `([1-9][0-9]*)`' <<<"$soul_drill_receipt" || die "channel soul-drill receipt did not report available soul files"
grep -Eq 'present_soul_files: `([1-9][0-9]*)`' <<<"$soul_drill_receipt" || die "channel soul-drill receipt did not report present soul files"
grep -Eq 'eligible_soul_files: `([1-9][0-9]*)`' <<<"$soul_drill_receipt" || die "channel soul-drill receipt did not report eligible soul files"
grep -Eq 'memory_notes: `([0-9]+)`' <<<"$soul_drill_receipt" || die "channel soul-drill receipt did not report memory notes"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$soul_drill_id" "$soul_focus" ".gitclaw/SOUL.md" ".gitclaw/IDENTITY.md" ".gitclaw/USER.md" ".gitclaw/TOOLS.md" ".gitclaw/MEMORY.md" ".gitclaw/memory/" ".gitclaw/HEARTBEAT.md" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$soul_drill_receipt"; then
    die "channel soul-drill receipt leaked ${leaked}"
  fi
done

[[ "$(soul_drill_notification_count)" == "1" ]] || die "channel soul-drill did not queue exactly one soul-drill notification"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$notification_bodies" || die "soul-drill notification missing ${expected}"
done
grep -Eq 'Available soul files: ([1-9][0-9]*)' <<<"$notification_bodies" || die "soul-drill notification did not report available soul files"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$soul_drill_id" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$notification_bodies"; then
    die "soul-drill notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing soul-drill notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$soul_drill_id" "$soul_focus" "$expected_token" ".gitclaw/SOUL.md" ".gitclaw/IDENTITY.md" ".gitclaw/USER.md" ".gitclaw/TOOLS.md" ".gitclaw/MEMORY.md" ".gitclaw/memory/" ".gitclaw/HEARTBEAT.md"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels soul-practice ${soul_focus} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --practice-id ${soul_drill_id}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel soul-drill action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel soul-drill receipt"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$duplicate_receipt" || die "duplicate channel soul-drill receipt missing ${expected}"
done
[[ "$(soul_drill_notification_count)" == "1" ]] || die "duplicate channel soul-drill queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$soul_drill_id" "$soul_focus" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel soul-drill receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel soul-drill thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_SOUL_DRILL_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, drill ids, issue numbers, soul drill paths, focus text, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel soul-drill model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel soul-drill follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel soul-drill search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel soul-drill follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel soul-drill follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel soul-drill follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel soul-drill follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel soul-drill follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel soul-drill follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$soul_drill_id" "$account_id"; do
  if grep -Fq "$leaked" <<<"$model_comment"; then
    die "model channel soul-drill follow-up leaked ${leaked}"
  fi
done
