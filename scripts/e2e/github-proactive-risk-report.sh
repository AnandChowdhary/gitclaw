...
log() {
  echo "proactive-risk-report-e2e: $*" >&2
}
...
hidden_token="GITCLAW_PROACTIVE_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_PROACTIVE_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /proactive risk e2e ${timestamp}"
body="@gitclaw /proactive risk

Live proactive-risk E2E.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "proactive-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one proactive risk report comment"
risk_comment="$(latest_assistant_comment)"

for expected in \
  'model="gitclaw/proactive"' \
  "GitClaw Proactive Risk Report" \
  "Generated without a model call" \
  'proactive_risk_status: `ok`' \
  'verification_scope: `scheduled_issue_workflow`' \
  'prompt_files: `1`' \
  ...
  'llm_e2e_required_after_proactive_risk_change: `true`' \
  "### Workflow Risk Cards" \
  'path=`.github/workflows/gitclaw-proactive.yml`' \
  ...
  'risk_codes=`none`' \
  "### Prompt Risk Cards" \
  'path=`.gitclaw/proactive/repo-hygiene.md`' \
  'skill_hints=`repo-reader`' \
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "proactive risk report missing ${expected}"
done

for leaked in "$hidden_token" "Run a short, read-only repository hygiene check" "go run ./cmd/gitclaw proactive enqueue" "GITCLAW_PROACTIVE_PROMPT_FILE" "bounded repository search fixture phrase"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "proactive risk report leaked ${leaked}"
  fi
done
