#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-room-pulse slash action queues provider-visible thread pulse cards and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-room-pulse-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-room-pulse-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-room-pulse slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-room-pulse-e2e-${timestamp}"
ingest_message_id="room-pulse-ingest-${timestamp}"
notify_message_id="room-pulse-notify-${timestamp}"
pulse_id="room-pulse-${timestamp}"
pulse_focus="handoff"
pulse_note="Keep the room warm."
account_id="telegram-room-pulse-account-NOECHO_CHANNEL_ROOM_PULSE_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_ROOM_PULSE_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_ROOM_PULSE_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_ROOM_PULSE_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_ROOM_PULSE_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_ROOM_PULSE_CONTEXT_V1"
search_phrase="channel room pulse unique search fixture phrase"
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-room-pulse slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-room-pulse slash E2E.
...
room_pulse_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels room-pulse ${pulse_focus} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --pulse-id ${pulse_id}
Note: ${pulse_note}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$room_pulse_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel room pulse action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel room pulse action receipt"
room_pulse_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "llm_e2e_required_after_channel_room_pulse_action_change: \`true\`"; do
  grep -Fq -- "$expected" <<<"$room_pulse_receipt" || die "channel room pulse receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$pulse_id" "$pulse_focus" "$pulse_note" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$room_pulse_receipt"; then
    die "channel room pulse receipt leaked ${leaked}"
  fi
done

[[ "$(room_pulse_notification_count)" == "1" ]] || die "channel room pulse did not queue exactly one notification"
...
for expected in \
  ...
  grep -Fq -- "$expected" <<<"$notification_bodies" || die "room pulse notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$pulse_id" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$notification_bodies"; then
    die "room pulse notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing room pulse notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$pulse_id" "$expected_token" "$pulse_focus" "$pulse_note"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels thread-pulse ${pulse_focus} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --pulse-id ${pulse_id}
Note: ${pulse_note}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel room pulse action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel room pulse receipt"
...
for expected in \
  ...
  "task_issue_created: \`false\`" \
  "reminder_created: \`false\`" \
  "scheduled_workflow_created: \`false\`"; do
  grep -Fq -- "$expected" <<<"$duplicate_receipt" || die "duplicate channel room pulse receipt missing ${expected}"
done
[[ "$(room_pulse_notification_count)" == "1" ]] || die "duplicate channel room pulse queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$pulse_id" "$pulse_focus" "$pulse_note" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel room pulse receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel room pulse thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_ROOM_PULSE_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, room pulse ids, focus values, notes, suggested steps, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel room pulse model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel room pulse follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq -- "$expected_token" <<<"$model_comment" || die "assistant did not include channel room pulse search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel room pulse follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel room pulse follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel room pulse follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel room pulse follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel room pulse follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel room pulse follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$pulse_id" "$pulse_focus" "$pulse_note" "$account_id"; do
  if grep -Fq -- "$leaked" <<<"$model_comment"; then
    die "model channel room pulse follow-up leaked ${leaked}"
  fi
done
