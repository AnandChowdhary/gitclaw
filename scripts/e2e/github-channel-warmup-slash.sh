...
log() {
  echo "channel-warmup-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-warmup-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-warmup slash E2E appears to be running: ${lock_dir}"
fi
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-warmup slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-warmup slash E2E.
...
wait_for_issue_comment_run_for_title "$warmup_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel warmup action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel warmup action receipt"
warmup_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
...
[[ "$(warmup_notification_count)" == "1" ]] || die "channel warmup did not queue exactly one notification"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$notification_bodies" || die "warmup notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$warmup_id" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$notification_bodies"; then
    die "warmup notification leaked ${leaked}"
  fi
done
...
wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel warmup action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel warmup receipt"
...
model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel warmup model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel warmup follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel warmup search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel warmup follow-up marker did not use configured GitHub models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel warmup follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel warmup follow-up marker missing selected repo-reader skill"
