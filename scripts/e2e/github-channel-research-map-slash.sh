#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel research-map queues catalog command paths and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-research-map-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-research-map-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel research-map slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-research-map-e2e-${timestamp}"
ingest_message_id="research-map-ingest-${timestamp}"
notify_message_id="research-map-notify-${timestamp}"
research_map_id="research-map-${timestamp}"
research_focus="openclaw"
account_id="telegram-research-map-account-NOECHO_CHANNEL_RESEARCH_MAP_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_RESEARCH_MAP_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_RESEARCH_MAP_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_RESEARCH_MAP_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_RESEARCH_MAP_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_RESEARCH_MAP_CONTEXT_V1"
search_phrase="channel research map unique search fixture phrase"
...
research_map_notification_count() {
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel research-map slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel research-map slash E2E.
...
map_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels research-map ${research_focus} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --map-id ${research_map_id}
Note: Keep upstream mapping reviewed
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$map_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel research-map action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel research-map action receipt"
map_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "llm_e2e_required_after_channel_research_map_action_change: \`true\`"; do
  grep -Fq -- "$expected" <<<"$map_receipt" || die "channel research-map receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$research_map_id" "$research_focus" "docs.openclaw.ai" "$expected_token" "Keep upstream mapping reviewed"; do
  if grep -Fqi -- "$leaked" <<<"$map_receipt"; then
    die "channel research-map receipt leaked ${leaked}"
  fi
done

[[ "$(research_map_notification_count)" == "1" ]] || die "channel research-map did not queue exactly one notification"
...
for expected in \
  ...
  "\`@gitclaw /channels research-spotlight ${research_focus} --message-id <id> --notify-message-id <id>\`" \
  "\`@gitclaw /channels compass ${research_focus} --compass-id <id> --message-id <id> --notify-message-id <id>\`" \
  ...
  grep -Fq -- "$expected" <<<"$notification_bodies" || die "research-map notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$research_map_id" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$notification_bodies"; then
    die "research-map notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing research-map notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$research_map_id" "$research_focus" "docs.openclaw.ai" "$expected_token" "Keep upstream mapping reviewed"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels research-path ${research_focus} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --map-id ${research_map_id}
Note: Keep upstream mapping reviewed
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel research-map action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel research-map receipt"
...
for expected in \
  ...
  grep -Fq -- "$expected" <<<"$duplicate_receipt" || die "duplicate channel research-map receipt missing ${expected}"
done
[[ "$(research_map_notification_count)" == "1" ]] || die "duplicate channel research-map queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$research_map_id" "$research_focus" "$expected_token" "Keep upstream mapping reviewed"; do
  if grep -Fqi -- "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel research-map receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel research-map thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_RESEARCH_MAP_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, map ids, issue numbers, focus text, source URLs, step text, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel research-map model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel research-map follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq -- "$expected_token" <<<"$model_comment" || die "assistant did not include channel research-map search_files token ${expected_token}"
if ! grep -Fq -- 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq -- 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel research-map follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq -- 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel research-map follow-up marker missing prompt context hash"
grep -Fq -- 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel research-map follow-up marker missing selected repo-reader skill"
grep -Fq -- 'tools="' <<<"$model_comment" || die "assistant channel research-map follow-up marker missing prompt-visible tools"
grep -Fq -- 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel research-map follow-up marker did not prove search_files was prompt-visible"
grep -Fq -- 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel research-map follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$research_map_id" "$account_id" "$research_focus"; do
  if grep -Fq -- "$leaked" <<<"$model_comment"; then
    die "model channel research-map follow-up leaked ${leaked}"
  fi
done
