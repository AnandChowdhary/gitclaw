...
log() {
  echo "skills-sources-info-e2e: $*" >&2
}
...
hidden_token="NOECHO_SKILL_SOURCE_INFO_${timestamp}"
followup_hidden_token="NOECHO_SKILL_SOURCE_INFO_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_SKILL_SOURCE_INFO_CONTEXT_V1"
expected_hash="2f9e68a57bd6"
search_phrase="skill source info unique search fixture phrase"
title="@gitclaw /skills sources info repo-reader e2e ${timestamp}"
body="@gitclaw /skills sources info repo-reader

Live skill source info E2E. Keep the focused source-pin report body-free.
Do not include this hidden skill source info token: ${hidden_token}"

local_report="$(go run ./cmd/gitclaw skills sources info repo-reader)"
for expected in \
  ...
  grep -Fq -- "$expected" <<<"$local_report" || die "local skill source info report missing ${expected}"
done
...
for leaked in "Skill context verification token" "GITCLAW_SKILL_CONTEXT_V1" "$expected_token" "$search_phrase"; do
  if grep -Fq "$leaked" <<<"$local_report"; then
    die "local skill source info report leaked ${leaked}"
  fi
done
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "skills-sources-info e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one skill source info report comment"
...
for expected in \
  ...
  'raw_body_gate=`hash_only`' \
  'model_e2e_gate=`required`'; do
  grep -Fq -- "$expected" <<<"$info_comment" || die "skill source info report missing ${expected}"
done

for leaked in "$hidden_token" "Live skill source info E2E" "Skill context verification token" "GITCLAW_SKILL_CONTEXT_V1" "$expected_token" "$search_phrase"; do
  if grep -Fq "$leaked" <<<"$info_comment"; then
    die "skill source info report leaked ${leaked}"
  fi
done
