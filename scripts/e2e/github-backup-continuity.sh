...
token="NOECHO_BACKUP_CONTINUITY_${timestamp}"
followup_hidden_token="NOECHO_BACKUP_CONTINUITY_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_BACKUP_CONTINUITY_CONTEXT_V1"
search_phrase="backup continuity unique search fixture phrase"
title="@gitclaw /backup continuity e2e ${timestamp}"
body="Live backup continuity E2E.

Hidden backup continuity token: ${token}
This should produce a deterministic backup report, then the fetched backup branch should render a body-free chronological gap gate for the verified backup history."
...
cleanup() {
  ...
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "backup-continuity e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
for expected in \
  ...
  "requested_local_command: \`gitclaw backup continuity --root .gitclaw/backups --repo ${repo} --max-gap-hours 168\`" \
...
if grep -Fq "$expected_token" <<<"$comments" || grep -Fq "$search_phrase" <<<"$comments"; then
  die "backup continuity report leaked follow-up fixture context"
fi

for leaked in "$token" "$title"; do
  if grep -Fq "$leaked" <<<"$comments"; then
    die "backup report leaked issue-side continuity input ${leaked}"
  fi
done
...
for _ in {1..60}; do
  ...
  if [[ -f "$index_path" ]] &&
    ...
    continuity_output="$(go run ./cmd/gitclaw backup continuity --root "${backup_checkout}/.gitclaw/backups" --repo "$repo" --max-gap-hours 168)"
    for expected in \
      ...
      "points_scanned: \`${issue_count}\`" \
      'max_gap_seconds: `604800`' \
      'gaps_over_max: `0`' \
      ...
      grep -Fq "$expected" <<<"$continuity_output" || die "backup continuity missing ${expected}"
    done
    if grep -Fq "$token" <<<"$continuity_output"; then
      die "backup continuity leaked issue body token"
    fi
    if grep -Fq "$title" <<<"$continuity_output"; then
      die "backup continuity leaked issue title"
    fi
    url="$(jq -r '.url' <<<"$run_json")"
    break
  fi
...
[[ -n "${url:-}" ]] || die "backup continuity did not observe issue #${issue_number} in ${backup_branch}"
