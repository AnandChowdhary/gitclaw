...
hidden_token="GITCLAW_AGENTS_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_AGENTS_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /agents risk e2e ${timestamp}"
body="@gitclaw /agents risk

Live agents-risk E2E.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "agents-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
for expected in \
  ...
  'llm_e2e_required_after_agent_risk_change: `true`' \
  "### Agent Policy Risk Card" \
  'kind=`agent-policy` path=`.gitclaw/AGENTS.md`' \
  'risk_findings=`0`' \
  'risk_codes=`none`' \
  "### Agent Spec Risk Cards" \
  'kind=`agent-spec` name=`repo-assistant` path=`.gitclaw/agents/repo-assistant.md`' \
  'role=`primary`' \
  ...
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "agents risk report missing ${expected}"
done

for leaked in "$hidden_token" "GITCLAW_AGENTS_CONTEXT_V1" "This declarative agent record" "Agent policy verification token" "bounded repository search fixture phrase"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "agents risk report leaked ${leaked}"
  fi
done
