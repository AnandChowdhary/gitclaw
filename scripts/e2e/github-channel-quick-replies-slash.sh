#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-quick-replies slash action queues provider-visible reply chips and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-quick-replies-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-quick-replies-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-quick-replies slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-quick-replies-e2e-${timestamp}"
ingest_message_id="quick-replies-ingest-${timestamp}"
notify_message_id="quick-replies-notify-${timestamp}"
reply_id="quick-replies-${timestamp}"
reply_lane="handoff"
reply_note="Keep replies short."
account_id="telegram-quick-replies-account-NOECHO_CHANNEL_QUICK_REPLIES_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_QUICK_REPLIES_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_QUICK_REPLIES_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_QUICK_REPLIES_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_QUICK_REPLIES_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_QUICK_REPLIES_CONTEXT_V1"
search_phrase="channel quick replies unique search fixture phrase"
...
quick_replies_notification_count() {
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-quick-replies slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-quick-replies slash E2E.
...
quick_replies_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels quick-replies ${reply_lane} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --reply-id ${reply_id}
Note: ${reply_note}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$quick_replies_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel quick replies action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel quick replies action receipt"
quick_replies_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "llm_e2e_required_after_channel_quick_replies_action_change: \`true\`"; do
  grep -Fq -- "$expected" <<<"$quick_replies_receipt" || die "channel quick replies receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$reply_id" "$reply_lane" "$reply_note" "$expected_token" "/channels room-pulse handoff"; do
  if grep -Fq -- "$leaked" <<<"$quick_replies_receipt"; then
    die "channel quick replies receipt leaked ${leaked}"
  fi
done

[[ "$(quick_replies_notification_count)" == "1" ]] || die "channel quick replies did not queue exactly one notification"
...
for expected in \
  ...
  grep -Fq -- "$expected" <<<"$notification_bodies" || die "quick replies notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$reply_id" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$notification_bodies"; then
    die "quick replies notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing quick replies notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$reply_id" "$expected_token" "$reply_lane" "$reply_note" "/channels room-pulse handoff"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels reply-chips ${reply_lane} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --reply-id ${reply_id}
Note: ${reply_note}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel quick replies action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel quick replies receipt"
...
for expected in \
  ...
  grep -Fq -- "$expected" <<<"$duplicate_receipt" || die "duplicate channel quick replies receipt missing ${expected}"
done
[[ "$(quick_replies_notification_count)" == "1" ]] || die "duplicate channel quick replies queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$reply_id" "$reply_lane" "$reply_note" "$expected_token" "/channels room-pulse handoff"; do
  if grep -Fq -- "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel quick replies receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel quick replies thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_QUICK_REPLIES_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, quick reply ids, lane values, notes, option commands, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel quick replies model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel quick replies follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq -- "$expected_token" <<<"$model_comment" || die "assistant did not include channel quick replies search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel quick replies follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel quick replies follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel quick replies follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel quick replies follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel quick replies follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel quick replies follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$reply_id" "$reply_lane" "$reply_note" "$account_id"; do
  if grep -Fq -- "$leaked" <<<"$model_comment"; then
    die "model channel quick replies follow-up leaked ${leaked}"
  fi
done
