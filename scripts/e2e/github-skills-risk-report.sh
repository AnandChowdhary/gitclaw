...
hidden_token="GITCLAW_SKILLS_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_SKILLS_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /skills risk e2e ${timestamp}"
body="@gitclaw /skills risk

Live skills-risk E2E.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "skills-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one skills risk report comment"
...
for expected in \
  ...
  'available_skills: `1`' \
  'scanned_skills: `1`' \
  'skills_with_risk_findings: `0`' \
  'skill_risk_findings: `0`' \
  ...
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "skills risk report missing ${expected}"
done

for leaked in "$hidden_token" "GITCLAW_SKILL_CONTEXT_V1"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "skills risk report leaked ${leaked}"
  fi
done
