...
token="NOECHO_BACKUP_FRESHNESS_${timestamp}"
followup_hidden_token="NOECHO_BACKUP_FRESHNESS_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_BACKUP_FRESHNESS_CONTEXT_V1"
search_phrase="backup freshness unique search fixture phrase"
title="@gitclaw /backup freshness e2e ${timestamp}"
body="Live backup freshness E2E.

Hidden backup freshness token: ${token}
This should produce a deterministic backup report, then the fetched backup branch should render a body-free freshness gate for the latest verified backup."
...
cleanup() {
  ...
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "backup-freshness e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
for expected in \
  ...
  "requested_local_command: \`gitclaw backup freshness --root .gitclaw/backups --repo ${repo} --max-age-hours 24\`" \
...
if grep -Fq "$expected_token" <<<"$comments" || grep -Fq "$search_phrase" <<<"$comments"; then
  die "backup freshness report leaked follow-up fixture context"
fi

for leaked in "$token" "$title"; do
  if grep -Fq "$leaked" <<<"$comments"; then
    die "backup report leaked issue-side freshness input ${leaked}"
  fi
done
...
for _ in {1..60}; do
  ...
  if [[ -f "$index_path" ]] &&
    ...
    freshness_output="$(go run ./cmd/gitclaw backup freshness --root "${backup_checkout}/.gitclaw/backups" --repo "$repo" --max-age-hours 24)"
    for expected in \
      ...
      grep -Fq "$expected" <<<"$freshness_output" || die "backup freshness missing ${expected}"
    done
    if grep -Fq "$token" <<<"$freshness_output"; then
      die "backup freshness leaked issue body token"
    fi
    if grep -Fq "$title" <<<"$freshness_output"; then
      die "backup freshness leaked issue title"
    fi
    url="$(jq -r '.url' <<<"$run_json")"
    break
  fi
...
[[ -n "${url:-}" ]] || die "backup freshness did not observe issue #${issue_number} in ${backup_branch}"
