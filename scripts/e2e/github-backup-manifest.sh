...
token="NOECHO_BACKUP_MANIFEST_${timestamp}"
followup_hidden_token="NOECHO_BACKUP_MANIFEST_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_BACKUP_MANIFEST_CONTEXT_V1"
search_phrase="backup manifest unique search fixture phrase"
title="@gitclaw /backup manifest e2e ${timestamp}"
body="Live backup manifest E2E.

Hidden backup manifest token: ${token}
This should produce a deterministic backup report, then the fetched backup branch should render a file-level manifest without leaking this token."
...
cleanup() {
  ...
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "backup manifest e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
for expected in \
  ...
  "requested_local_command: \`gitclaw backup manifest --root .gitclaw/backups --repo ${repo} --issue ${issue_number}\`" \
...
if grep -Fq "$expected_token" <<<"$comments" || grep -Fq "$search_phrase" <<<"$comments"; then
  die "backup manifest report leaked follow-up fixture context"
fi

for leaked in "$token" "$title"; do
  if grep -Fq "$leaked" <<<"$comments"; then
    die "backup report leaked issue-side manifest input ${leaked}"
  fi
done
...
for _ in {1..60}; do
  ...
  if [[ -f "$index_path" ]] &&
    ...
    for expected in \
      ...
      'sha256_12='; do
      grep -Fq "$expected" <<<"$manifest_output" || die "manifest missing ${expected}"
    done
    if grep -Fq "$token" <<<"$manifest_output"; then
      die "manifest leaked issue body token"
    fi
    url="$(jq -r '.url' <<<"$run_json")"
    break
  fi
...
[[ -n "${url:-}" ]] || die "backup manifest did not observe issue #${issue_number} in ${backup_branch}"
