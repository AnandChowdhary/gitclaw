...
token="NOECHO_PROMPT_LIST_REPORT_${timestamp}"
followup_hidden_token="NOECHO_PROMPT_LIST_REPORT_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_PROMPT_LIST_REPORT_CONTEXT_V1"
search_phrase="prompt list report unique search fixture phrase"
noise=""
...
url="$(jq -r '.url' <<<"$run_json")"
log "prompt list report verified for issue #${issue_number}: ${url}"
...
Do not answer with the prompt-list alias nonce, prompt hash, issue title, issue number, prompt bytes, or any token from this issue/comments.
