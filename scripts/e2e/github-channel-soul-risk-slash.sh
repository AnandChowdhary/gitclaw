#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-soul-risk queues high-authority persistent-state risk cards and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-soul-risk-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-soul-risk-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-soul-risk slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-soul-risk-e2e-${timestamp}"
ingest_message_id="soul-risk-ingest-${timestamp}"
notify_message_id="soul-risk-notify-${timestamp}"
soul_risk_id="soul-risk-${timestamp}"
account_id="telegram-soul-risk-account-NOECHO_CHANNEL_SOUL_RISK_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_SOUL_RISK_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_SOUL_RISK_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_SOUL_RISK_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_SOUL_RISK_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_SOUL_RISK_CONTEXT_V1"
search_phrase="channel soul risk unique search fixture phrase"
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-soul-risk slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-soul-risk slash E2E.
...
soul_risk_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels soul-risk --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --risk-id ${soul_risk_id}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$soul_risk_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel soul-risk action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel soul-risk action receipt"
soul_risk_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
...
[[ "$(soul_risk_notification_count)" == "1" ]] || die "channel soul-risk did not queue exactly one soul-risk notification"
...
for expected in \
  ...
  "Context documents: " \
  "Scanned documents: " \
  "Documents with risk findings: 0" \
  "Risk findings: 0" \
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels authority-risk --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --risk-id ${soul_risk_id}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel soul-risk action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel soul-risk receipt"
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel soul-risk thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_SOUL_RISK_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, risk ids, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel soul-risk model follow-up"
