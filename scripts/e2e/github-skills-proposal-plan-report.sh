...
log() {
  echo "skills-proposal-plan-report-e2e: $*" >&2
}
...
hidden_token="GITCLAW_SKILLS_PROPOSAL_PLAN_E2E_${timestamp}"
followup_hidden_token="GITCLAW_SKILLS_PROPOSAL_FOLLOWUP_E2E_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /skills proposal-plan repo-reader e2e ${timestamp}"
body="@gitclaw /skills proposal-plan repo-reader

Draft a reusable repo-reader improvement proposal.
Do not include this hidden proposal token: ${hidden_token}"
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "skills-proposal-plan-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one skills proposal-plan report comment"
...
for expected in \
  ...
  'target_sha256_12:'; do
  grep -Fq -- "$expected" <<<"$proposal_comment" || die "skills proposal-plan report missing ${expected}"
done

for leaked in "$hidden_token" "Draft a reusable repo-reader improvement proposal" "GITCLAW_SKILL_CONTEXT_V1" "$expected_token" "$search_phrase"; do
  if grep -Fq "$leaked" <<<"$proposal_comment"; then
    die "skills proposal-plan report leaked ${leaked}"
  fi
done
