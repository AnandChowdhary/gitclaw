...
hidden_token="GITCLAW_PROMPT_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_PROMPT_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /prompt risk e2e ${timestamp}"
body="@gitclaw /prompt risk @file:docs/search-fixture.md:1-1

Live prompt-risk E2E.
Use the repo-reader skill and search for ${search_phrase}, but the deterministic prompt risk report must stay body-free.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "prompt-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one prompt risk report comment"
...
for expected in \
  ...
  'kind=`prompt-budget` prompt_bytes=' \
  "### Transcript Risk Card" \
  'kind=`transcript` transcript_messages=`1`' \
  ...
  'kind=`runtime-boundary` prompt_artifact_enabled=`false`' \
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "prompt risk report missing ${expected}"
done
...
for leaked in \
  ...
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "prompt risk report leaked ${leaked}"
  fi
done
