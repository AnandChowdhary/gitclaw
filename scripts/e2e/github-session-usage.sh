...
hidden_token="GITCLAW_SESSION_USAGE_HIDDEN_${timestamp}"
comment_token="GITCLAW_SESSION_USAGE_COMMENT_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw session usage e2e ${timestamp}"
body="Live session usage E2E.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "session-usage e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
grep -Fq 'usage_prompt_tokens="' <<<"$first_comment" || die "assistant marker missing prompt token telemetry"
grep -Fq 'usage_completion_tokens="' <<<"$first_comment" || die "assistant marker missing completion token telemetry"
...
Please audit assistant-turn token telemetry.
...
wait_for_assistant_count 2 || die "expected session usage report as second assistant comment"
...
for expected in \
  ...
  "### Usage Ledger" \
  'model=`openai/' \
  'usage_turns=`1` model_backed_turns=`1` deterministic_turns=`0`' \
  "### Usage Turn Evidence" \
  ...
  grep -Fq "$expected" <<<"$session_comment" || die "session usage report missing ${expected}"
done

for leaked in "$hidden_token" "$comment_token" "$search_phrase" "Please audit assistant-turn token telemetry"; do
  if grep -Fq "$leaked" <<<"$session_comment"; then
    die "session usage report leaked ${leaked}"
  fi
done
