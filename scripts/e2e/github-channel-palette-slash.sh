#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-palette slash action queues provider-visible palette updates and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-palette-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-palette-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-palette slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-palette-e2e-${timestamp}"
ingest_message_id="palette-ingest-${timestamp}"
notify_message_id="palette-notify-${timestamp}"
palette_id="palette-${timestamp}"
palette_lane="fun"
palette_note="Use this tiny launcher"
account_id="telegram-palette-account-NOECHO_CHANNEL_PALETTE_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_PALETTE_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_PALETTE_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_PALETTE_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_PALETTE_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_PALETTE_CONTEXT_V1"
search_phrase="channel palette unique search fixture phrase"
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-palette slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-palette slash E2E.
...
palette_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels palette ${palette_lane} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --palette-id ${palette_id}
Note: ${palette_note}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$palette_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel palette action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel palette action receipt"
palette_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "llm_e2e_required_after_channel_palette_action_change: \`true\`"; do
  grep -Fq "$expected" <<<"$palette_receipt" || die "channel palette receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$palette_id" "$palette_lane" "$palette_note" "$expected_token" "/channels roll --dice 2d6"; do
  if grep -Fq "$leaked" <<<"$palette_receipt"; then
    die "channel palette receipt leaked ${leaked}"
  fi
done

[[ "$(palette_notification_count)" == "1" ]] || die "channel palette did not queue exactly one notification"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$notification_bodies" || die "palette notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$palette_id" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$notification_bodies"; then
    die "palette notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing palette notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$palette_id" "$expected_token" "$palette_lane" "$palette_note" "/channels roll --dice 2d6"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels menu ${palette_lane} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --palette-id ${palette_id}
Note: ${palette_note}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel palette action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel palette receipt"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$duplicate_receipt" || die "duplicate channel palette receipt missing ${expected}"
done
[[ "$(palette_notification_count)" == "1" ]] || die "duplicate channel palette queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$palette_id" "$palette_lane" "$palette_note" "$expected_token" "/channels roll --dice 2d6"; do
  if grep -Fq "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel palette receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel palette thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_PALETTE_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, palette ids, palette lanes, palette notes, palette shortcut commands, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel palette model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel palette follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel palette search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel palette follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel palette follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel palette follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel palette follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel palette follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel palette follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$palette_id" "$palette_lane" "$palette_note" "$account_id"; do
  if grep -Fq "$leaked" <<<"$model_comment"; then
    die "model channel palette follow-up leaked ${leaked}"
  fi
done
