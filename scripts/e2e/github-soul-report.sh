...
title="@gitclaw /soul e2e ${timestamp}"
body="Live soul-report E2E.

Please list the loaded GitClaw soul, identity, user, memory, and dated memory files.
This should produce a deterministic soul report without calling a model."
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "soul-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one soul report comment"
...
if grep -Fq "GitClaw is a repo-native GitHub issue assistant" <<<"$comments"; then
  die "soul report leaked full SOUL.md body"
fi

if grep -Fq "GITCLAW_MEMORY_CONTEXT_V1" <<<"$comments"; then
  die "soul report leaked full memory body"
fi
