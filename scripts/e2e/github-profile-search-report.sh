...
log() {
  echo "profile-search-report-e2e: $*" >&2
}
...
hidden_token="NOECHO_PROFILE_SEARCH_${timestamp}"
...
Do not include this hidden profile search token: ${hidden_token}"

local_report="$(go run ./cmd/gitclaw profile search skill_path)"
for expected in \
  ...
  '### Results' \
  'kind=`skill-source` name=`repo-reader`' \
  ...
  grep -Fq -- "$expected" <<<"$local_report" || die "local profile search report missing ${expected}"
done

for leaked in "$expected_token" "$search_phrase" "PROFILE_SEARCH_ISSUE_QUERY_SECRET_${timestamp}" "GitClaw is a repo-native GitHub issue assistant" "Use GitClaw's read-only repository context" "Prefer repository context and deterministic tool outputs"; do
  if grep -Fq "$leaked" <<<"$local_report"; then
    die "local profile search report leaked ${leaked}"
  fi
done
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "profile-search-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one profile search report comment"
...
for expected in \
  ...
  'backup_payload_gate=`excluded`' \
  'llm_e2e_gate=`required`'; do
  grep -Fq -- "$expected" <<<"$search_comment" || die "profile search report missing ${expected}"
done

for leaked in "$hidden_token" "PROFILE_SEARCH_ISSUE_QUERY_SECRET_${timestamp}" "Live profile-search E2E" "$expected_token" "$search_phrase" "GitClaw is a repo-native GitHub issue assistant" "Use GitClaw's read-only repository context" "Prefer repository context and deterministic tool outputs"; do
  if grep -Fq "$leaked" <<<"$search_comment"; then
    die "profile search report leaked ${leaked}"
  fi
done
