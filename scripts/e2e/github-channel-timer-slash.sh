#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-timer slash action queues provider-visible timebox cards and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-timer-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-timer-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-timer slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-timer-e2e-${timestamp}"
ingest_message_id="timer-ingest-${timestamp}"
notify_message_id="timer-notify-${timestamp}"
timer_id="timer-${timestamp}"
timer_duration="25m"
timer_duration_seconds="1500"
timer_label="focus-sprint"
timer_mode="focus"
timer_note="Pairing window is open"
account_id="telegram-timer-account-NOECHO_CHANNEL_TIMER_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_TIMER_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_TIMER_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_TIMER_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_TIMER_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_TIMER_CONTEXT_V1"
search_phrase="channel timer unique search fixture phrase"
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-timer slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-timer slash E2E.
...
timer_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels timer ${timer_duration} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --timer-id ${timer_id} --mode ${timer_mode}
Label: ${timer_label}
Note: ${timer_note}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$timer_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel timer action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel timer action receipt"
timer_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  ...
  "llm_e2e_required_after_channel_timer_action_change: \`true\`"; do
  grep -Fq "$expected" <<<"$timer_receipt" || die "channel timer receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$timer_id" "$timer_duration" "$timer_label" "$timer_note" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$timer_receipt"; then
    die "channel timer receipt leaked ${leaked}"
  fi
done

[[ "$(timer_notification_count)" == "1" ]] || die "channel timer did not queue exactly one notification"
...
for expected in \
  ...
  "Note: ${timer_note}" \
  "Timer hash: " \
  "Duration seconds: ${timer_duration_seconds}" \
  "Note hash: " \
  ...
  grep -Fq "$expected" <<<"$notification_bodies" || die "timer notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$timer_id" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$notification_bodies"; then
    die "timer notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing timer notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$timer_id" "$expected_token" "$timer_duration" "$timer_label" "$timer_mode" "$timer_note"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels timebox ${timer_duration} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --timer-id ${timer_id} --mode ${timer_mode}
Label: ${timer_label}
Note: ${timer_note}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel timer action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel timer receipt"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$duplicate_receipt" || die "duplicate channel timer receipt missing ${expected}"
done
[[ "$(timer_notification_count)" == "1" ]] || die "duplicate channel timer queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$timer_id" "$timer_duration" "$timer_label" "$timer_note" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel timer receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel timer thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_TIMER_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, timer ids, timer durations, timer labels, timer modes, timer notes, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel timer model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel timer follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel timer search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel timer follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel timer follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel timer follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel timer follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel timer follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel timer follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$timer_id" "$timer_duration" "$timer_label" "$timer_mode" "$timer_note" "$account_id"; do
  if grep -Fq "$leaked" <<<"$model_comment"; then
    die "model channel timer follow-up leaked ${leaked}"
  fi
done
