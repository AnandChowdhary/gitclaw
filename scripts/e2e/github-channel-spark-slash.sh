#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel-spark slash action queues provider-visible idea-spark warmup cards and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-spark-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-spark-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel-spark slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-spark-e2e-${timestamp}"
ingest_message_id="spark-ingest-${timestamp}"
notify_message_id="spark-notify-${timestamp}"
spark_id="spark-${timestamp}"
spark_theme="spark"
spark_note="Find the smallest experiment"
account_id="telegram-spark-account-NOECHO_CHANNEL_SPARK_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_SPARK_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_SPARK_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_SPARK_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_SPARK_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_SPARK_CONTEXT_V1"
search_phrase="channel spark unique search fixture phrase"
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel-spark slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel-spark slash E2E.
...
spark_started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels spark --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --spark-id ${spark_id}
Note: ${spark_note}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$spark_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel spark action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel spark action receipt"
spark_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  "GitClaw Channel Warmup Action" \
  "Generated without a model call" \
  'model="gitclaw/channels"' \
  "requested_channel_command: \`/channels spark\`" \
  ...
  grep -Fq -- "$expected" <<<"$spark_receipt" || die "channel spark receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$spark_id" "$spark_note" "$expected_token" "smallest interesting"; do
  if grep -Fq -- "$leaked" <<<"$spark_receipt"; then
    die "channel spark receipt leaked ${leaked}"
  fi
done

[[ "$(spark_notification_count)" == "1" ]] || die "channel spark did not queue exactly one notification"
...
for expected in \
  ...
  "Spark hash: " \
  "Note hash: " \
  "Spark persistence: advisory only; no durable channel state changed." \
  "Spark source: GitHub channel action." \
  ...
  grep -Fq -- "$expected" <<<"$notification_bodies" || die "spark notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$spark_id" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$notification_bodies"; then
    die "spark notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing spark notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$spark_id" "$expected_token" "$spark_note" "smallest interesting"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels brainstorm --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --spark-id ${spark_id}
Note: ${spark_note}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel spark action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel spark receipt"
duplicate_receipt="$(latest_assistant_comment_for_issue "$issue_number")"
for expected in \
  "GitClaw Channel Warmup Action" \
  "requested_channel_command: \`/channels brainstorm\`" \
  ...
  grep -Fq -- "$expected" <<<"$duplicate_receipt" || die "duplicate channel spark receipt missing ${expected}"
done
[[ "$(spark_notification_count)" == "1" ]] || die "duplicate channel spark queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$spark_id" "$spark_note" "$expected_token" "smallest interesting"; do
  if grep -Fq -- "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel spark receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel spark thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_SPARK_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, spark ids, warmup ids, spark themes, spark notes, spark prompt text, issue numbers, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel spark model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel spark follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq -- "$expected_token" <<<"$model_comment" || die "assistant did not include channel spark search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel spark follow-up marker did not use configured GitHub models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel spark follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel spark follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel spark follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel spark follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel spark follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$spark_id" "$spark_note" "$account_id"; do
  if grep -Fq -- "$leaked" <<<"$model_comment"; then
    die "model channel spark follow-up leaked ${leaked}"
  fi
done
