...
hidden_token="GITCLAW_TASKS_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_TASKS_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /tasks risk e2e ${timestamp}"
body="@gitclaw /tasks risk
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "tasks-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one tasks risk report comment"
risk_comment="$(latest_assistant_comment)"

for expected in \
  'model="gitclaw/tasks"' \
  "GitClaw Task Risk Report" \
  "Generated without a model call" \
  'task_risk_status: `ok`' \
  'verification_scope: `github_issue_task_metadata`' \
  'task_policy_present: `true`' \
  'task_policy_loaded_for_model: `true`' \
  'task_specs: `1`' \
  'scanned_task_specs: `1`' \
  ...
  'llm_e2e_required_after_task_risk_change: `true`' \
  'current_task_status: `ready`' \
  'current_task_labels: `1`' \
  "### Task Policy Risk Card" \
  'kind=`task-policy` path=`.gitclaw/TASKS.md`' \
  'risk_findings=`0`' \
  'risk_codes=`none`' \
  "### Task Spec Risk Cards" \
  'kind=`task-spec` name=`issue-native-board` path=`.gitclaw/tasks/issue-native-board.md`' \
  'mode=`issue-native`' \
  'statuses=`6`' \
  'labels=`7`' \
  ...
  'transcript_bodies_scanned=`false`' \
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "tasks risk report missing ${expected}"
done

for leaked in "$hidden_token" "GITCLAW_TASKS_CONTEXT_V1" "This declarative task board" "Task policy verification token" "bounded repository search fixture phrase"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "tasks risk report leaked ${leaked}"
  fi
done
