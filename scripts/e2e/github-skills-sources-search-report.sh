...
log() {
  echo "skills-sources-search-e2e: $*" >&2
}
...
hidden_token="NOECHO_SKILL_SOURCE_SEARCH_${timestamp}"
followup_hidden_token="NOECHO_SKILL_SOURCE_SEARCH_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_SKILL_SOURCE_SEARCH_CONTEXT_V1"
expected_hash="2f9e68a57bd6"
search_phrase="skill source search unique search fixture phrase"
title="@gitclaw /skills sources search repo-local manual-review"
body="@gitclaw /skills sources search repo-local manual-review

Live skill source search E2E ${timestamp}. Keep the source-pin search report body-free.
Do not include this hidden skill source search token: ${hidden_token}"

local_report="$(go run ./cmd/gitclaw skills sources search repo-local manual-review)"
for expected in \
  ...
  'risk_codes=`none`'; do
  grep -Fq -- "$expected" <<<"$local_report" || die "local skill source search report missing ${expected}"
done

for leaked in "$hidden_token" "Live skill source search E2E" "Skill context verification token" "GITCLAW_SKILL_CONTEXT_V1" "$expected_token" "$search_phrase"; do
  if grep -Fq "$leaked" <<<"$local_report"; then
    die "local skill source search report leaked ${leaked}"
  fi
done
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "skills-sources-search e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one skill source search report comment"
...
for expected in \
  ...
  'risk_max_severity=`none`' \
  'risk_codes=`none`'; do
  grep -Fq -- "$expected" <<<"$search_comment" || die "skill source search report missing ${expected}"
done

for leaked in "$hidden_token" "Live skill source search E2E" "Skill context verification token" "GITCLAW_SKILL_CONTEXT_V1" "$expected_token" "$search_phrase"; do
  if grep -Fq "$leaked" <<<"$search_comment"; then
    die "skill source search report leaked ${leaked}"
  fi
done
