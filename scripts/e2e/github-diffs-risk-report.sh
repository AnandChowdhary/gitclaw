...
hidden_token="GITCLAW_DIFFS_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_DIFFS_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /diffs risk e2e ${timestamp}"
body="@gitclaw /diffs risk
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "diffs-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one diffs risk report comment"
...
for expected in \
  ...
  'kind=`changed-file` none' \
  "### Current Diff Request Risk Card" \
  'current_issue_diff_request=`true`' \
  'issue_body_scanned=`false`' \
  'comment_bodies_scanned=`false`' \
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "diffs risk report missing ${expected}"
done

for leaked in "$hidden_token" "GITCLAW_DIFFS_CONTEXT_V1" "This declarative diff record" "DIFFS_CONTEXT" "bounded repository search fixture phrase"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "diffs risk report leaked ${leaked}"
  fi
done
