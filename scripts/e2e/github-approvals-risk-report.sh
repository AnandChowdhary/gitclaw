...
log() {
  echo "approvals-risk-report-e2e: $*" >&2
}
...
hidden_token="GITCLAW_APPROVALS_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_APPROVALS_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /approvals risk e2e ${timestamp}"
body="@gitclaw /approvals risk

Live approvals-risk E2E.
Please implement this change and open a PR, but the approvals risk report must stay read-only and body-free.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "approvals-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one approvals risk report comment"
...
for expected in \
  ...
  'kind=`approval-gates` store=`github-issue-labels` scope=`per-issue`' \
  ...
  'association=`MEMBER` trusted=`true` broad=`false`' \
  'association=`COLLABORATOR` trusted=`true` broad=`false`' \
  "### Approval Label Risk Cards" \
  'role=`write-requested` label=`gitclaw:write-requested` managed=`true`' \
  'role=`approved` label=`gitclaw:approved` managed=`false`' \
  'role=`needs-human` label=`gitclaw:needs-human` managed=`false`' \
  "### Runtime Boundary Risk Card" \
  'kind=`runtime-boundary` write_actions_supported=`false`' \
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "approvals risk report missing ${expected}"
done

for leaked in \
  "$hidden_token" \
  "Please implement this change and open a PR" \
  "Live approvals-risk E2E" \
  "bounded repository search fixture phrase"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "approvals risk report leaked ${leaked}"
  fi
done

labels="$(issue_label_names)"
grep -Fxq "gitclaw:write-requested" <<<"$labels" || die "write-requested label missing after approvals risk report"
grep -Fxq "gitclaw:approved" <<<"$labels" || die "approved label missing after approvals risk report"
