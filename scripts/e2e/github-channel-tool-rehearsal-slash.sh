#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel rehearse-tool creates a tool rehearsal issue and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-tool-rehearsal-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-tool-rehearsal-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel tool rehearsal slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-tool-rehearsal-e2e-${timestamp}"
ingest_message_id="tool-rehearsal-ingest-${timestamp}"
rehearsal_id="channel-tool-rehearsal-${timestamp}"
notify_message_id="tool-rehearsal-notify-${timestamp}"
account_id="telegram-tool-rehearsal-account-NOECHO_CHANNEL_TOOL_REHEARSAL_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_TOOL_REHEARSAL_INGEST_${timestamp}"
source_hidden_token="NOECHO_CHANNEL_TOOL_REHEARSAL_SOURCE_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_TOOL_REHEARSAL_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_TOOL_REHEARSAL_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_TOOL_REHEARSAL_CONTEXT_V1"
search_phrase="channel tool rehearsal unique search fixture phrase"
notify_message_hash="$(sha256_12 "$notify_message_id")"
issue_number=""
rehearsal_issue_number=""
issue_title="GitClaw ${channel} thread ${thread_id}"
rehearsal_issue_title="GitClaw tool rehearsal: gitclaw.search_files (${rehearsal_id})"
...
tool_rehearsal_notification_count() {
  ...
  for number in "$issue_number" "$rehearsal_issue_number"; do
    if [[ -n "${number:-}" && "$number" != "null" ]]; then
      ...
      if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
        gh issue close "$number" --repo "$repo" --comment "channel tool rehearsal slash e2e cleanup" >/dev/null 2>&1 || true
      fi
    fi
  done
  rm -rf "$lock_dir"
...
Mirrored Telegram thread for channel tool rehearsal slash E2E.
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels rehearse-tool search_files --id ${rehearsal_id} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id}

Please rehearse a channel-origin search_files tool boundary.
Do not include this hidden source token: ${source_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$request_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel tool rehearsal action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel tool rehearsal action receipt"
...
for expected in \
  ...
  "llm_e2e_required_after_channel_tool_rehearsal_change: \`true\`"; do
  grep -Fq "$expected" <<<"$request_receipt" || die "channel tool rehearsal receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$source_hidden_token" "$rehearsal_id" "$thread_id" "$ingest_message_id" "$notify_message_id" "Please rehearse a channel-origin"; do
  if grep -Fq "$leaked" <<<"$request_receipt"; then
    die "channel tool rehearsal receipt leaked ${leaked}"
  fi
done

rehearsal_issue_number="$(sed -n 's/.*rehearsal_issue: `#\([0-9][0-9]*\)`.*/\1/p' <<<"$request_receipt" | head -n 1)"
[[ -n "$rehearsal_issue_number" && "$rehearsal_issue_number" != "null" ]] || die "could not resolve tool rehearsal issue number"
log "channel tool rehearsal created rehearsal issue #${rehearsal_issue_number}"

[[ "$(tool_rehearsal_notification_count)" == "1" ]] || die "channel tool rehearsal did not queue exactly one rehearsal-link outbound comment"
rehearsal_json="$(gh issue view "$rehearsal_issue_number" --repo "$repo" --json title,body,state)"
rehearsal_title="$(jq -r '.title' <<<"$rehearsal_json")"
rehearsal_body="$(jq -r '.body' <<<"$rehearsal_json")"
[[ "$rehearsal_title" == "$rehearsal_issue_title" ]] || die "unexpected tool rehearsal issue title: ${rehearsal_title}"
for expected in \
  ...
  grep -Fq "$expected" <<<"$rehearsal_body" || die "tool rehearsal issue body missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$source_hidden_token" "Please rehearse a channel-origin"; do
  if grep -Fq "$leaked" <<<"$rehearsal_body"; then
    die "tool rehearsal issue body leaked ${leaked}"
  fi
done
...
grep -Fq "gitclaw:channel-outbound" <<<"$outbound_bodies" || die "channel issue missing tool rehearsal outbound marker"
grep -Fq "GitClaw channel tool rehearsal" <<<"$outbound_bodies" || die "channel issue missing visible tool rehearsal notification"
grep -Fq "Normalized tool: gitclaw.search_files" <<<"$outbound_bodies" || die "channel issue missing normalized tool in notification"
grep -Fq "Rehearsal issue: #${rehearsal_issue_number}" <<<"$outbound_bodies" || die "channel issue missing rehearsal issue number in notification"
if grep -Fq "$source_hidden_token" <<<"$outbound_bodies"; then
  die "channel tool rehearsal notification leaked source token"
fi
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing tool rehearsal outbound hash ${notify_message_hash}"
...
wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel tool rehearsal action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel tool rehearsal receipt"
...
for expected in \
  ...
  grep -Fq "$expected" <<<"$duplicate_receipt" || die "duplicate channel tool rehearsal receipt missing ${expected}"
done
[[ "$(tool_rehearsal_notification_count)" == "1" ]] || die "duplicate channel tool rehearsal queued another rehearsal-link notification"
for leaked in "$duplicate_hidden_token" "$rehearsal_id" "$thread_id" "$ingest_message_id" "$notify_message_id"; do
  if grep -Fq "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel tool rehearsal receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$rehearsal_issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel-created tool rehearsal and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_TOOL_REHEARSAL_\`.
...
Do not include rehearsal ids, message ids, thread ids, account hashes, issue numbers, requested tool names, or source bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$rehearsal_issue_title")" || die "timed out waiting for channel tool rehearsal model follow-up"
wait_for_assistant_count_for_issue "$rehearsal_issue_number" 1 || die "expected model-backed channel tool rehearsal follow-up"
model_comment="$(latest_assistant_comment_for_issue "$rehearsal_issue_number")"

grep -Fq "$expected_token" <<<"$model_comment" || die "assistant did not include channel tool rehearsal search_files token ${expected_token}"
if ! grep -Fq 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel tool rehearsal follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel tool rehearsal follow-up marker missing prompt context hash"
grep -Fq 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel tool rehearsal follow-up marker missing selected repo-reader skill"
grep -Fq 'tools="' <<<"$model_comment" || die "assistant channel tool rehearsal follow-up marker missing prompt-visible tools"
grep -Fq 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel tool rehearsal follow-up marker did not prove search_files was prompt-visible"
grep -Fq 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel tool rehearsal follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$source_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$account_id" "$rehearsal_id" "$notify_message_id"; do
  if grep -Fq "$leaked" <<<"$model_comment"; then
    die "model channel tool rehearsal follow-up leaked ${leaked}"
  fi
done
