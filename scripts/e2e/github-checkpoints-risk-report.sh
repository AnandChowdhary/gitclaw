...
log() {
  echo "checkpoints-risk-report-e2e: $*" >&2
}
...
hidden_token="GITCLAW_CHECKPOINTS_RISK_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_CHECKPOINTS_RISK_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /rollback risk e2e ${timestamp}"
body="@gitclaw /rollback risk

Live checkpoints-risk E2E.
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "checkpoints-risk-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
wait_for_assistant_count 1 || die "expected one checkpoints risk report comment"
...
for expected in \
  ...
  'current_issue_checkpoint_request=`true`' \
  'issue_body_scanned=`false`' \
  'comment_bodies_scanned=`false`' \
  "### Risk Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$risk_comment" || die "checkpoints risk report missing ${expected}"
done

for leaked in "$hidden_token" "$search_phrase" "Live checkpoints-risk E2E" "Do not include this hidden issue token"; do
  if grep -Fq "$leaked" <<<"$risk_comment"; then
    die "checkpoints risk report leaked ${leaked}"
  fi
done
