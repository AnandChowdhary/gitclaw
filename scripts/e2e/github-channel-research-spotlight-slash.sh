#!/usr/bin/env bash
# gitclaw-doctor-live-issue: channel research-spotlight queues catalog cards and proves model/tool follow-up.
set -euo pipefail

log() {
  echo "channel-research-spotlight-slash-e2e: $*" >&2
}
...
lock_dir="/tmp/gitclaw-channel-research-spotlight-slash-e2e.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  die "another channel research-spotlight slash E2E appears to be running: ${lock_dir}"
fi
...
channel="telegram"
thread_id="channel-research-spotlight-e2e-${timestamp}"
ingest_message_id="research-spotlight-ingest-${timestamp}"
notify_message_id="research-spotlight-notify-${timestamp}"
research_spotlight_id="research-spotlight-${timestamp}"
research_focus="openclaw"
account_id="telegram-research-spotlight-account-NOECHO_CHANNEL_RESEARCH_SPOTLIGHT_ACCOUNT_${timestamp}"
ingest_hidden_token="NOECHO_CHANNEL_RESEARCH_SPOTLIGHT_INGEST_${timestamp}"
command_hidden_token="NOECHO_CHANNEL_RESEARCH_SPOTLIGHT_COMMAND_${timestamp}"
duplicate_hidden_token="NOECHO_CHANNEL_RESEARCH_SPOTLIGHT_DUPLICATE_${timestamp}"
followup_hidden_token="NOECHO_CHANNEL_RESEARCH_SPOTLIGHT_FOLLOWUP_${timestamp}"
expected_token="GITCLAW_CHANNEL_RESEARCH_SPOTLIGHT_CONTEXT_V1"
search_phrase="channel research spotlight unique search fixture phrase"
...
research_spotlight_notification_count() {
...
cleanup() {
  if [[ -n "${issue_number:-}" && "$issue_number" != "null" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "channel research-spotlight slash e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
  rm -rf "$lock_dir"
}
...
Mirrored Telegram thread for channel research-spotlight slash E2E.
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels research-spotlight ${research_focus} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --spotlight-id ${research_spotlight_id}
Do not include this command hidden token in the receipt: ${command_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$spotlight_started_at" "$issue_title" >/dev/null || die "timed out waiting for channel research-spotlight action"
wait_for_assistant_count_for_issue "$issue_number" 2 || die "expected channel research-spotlight action receipt"
...
for expected in \
  ...
  "llm_e2e_required_after_channel_research_spotlight_change: \`true\`"; do
  grep -Fq -- "$expected" <<<"$spotlight_receipt" || die "channel research-spotlight receipt missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$research_spotlight_id" "$research_focus" "docs.openclaw.ai" "$expected_token"; do
  if grep -Fqi -- "$leaked" <<<"$spotlight_receipt"; then
    die "channel research-spotlight receipt leaked ${leaked}"
  fi
done

[[ "$(research_spotlight_notification_count)" == "1" ]] || die "channel research-spotlight did not queue exactly one notification"
...
for expected in \
  ...
  grep -Fq -- "$expected" <<<"$notification_bodies" || die "research-spotlight notification missing ${expected}"
done
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$research_spotlight_id" "$expected_token"; do
  if grep -Fq -- "$leaked" <<<"$notification_bodies"; then
    die "research-spotlight notification leaked ${leaked}"
  fi
done
...
jq -e --arg hash "$notify_message_hash" '.messages[] | select(.kind == "channel-outbound" and .outbound_message_sha256_12 == $hash)' "$outbox_file" >/dev/null || die "outbox file missing research-spotlight notify hash ${notify_message_hash}"
for leaked in "$account_id" "$ingest_hidden_token" "$command_hidden_token" "$research_spotlight_id" "$research_focus" "docs.openclaw.ai" "$expected_token"; do
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw /channels research-draw ${research_focus} --message-id ${ingest_message_id} --notify-message-id ${notify_message_id} --spotlight-id ${research_spotlight_id}
Do not include this duplicate hidden token in any receipt: ${duplicate_hidden_token}" >/dev/null

wait_for_issue_comment_run_for_title "$duplicate_started_at" "$issue_title" >/dev/null || die "timed out waiting for duplicate channel research-spotlight action"
wait_for_assistant_count_for_issue "$issue_number" 3 || die "expected duplicate channel research-spotlight receipt"
...
for expected in \
  ...
  grep -Fq -- "$expected" <<<"$duplicate_receipt" || die "duplicate channel research-spotlight receipt missing ${expected}"
done
[[ "$(research_spotlight_notification_count)" == "1" ]] || die "duplicate channel research-spotlight queued another notification"
for leaked in "$duplicate_hidden_token" "$thread_id" "$ingest_message_id" "$notify_message_id" "$research_spotlight_id" "$research_focus" "$expected_token"; do
  if grep -Fqi -- "$leaked" <<<"$duplicate_receipt"; then
    die "duplicate channel research-spotlight receipt leaked ${leaked}"
  fi
done
...
gh issue comment "$issue_number" \
  --repo "$repo" \
  --body "@gitclaw Continue this channel research-spotlight thread and use the repo-reader skill.

Search the repository for \`${search_phrase}\`.
The matching repository search result line has the form \`${search_phrase} => <token>\`.
The exact answer starts with \`GITCLAW_CHANNEL_RESEARCH_SPOTLIGHT_\`.
...
Do not include provider ids, notification ids, thread ids, message ids, account hashes, spotlight ids, issue numbers, focus text, source URLs, or previous channel bodies.
Do not include this hidden follow-up token: ${followup_hidden_token}
Keep the answer under 30 words." >/dev/null

model_run_json="$(wait_for_issue_comment_run_for_title "$model_started_at" "$issue_title")" || die "timed out waiting for channel research-spotlight model follow-up"
wait_for_assistant_count_for_issue "$issue_number" 4 || die "expected model-backed channel research-spotlight follow-up"
model_comment="$(latest_assistant_comment_for_issue "$issue_number")"

grep -Fq -- "$expected_token" <<<"$model_comment" || die "assistant did not include channel research-spotlight search_files token ${expected_token}"
if ! grep -Fq -- 'model="openai/gpt-5-nano"' <<<"$model_comment" && ! grep -Fq -- 'model="openai/gpt-4.1-nano"' <<<"$model_comment"; then
  die "assistant channel research-spotlight follow-up marker did not use configured GitHub Models primary or fallback"
fi
grep -Fq -- 'prompt_context_sha256_12="' <<<"$model_comment" || die "assistant channel research-spotlight follow-up marker missing prompt context hash"
grep -Fq -- 'skills="repo-reader"' <<<"$model_comment" || die "assistant channel research-spotlight follow-up marker missing selected repo-reader skill"
grep -Fq -- 'tools="' <<<"$model_comment" || die "assistant channel research-spotlight follow-up marker missing prompt-visible tools"
grep -Fq -- 'gitclaw.search_files' <<<"$model_comment" || die "assistant channel research-spotlight follow-up marker did not prove search_files was prompt-visible"
grep -Fq -- 'usage_total_tokens="' <<<"$model_comment" || die "assistant channel research-spotlight follow-up marker missing usage token telemetry"
for leaked in "$ingest_hidden_token" "$command_hidden_token" "$duplicate_hidden_token" "$followup_hidden_token" "$research_spotlight_id" "$account_id" "$research_focus"; do
  if grep -Fq -- "$leaked" <<<"$model_comment"; then
    die "model channel research-spotlight follow-up leaked ${leaked}"
  fi
done
