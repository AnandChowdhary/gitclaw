...
log() {
  echo "tools-boundary-report-e2e: $*" >&2
}
...
hidden_token="GITCLAW_TOOLS_BOUNDARY_HIDDEN_${timestamp}"
followup_hidden_token="GITCLAW_TOOLS_BOUNDARY_FOLLOWUP_HIDDEN_${timestamp}"
expected_token="GITCLAW_SEARCH_CONTEXT_V1"
search_phrase="bounded repository search fixture phrase"
title="@gitclaw /tools boundary e2e ${timestamp}"
body="@gitclaw /tools boundary

Please inspect \`go.mod\` and search for \`${search_phrase}\`.
Do not include this hidden issue token: ${hidden_token}"

local_report="$(go run ./cmd/gitclaw tools boundary "go.mod \"${search_phrase}\"")"
for expected in \
  ...
  'gate=`prompt_injection` state=`0` result=`pass`'; do
  grep -Fq -- "$expected" <<<"$local_report" || die "local tools boundary report missing ${expected}"
done
...
for leaked in "module github.com/AnandChowdhary/gitclaw" "$search_phrase" "$expected_token"; do
  if grep -Fq "$leaked" <<<"$local_report"; then
    die "local tools boundary report leaked ${leaked}"
  fi
done
...
cleanup() {
  if [[ -n "${issue_number:-}" ]]; then
    ...
    if [[ "${GITCLAW_E2E_KEEP_ISSUE:-0}" != "1" ]]; then
      gh issue close "$issue_number" --repo "$repo" --comment "tools-boundary-report e2e cleanup" >/dev/null 2>&1 || true
    fi
  fi
}
trap cleanup EXIT
...
boundary_run_json="$(wait_for_run issues "$issue_started_at")" || die "timed out waiting for issues workflow run"
wait_for_assistant_count 1 || die "expected one tools boundary report comment"
boundary_comment="$(latest_assistant_comment)"
...
for expected in \
  ...
  'tool_outputs_hashed: `4`' \
  'prompt_boundary_findings: `0`' \
  'prompt_boundary_high_findings: `0`' \
  'model_callable_structured_tools: `false`' \
  ...
  "### Boundary Findings" \
  "- none"; do
  grep -Fq -- "$expected" <<<"$boundary_comment" || die "tools boundary report missing ${expected}"
done

for leaked in "$hidden_token" "module github.com/AnandChowdhary/gitclaw" "$expected_token" "$search_phrase"; do
  if grep -Fq "$leaked" <<<"$boundary_comment"; then
    die "tools boundary report leaked ${leaked}"
  fi
done
...
boundary_url="$(jq -r '.url' <<<"$boundary_run_json")"
model_url="$(jq -r '.url' <<<"$model_run_json")"
log "passed for issue #${issue_number}: ${boundary_url} (model follow-up: ${model_url})"
