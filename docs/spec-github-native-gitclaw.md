...
The same channel-thread issue can ask for a compact cockpit/status board:

```text
@gitclaw /channels cockpit research --cockpit-id <stable-cockpit-id> --message-id <provider-message-id> --notify-message-id <stable-outbound-id>
```

`/channels cockpit`, `/channels control-room`, `/channels status-board`,
`/channels dashboard`, `/channels ops-board`, `/channels channel-cockpit`,
`/channels openclaw-cockpit`, and `/channels hermes-cockpit` infer the current
channel and thread id from the issue marker when no explicit route/channel/
thread target is provided. They normalize a lane across skills, tools, soul,
memory, backups, research, channels, launch, or fun, then queue a
provider-facing card with a board sentence, four bounded gauges, and copyable
follow-up commands. The source receipt records only route/thread/message/
cockpit/lane/note/card hashes, counts, duplicate status, delivery metadata,
and hard no-model/no-execution/no-provider-API/no-workflow/no-policy/
no-schedule/no-repository-mutation gates. It does not print raw cockpit ids,
raw lane names, raw notes, raw gauge text, raw command text, raw provider ids,
raw channel bodies, raw issue/comment bodies, prompts, or tool outputs.
Duplicates are suppressed by `channel + notify_message_id`. Changes to this
surface require a live E2E that ingests a real channel issue, queues a cockpit
card, validates metadata-only outbox discovery, verifies duplicate suppression,
verifies no dynamic cockpit generation/source fetch/live browse/model/tool/
provider API/workflow/policy/schedule/repository mutation happened, and then
continues on the same GitHub issue with a real GitHub Models repo-reader/search
follow-up.
...
For provider-facing next actions that should feel like native reply chips
rather than a report, GitClaw also supports:

```text
@gitclaw /channels quick-replies handoff --reply-id <stable-replies-id> --message-id <stable-inbound-id> --notify-message-id <stable-outbound-id>
Note: Keep replies short.
```

`/channels quick-replies`, `/channels quick-reply`, `/channels reply-options`,
`/channels reply-chips`, `/channels chips`, `/channels suggest-replies`,
`/channels suggested-replies`, `/channels reply-suggestions`, and
`/channels next-replies` queue one provider-facing reply-chip card back onto the
current `gitclaw:channel-thread` issue or an explicit reviewed route. Lanes are
static and bounded: `general`, `handoff`, `skills`, `tools`, and `fun`. The
provider-facing outbound card may show the lane, optional note, option labels,
copyable suggested commands, and short reasons. The source receipt remains
body-free and reports only hashes, note size, option count, duplicate status,
outbox delivery instructions, and safety gates. This action does not execute
commands, create artifacts/tasks/reminders, install skills, execute tools, call
a model, call provider APIs, edit workflows, mutate repository files, print raw
lanes, print raw notes, print raw reply ids, print option text, or print channel
bodies. Duplicates are suppressed by `channel + notify_message_id`. Changes to
this surface require a live E2E that ingests a real channel issue, queues the
quick-replies card, validates metadata-only outbox discovery, verifies duplicate
suppression, proves no command/artifact/task/reminder/model/provider-API/
workflow/skill/tool/repository mutation side effects, and then continues on the
same GitHub issue with a real GitHub Models repo-reader/search follow-up.

For provider-facing team posture that should feel like a tiny deterministic
spin rather than a report, GitClaw also supports:

```text
@gitclaw /channels status-wheel release --wheel-id <stable-wheel-id> --message-id <stable-inbound-id> --notify-message-id <stable-outbound-id>
Note: Keep the spin small.
```

`/channels status-wheel`, `/channels statuswheel`, `/channels wheel`,
`/channels spin`, `/channels status-spin`, `/channels signal-wheel`,
`/channels signal-spin`, `/channels traffic-light`, and `/channels traffic`
queue one provider-facing status spin back onto the current
`gitclaw:channel-thread` issue or an explicit reviewed route. Lanes are static
and bounded: `focus`, `release`, `triage`, `tools`, `soul`, and `fun`. The
provider-facing outbound card may show the lane, selected status, selected
micro-action, note, deck hash, and deterministic seed hash. The source receipt
remains body-free and reports only hashes, note size, deck count, selected
index, duplicate status, outbox delivery instructions, and safety gates. This
action does not call a model, use external randomness, execute commands, create
artifacts/tasks/reminders, install skills, execute tools, call provider APIs,
edit workflows, persist status, mutate repository files, print raw lanes, print
raw notes, print raw wheel ids, print deck text, print selected status/action
text, or print channel bodies. Duplicates are suppressed by
`channel + notify_message_id`. Changes to this surface require a live E2E that
ingests a real channel issue, queues the status-wheel card, validates
metadata-only outbox discovery, verifies duplicate suppression, proves no
model/randomness/command/artifact/task/reminder/provider-API/workflow/status/
skill/tool/repository mutation side effects, and then continues on the same
GitHub issue with a real GitHub Models repo-reader/search follow-up.
...
For lightweight celebrations that should stay in chat instead of becoming a
durable kudos issue, GitClaw also supports:

```text
@gitclaw /channels toast launch-ready --toast-id <stable-toast-id> --message-id <stable-inbound-id> --notify-message-id <stable-outbound-id>
Reason: Release handoff steady.
```

`/channels toast`, `/channels cheer`, `/channels cheers`, `/channels salute`,
`/channels ship-it`, `/channels shipit`, `/channels high-five`, and
`/channels highfive` queue one provider-facing celebration toast back onto the
current `gitclaw:channel-thread` issue or an explicit reviewed route. The
toast title, tone, and optional `Reason: ...` trailing text are visible in the
provider-facing update; the source receipt remains body-free and reports only
hashes, reason size, duplicate status, outbox delivery instructions, and safety
gates. It does not open durable kudos issues, call a model, call provider APIs,
edit workflows, mutate repository files, print raw toast ids, print raw toast
titles, or print raw reasons. Duplicates are suppressed by
`channel + notify_message_id`. Changes to this surface require a live E2E that
ingests a real channel issue, queues the toast card, validates metadata-only
outbox discovery, verifies duplicate suppression, proves no kudos issue/model/
...
```text
@gitclaw /channels timer 25m --timer-id <stable-timer-id> --message-id <stable-inbound-id> --notify-message-id <stable-outbound-id>
Label: Pairing sprint
Note: Check in before handoff.
```

`/channels timer`, `/channels timebox`, `/channels countdown`,
`/channels focus`, `/channels focus-timer`, `/channels pomodoro`, and
`/channels break` queue one provider-facing timer/timebox cue back onto the
current `gitclaw:channel-thread` issue or an explicit reviewed route. The
duration, label, mode, and optional `Note: ...` trailing text are visible in
the provider-facing update; the source receipt remains body-free and reports
only hashes, duration seconds, sizes, duplicate status, outbox delivery
instructions, and safety gates. It does not create reminder issues, schedule
workflows, start provider timers, call a model, call provider APIs, edit
workflows, mutate repository files, print raw timer ids, print raw labels, or
print raw notes. Duplicates are suppressed by `channel + notify_message_id`.
Changes to this surface require a live E2E that ingests a real channel issue,
queues the timer card, validates metadata-only outbox discovery, verifies
duplicate suppression, proves no reminder issue/scheduled workflow/provider
timer/model/provider-API/workflow/repository mutation was performed, and then
continues on the same GitHub issue with a real GitHub Models repo-reader/search
follow-up.
...
For channel-native orientation that should guide safe next steps across
repo-backed capabilities without executing anything, GitClaw also supports:

```text
@gitclaw /channels compass all --compass-id <stable-compass-id> --message-id <stable-inbound-id> --notify-message-id <stable-outbound-id>
Note: Start with body-free recall, then propose reviewed writes.
```

`/channels compass`, `/channels orient`, `/channels orientation`,
`/channels navigator`, `/channels navigate`, `/channels whereami`,
`/channels map`, `/channels wayfinder`, and `/channels guide` queue one
provider-facing safe-next-step orientation card back onto the current
`gitclaw:channel-thread` issue or an explicit reviewed route. The focus is
bounded to `all`, `core`, `skills`, `tools`, `soul`, `memory`, `backups`, or
`fun`. Each focus renders a compact static list of safe channel-native next
steps; it does not execute those commands. Optional `Note: ...` trailing text
is included in the provider-facing update. The source receipt remains
body-free and reports only hashes, sizes, step count, duplicate status, outbox
delivery instructions, and safety gates. It does not execute commands, install
skills, execute tools, read backup payloads, read soul bodies, call provider
APIs, call a model, mutate repository files, or perform provider delivery. It
does not print raw compass ids, focus values, notes, step text, thread ids,
message ids, or channel bodies in the source receipt. Duplicates are
suppressed by `channel + notify_message_id`. Changes to this surface require a
live E2E that ingests a real channel issue, queues the compass, validates
metadata-only outbox discovery, verifies duplicate suppression, proves no
command-execution/skill-install/tool-execution/backup-payload-read/
soul-body-read/provider-API/model/repository mutation was performed, and then
continues on the same GitHub issue with a real GitHub Models
...
For channel-native archive discovery that should surface one useful backup
candidate without restoring anything, GitClaw also supports:

```text
@gitclaw /channels backup-spotlight deployment --spotlight-id <stable-spotlight-id> --message-id <stable-inbound-id> --notify-message-id <stable-outbound-id>
```

`/channels backup-spotlight`, `/channels backups-spotlight`,
`/channels spotlight-backup`, `/channels backup-pick`,
`/channels backup-draw`, `/channels recovery-spotlight`,
`/channels recovery-draw`, `/channels archive-spotlight`, and
`/channels archive-draw` infer the current channel and thread id from the
issue marker when no explicit route/channel thread target is provided. They
inspect the repo's `gitclaw-backups` archive with the same body-free backup
search and timeline metadata used by `/backup search` and `/backup timeline`.
If the focus matches backed-up archive lines, the action draws deterministically
from those hash-only search results; otherwise it falls back to recent backup
timeline metadata. In a GitHub Actions handler job, the local checkout normally
does not contain `.gitclaw/backups`; the action must therefore fetch
`gitclaw-backups` read-only into a temporary git worktree, build the spotlight
from that fetched backup root, and clean it up before exit. If local backups
are present, the action may use them without fetching. It must never write the
backup branch, restore files, replay GitHub API calls, call provider APIs,
mutate repository files, execute tools, or call a model.

The provider-facing outbound comment reports only spotlight status, backup
verify status, backup fetch status, backup branch, focus hash, focus term
count, schema version, issue/search/candidate counts, selected index,
selection hashes, and one selected backup card with issue number,
backup-relative path, source, source kind, role, trust flag, line/score counts,
body hashes, line hashes, payload size/hash, comment/transcript counts, title
hash, timestamp, event, and safe follow-up commands. The source receipt is
stricter: it records target issue/comment ids, route/thread/message hashes,
spotlight id hash, focus/note hashes, backup root hash, selected issue/path/
source/role/line/payload hashes, selection hashes, outbox delivery
instructions, and safety gates. It does not print raw focus text, notes,
spotlight ids, provider thread/message ids, backup roots, backup paths, issue
titles, backup payloads, channel message bodies, issue bodies, comment bodies,
transcript bodies, prompts, or tool outputs. Duplicates are suppressed by
`channel + notify_message_id`. Changes to this surface require a live E2E that
ingests a real channel issue, waits until that issue is present on the real
`gitclaw-backups` branch, queues a backup-spotlight notification, validates
metadata-only outbox discovery, verifies duplicate suppression, and then
continues on the same GitHub issue with a real GitHub Models repo-reader/search
follow-up.
...
@gitclaw /channels platform <provider> --state <state> --message-id <provider-message-id>
...
@gitclaw /channels propose-bundle --bundle-id <stable-bundle-proposal-id> --message-id <provider-message-id>
...
The same channel-thread issue can also capture a link card:

```text
@gitclaw /channels link --link-id <stable-link-id> --url <url> --message-id <provider-message-id>
Link: short link-card title
Notes:
optional human-readable follow-up context
```

`/channels link`, `/channels url`, `/channels link-card`,
`/channels reference`, and `/channels ref` infer the current channel and
thread id from the issue marker when no explicit route/channel/thread target
is provided. They create or reuse one open GitHub issue carrying a hidden
`gitclaw:channel-link` marker for the stable link id, label it with `gitclaw`
so normal conversation can continue there, and queue a provider-facing
link-card issue link back to the mirrored channel thread. The link-card issue
contains the human-readable title and notes plus the URL hash because it is the
reviewable, searchable follow-up surface; the source receipt remains
body-free, reporting only link/thread/message/title/note/URL hashes, duplicate
status, notification queue metadata, and delivery gates. It does not call a
model, call provider APIs, fetch URLs, expand redirects, scrape pages, print
raw link ids, print raw thread ids, print raw source or notification message
ids, print raw URLs, print channel message bodies, or print raw titles/notes
in the source receipt. Duplicates are suppressed first by `link_id` for the
GitHub link-card issue and then by `channel + notify_message_id` for the
provider-facing link-card notification. Changes to this surface require a live
E2E that captures the link card, validates the metadata-only link-card outbox,
checks duplicate suppression, verifies raw URLs do not leak into receipts, and
...
```text
@gitclaw /channels access-request --access-id <stable-access-id> --scope <scope> --role <role> --message-id <provider-message-id>
Requester: visible requester name
Reason:
human-readable reason for review
```

`/channels access`, `/channels access-request`, `/channels request-access`,
`/channels pair-request`, `/channels pairing-request`, `/channels pairing`,
`/channels allow-request`, `/channels permission-request`, and
`/channels permissions` infer the current channel and thread id from the issue
marker when no explicit route/channel/thread target is provided. They create
or reuse one open GitHub issue carrying a hidden
`gitclaw:channel-access-request` marker for the stable access id, label it
with `gitclaw` so normal conversation can continue there, and queue a
provider-facing access-review link back to the mirrored channel thread. The
review issue contains the human-readable requester, scope, requested role, and
reason because it is the review surface; provider user IDs and provider
handles remain hashed. The source receipt remains body-free, reporting only
access/thread/message/requester/provider-user/scope/role/reason hashes,
duplicate status, notification queue metadata, and delivery gates. It does not
call a model, call provider APIs, grant access, mutate allowlists, issue
pairing codes, print raw access ids, print raw provider user identifiers, print
raw thread ids, print raw source or notification message ids, print channel
message bodies, or print raw requester/scope/role/reason in the source
receipt. Duplicates are suppressed first by `access_id` for the GitHub
access-review issue and then by `channel + notify_message_id` for the
provider-facing review link. Changes to this surface require a live E2E that
opens the access-review issue, validates the metadata-only review-link outbox,
...
The same channel-thread issue can answer a platform/adapter status request
without controlling the adapter:

```text
@gitclaw /channels platform telegram --state running --message-id <provider-message-id>
Reason:
operator-only platform note
```

`/channels platform`, `/channels platforms`, `/channels adapter`,
`/channels adapter-status`, `/channels gateway-status`, and
`/channels bridge-status` infer the current channel and thread id from the
issue marker when no explicit route/channel/thread target is provided. They
queue one provider-facing platform-status message back to the mirrored thread
and do not create any durable artifact issue. The provider-visible message can
include the provider name, adapter state claim, GitHub Actions
`workflow_dispatch` runtime, channel-state storage, outbox/delivery path,
provider ingress/outbound contract, and workflow-surface booleans. The source
receipt remains body-free for operator-only fields, reporting only
thread/message/provider/state/reason/home hashes, duplicate notification state,
workflow booleans, and delivery gates. It does not call a model, call provider
APIs, start a gateway, pause or resume adapters, mutate breaker state, change a
home channel, print raw thread ids, print raw source or notification message
ids, print raw reasons, print raw home selectors, or print channel message
bodies in the source receipt. Duplicate notifications are suppressed by
`channel + notify_message_id`. Changes to this surface require a live E2E that
validates the metadata-only platform-status outbox, checks duplicate
suppression, verifies no adapter/control side effects occurred, and then
...
The same channel-thread issue can answer a skills/capabilities status request
without loading full skill bodies into the channel reply:

```text
@gitclaw /channels skills --message-id <provider-message-id>
```

`/channels skills`, `/channels skill-status`, `/channels skills-status`,
`/channels skill-list`, `/channels skills-list`, `/channels capabilities`, and
`/channels capability-status` infer the current channel and thread id from the
issue marker when no explicit route/channel/thread target is provided. They
queue one provider-facing skill-status message back to the mirrored thread and
do not create any durable artifact issue. The provider-visible message can
include compact skill counts, enabled skill names, selected-skill count for the
current turn, frontmatter/description coverage counts, missing-requirement
counts, and the current Actions checkout as the snapshot source. The source
receipt keeps thread ids, source message ids, notification message ids, status
ids, skill names, skill paths, and channel bodies out of band with hashes. It
does not call a model, install skills, update skills, contact registries, run
installer scripts, mutate the repository, call provider APIs, print raw skill
bodies, print raw skill descriptions, print raw thread ids, print raw source or
notification message ids, print raw status ids, or print channel message bodies
in the source receipt. Duplicate notifications are suppressed by
`channel + notify_message_id`. Changes to this surface require a live E2E that
validates the metadata-only skill-status outbox, checks duplicate suppression,
...
The same channel-thread issue can ask for one chat-native skill spotlight
without installing skills or loading full skill bodies:
...
The same channel-thread issue can ask for a safe skill-use map before opening
any reviewed skill issue:

```text
@gitclaw /channels skill-map repo-reader --map-id <stable-map-id> --message-id <provider-message-id> --notify-message-id <stable-outbound-id>
Note: Keep skill changes reviewed.
```

`/channels skill-map`, `/channels skills-map`, `/channels skill-path`,
`/channels skills-path`, `/channels skill-flow`, `/channels skills-flow`,
`/channels skill-runbook`, `/channels skills-runbook`,
`/channels skill-safety`, and `/channels safe-skill` infer the current channel
and thread id from the issue marker when no explicit route/channel/thread
target is provided. They queue one provider-facing sequence card that shows the
safe path through skill status, skill search, skill info, proposal, rehearsal,
and skill-note commands. This is a map, not installation or review creation:
the action does not call a model, load full `SKILL.md` bodies, install skills,
update skills, contact registries, run installers, create skill proposal
issues, create skill rehearsal issues, create skill-note issues, call provider
APIs, mutate workflows, or mutate repository files. The provider card may show
the requested skill, note, and step commands because it is the user-facing map.
The source receipt remains body-free, reporting only hashes, sizes, counts,
duplicate status, notification metadata, skill snapshot metadata, and hard
delivery gates. It does not print raw map ids, raw requested skill names, raw
notes, raw provider thread/message ids, raw step text, raw skill names, raw
skill paths, raw skill descriptions, raw skill bodies, raw channel bodies,
source bodies, issue bodies, comment bodies, prompts, or tool outputs.
Duplicates are suppressed by `channel + notify_message_id`. Changes to this
surface require a live E2E that records the skill-map request, validates the
metadata-only outbox, checks duplicate suppression, proves no skill
installation/update/registry contact/installer run/review issue creation/model/
provider API/workflow mutation/repository mutation happened, and then
...
The same channel-thread issue can ask for a safe skill-source map before
opening any reviewed source-pin proposal:

```text
@gitclaw /channels source-map repo-reader --map-id <stable-map-id> --message-id <provider-message-id> --notify-message-id <stable-outbound-id>
Note: Keep source pins reviewed.
```

`/channels source-map`, `/channels sources-map`, `/channels skill-source-map`,
`/channels skill-sources-map`, `/channels source-pin-map`, `/channels
source-path`, `/channels source-runbook`, `/channels source-safety`, and
`/channels safe-source` infer the current channel and thread id from the issue
marker when no explicit route/channel/thread target is provided. They queue one
provider-facing sequence card that shows compact repo-local source-pin
provenance, trust, install-mode, approval, remote-fetch, hash, and risk
metadata plus the safe path through source list, info, verify, lock,
update-plan, and proposal commands. This is a map, not source activation or
proposal creation: the action does not call a model, contact registries, fetch
remote sources, load raw source refs, load source bodies, load full `SKILL.md`
bodies, install skills, update skills, write source pins, run installers,
install dependencies, create source proposal issues, call provider APIs, mutate
workflows, or mutate repository files. The provider card may show the requested
source name, note, selected source-pin metadata, and step commands because it is
the user-facing map. The source receipt remains body-free, reporting only
hashes, sizes, counts, duplicate status, notification metadata, source-pin
snapshot metadata, and hard delivery gates. It does not print raw map ids, raw
requested source names, raw notes, raw provider thread/message ids, raw step
text, raw source names, raw source paths, raw source refs, raw source bodies,
raw skill paths, raw skill bodies, raw channel bodies, source bodies, issue
bodies, comment bodies, prompts, or tool outputs. Duplicates are suppressed by
`channel + notify_message_id`. Changes to this surface require a live E2E that
records the source-map request, validates the metadata-only outbox, checks
duplicate suppression, proves no registry contact/remote fetch/skill install/
skill update/source-pin write/installer run/dependency install/source proposal
issue/model/provider API/workflow mutation/repository mutation happened, and
...
The same channel-thread issue can inspect one focused tool/capability card
without executing the tool or exposing raw schemas:

```text
@gitclaw /channels tool-info read_file --message-id <provider-message-id> --notify-message-id <stable-outbound-id>
```

`/channels tool-info`, `/channels tools-info`, `/channels tool-describe`,
`/channels describe-tool`, `/channels tool-card`,
`/channels capability-info`, and `/channels capability-describe` infer the
current channel and thread id from the issue marker when no explicit
route/channel/thread target is provided. They look up one deterministic tool
contract by full name or short suffix, queue one provider-facing focused tool
card, and do not create any durable artifact issue. The provider-facing
outbound comment reports tool-info status, requested/normalized tool hashes,
tool counts, matched tool name, source, enabled/disabled/allowlist state,
mode, mutation flag, trigger hash, active-output count, active-output hashes,
output sizes, and validation summary. The source receipt stays stricter: it
records target issue/comment ids, route/thread/message hashes, tool-info id
hash, requested/normalized tool hashes, match counts, matched-name/index
hashes, outbox delivery instructions, and safety gates. It does not call a
model, execute tools, run shell commands, launch MCP servers, activate
toolsets, call provider APIs, mutate repository files, print raw requested tool
names, print raw trigger text, print raw tool inputs, print raw tool outputs,
print raw tool schemas, print channel bodies, print issue/comment bodies, or
print prompts. Duplicates are suppressed by `channel + notify_message_id`.
Changes to this surface require a live E2E that ingests a real channel issue,
queues a tool-info notification, validates metadata-only outbox discovery,
verifies duplicate suppression, and then continues on the same GitHub issue
with a real GitHub Models repo-reader/search follow-up.

The same channel-thread issue can ask for one safe tool/capability spotlight
before choosing what to inspect next:
...
```text
@gitclaw /channels tool-map search_files --map-id <stable-map-id> --message-id <provider-message-id> --notify-message-id <stable-outbound-id>
Note: Keep tool execution reviewed.
```

`/channels tool-map`, `/channels tools-map`, `/channels tool-path`,
`/channels tools-path`, `/channels tool-flow`, `/channels tools-flow`,
`/channels tool-safety`, `/channels tools-safety`, `/channels safe-tool`, and
`/channels tool-runbook` infer the current channel and thread id from the
issue marker when no explicit route/channel/thread target is provided. They
queue one provider-facing sequence card that shows the safe path through tool
status, tool search, tool info, approval-plan, rehearsal, and request-run
commands. This is a map, not execution or review creation: the action does not
call a model, execute tools, run shell commands, launch MCP servers, activate
toolsets, create approval issues, create rehearsal issues, create run-request
issues, call provider APIs, mutate workflows, or mutate repository files. The
provider card may show the requested tool, note, and step commands because it
is the user-facing map. The source receipt remains body-free, reporting only
hashes, sizes, counts, duplicate status, notification metadata, tool snapshot
metadata, and hard delivery gates. It does not print raw map ids, raw requested
tool names, raw notes, raw provider thread/message ids, raw step text, raw tool
schemas, raw tool inputs, raw tool outputs, raw channel bodies, source bodies,
issue bodies, comment bodies, or prompts. Duplicates are suppressed by
`channel + notify_message_id`. Changes to this surface require a live E2E that
records the tool-map request, validates the metadata-only outbox, checks
duplicate suppression, proves no tool execution/review issue creation/model/
provider API/workflow mutation/repository mutation happened, and then
continues on the same channel issue with a normal GitHub Models
repo-reader/search follow-up.
...
The same channel-thread issue can answer a high-authority soul status request
without dumping persona, identity, user, memory, tool, or heartbeat bodies into
the provider channel:

```text
@gitclaw /channels soul-status --message-id <provider-message-id>
```

`/channels soul-status`, `/channels soul-snapshot`, `/channels soul-health`,
`/channels agent-soul`, `/channels identity-soul`, `/channels context-soul`,
and `/channels authority-status` infer the current channel and thread id from
the issue marker when no explicit route/channel/thread target is provided.
They queue one provider-facing soul-status message back to the mirrored thread
and do not create any durable artifact issue. The provider-visible message can
include the repo-local soul snapshot status, snapshot version/hash, loaded and
required entry counts, prompt-visible entry count, memory-note count,
validation state, required-file coverage, and risk state. The source receipt
remains body-free and hash-heavy: it reports only thread/message/status hashes,
soul snapshot hash, validation/risk counts, delivery metadata, and hard
disabled gates. It does not call a model, contact registries, export profiles,
write soul files, call provider APIs, mutate repository files, print raw soul
file paths, print raw soul bodies, print raw identity bodies, print raw user
bodies, print raw memory bodies, print raw tool guidance, print raw heartbeat
bodies, print prompts, print sessions, print backup payloads, print raw thread
ids, print raw source or notification message ids, print raw status ids, or
print channel message bodies in the source receipt. Duplicate notifications
are suppressed by `channel + notify_message_id`. Changes to this surface
require a live E2E that validates the metadata-only soul-status outbox, checks
duplicate suppression, verifies no registry/profile-export/soul-write/model/
repository/provider side effects occurred, and then continues on the channel
issue with a normal GitHub Models repo-reader/search follow-up.

The same channel-thread issue can inspect one high-authority context file
without dumping persona, identity, user, memory, tool, or heartbeat bodies into
the provider channel:

```text
@gitclaw /channels soul-info SOUL --message-id <provider-message-id> --notify-message-id <stable-outbound-id>
```

`/channels soul-info`, `/channels souls-info`, `/channels context-info`,
`/channels authority-info`, `/channels identity-info`,
`/channels policy-info`, `/channels memory-info`, `/channels heartbeat-info`,
`/channels soul-card`, `/channels authority-card`, and
`/channels context-card` infer the current channel and thread id from the
issue marker when no explicit route/channel/thread target is provided. They
reuse the same normalization as `gitclaw soul info <path>` for aliases such as
`SOUL`, `identity`, `memory`, `heartbeat`, `latest`, and dated memory-note
paths. The provider-visible message can include soul-info status,
requested/normalized path hashes, validation/risk counts, and one focused
match card with normalized context path, category, source, present, required,
canonical, latest, loaded-for-this-turn, byte/line counts, file hash, and
context-limit flags. The source receipt is stricter: it records only
thread/message/info hashes, requested/normalized path hashes, counts, delivery
metadata, and hard disabled gates. It does not call a model, contact
registries, export profiles, write soul or memory, call provider APIs, mutate
repository files, print raw requested or normalized context paths, print raw
thread/message/info ids, print raw soul bodies, print raw identity bodies,
print raw user bodies, print raw memory bodies, print raw tool guidance, print
raw heartbeat bodies, print prompts, print tool outputs, or print channel
message bodies in the source receipt. Duplicate notifications are suppressed
by `channel + notify_message_id`. Changes to this surface require a live E2E
that validates the metadata-only soul-info outbox, checks duplicate
suppression, verifies no registry/profile-export/soul-write/memory-write/
model/repository/provider side effects occurred, and then continues on the
channel issue with a normal GitHub Models repo-reader/search follow-up.

The same channel-thread issue can ask for one high-authority context spotlight
...
```text
@gitclaw /channels soul-risk --message-id <provider-message-id> --notify-message-id <stable-outbound-id>
```

`/channels soul-risk`, `/channels souls-risk`, `/channels authority-risk`,
`/channels identity-risk`, `/channels policy-risk`,
`/channels context-risk`, `/channels high-authority-risk`,
`/channels risk-soul`, `/channels risk-authority`, and
`/channels risk-context` infer the current channel and thread id from the
issue marker when no explicit route/channel/thread target is provided. They
reuse the same repo-local high-authority risk scan as `gitclaw soul risk` and
the same validation pass as `gitclaw soul validate`. The provider-visible
message can include risk status, validation status, registry/profile
verification state, counts, per-context metadata, file hashes, risk codes,
severities, and line hashes. The source receipt is stricter: it records only
thread/message/risk-id hashes, risk and validation counts, index hashes,
delivery metadata, and hard disabled gates. It does not call a model, contact
registries, export profiles, write soul or memory, call provider APIs, mutate
repository files, print raw risk ids, print raw context paths in the source
receipt, print raw soul bodies, print raw identity bodies, print raw user
bodies, print raw memory bodies, print raw tool guidance, print raw heartbeat
bodies, print prompts, print tool outputs, or print channel message bodies in
the source receipt. Duplicate notifications are suppressed by
`channel + notify_message_id`. Changes to this surface require a live E2E that
validates the metadata-only soul-risk outbox, checks duplicate suppression,
verifies no registry/profile-export/soul-write/memory-write/model/repository/
...
The same channel-thread issue can search high-authority context without
dumping persona, identity, user, memory, tool, or heartbeat bodies into the
provider channel:

```text
@gitclaw /channels soul-search operating boundary --message-id <provider-message-id> --notify-message-id <stable-outbound-id> --max-results 5
```

`/channels soul-search`, `/channels souls-search`, `/channels search-soul`,
`/channels search-souls`, `/channels soul-recall`,
`/channels authority-search`, `/channels identity-search`,
`/channels policy-search`, and `/channels high-authority-search` infer the
current channel and thread id from the issue marker when no explicit
route/channel/thread target is provided. They reuse the same body-free lexical
matcher as `gitclaw soul search <query>` over repo-local high-authority
context documents. The provider-visible message can include search status,
query hash, term count, max result count, files scanned, matched files,
matched lines, returned result count, context paths, categories, line numbers,
scores, matched term counts, file hashes, and line hashes. The source receipt
is stricter: it records only thread/message/search hashes, query hash, counts,
delivery metadata, and hard disabled gates. It does not call a model, contact
registries, export profiles, write soul files, call provider APIs, mutate
repository files, print raw search queries, print raw thread/message/search
ids, print raw soul file paths, print raw soul bodies, print raw identity
bodies, print raw user bodies, print raw memory bodies, print raw tool
guidance, print raw heartbeat bodies, print prompts, print tool outputs, or
print channel message bodies in the source receipt. Duplicate notifications
are suppressed by `channel + notify_message_id`. Changes to this surface
require a live E2E that validates the metadata-only soul-search outbox, checks
duplicate suppression, verifies no registry/profile-export/soul-write/model/
repository/provider side effects occurred, and then continues on the channel
issue with a normal GitHub Models repo-reader/search follow-up.
...
The same channel-thread issue can also turn a mirrored provider message into a
GitHub skill-bundle proposal intake lane:

```text
@gitclaw /channels propose-bundle --bundle-id <stable-bundle-proposal-id> --message-id <provider-message-id>
Name: short bundle name
Purpose:
why these skills should be used together
Skills:
- repo-reader
- self-grill
Instruction:
the reviewed bundle-level task-profile guidance
Policy:
review gates, allowed modes, and risk boundaries
Notes:
human-readable context
...
`/channels propose-bundle`, `/channels bundle-proposal`,
`/channels skill-bundle-proposal`, `/channels propose-skill-bundle`,
`/channels skill-bundle`, and `/channels bundle` infer the current channel and
thread id from the issue marker when no explicit channel/thread target is
provided. They create or reuse one open GitHub issue carrying the
`gitclaw:channel-bundle-proposal` marker, label it with `gitclaw` so normal
conversation can continue there, and queue a provider-facing proposal link
back to the mirrored channel thread. The proposal issue contains the readable
bundle name, purpose, proposed skill list, bundle instruction, policy, and
notes because it is the reviewable place to decide whether the bundle should
become `.gitclaw/skill-bundles/<name>.yaml`, documentation, a prompt pack, a
proactive workflow, or a test fixture. The source receipt remains body-free,
reporting only bundle/thread/message/name/purpose/skill/instruction/policy/note
hashes, skill count, duplicate status, notification queue metadata, and
delivery gates. The provider-facing acknowledgement can show the name, skill
count, and GitHub issue link but not the proposed skills, instruction, policy,
notes, provider IDs, or source message body. It does not call a model, install
skills, enable bundles, write bundle YAML, call provider APIs, mutate the
repository, print raw bundle proposal ids, print raw thread ids, print raw
source or notification message ids, print channel message bodies, or print raw
section text in the source receipt. Duplicates are suppressed first by
`bundle_id` for the GitHub proposal issue and then by
`channel + notify_message_id` for the provider-facing proposal link. Changes
to this surface require a live E2E that records the proposal, validates the
metadata-only proposal-link outbox, checks duplicate suppression, asserts that
no skill installation, bundle activation, bundle YAML write, or repository
mutation happened, and then continues on the proposal issue with a normal
GitHub Models repo-reader/search follow-up.
...
`/channels rehearse-checkpoint`, `/channels checkpoint-rehearse`,
`/channels checkpoint-rehearsal`, `/channels rehearse-rollback`,
`/channels rollback-rehearsal`, `/channels rollback-drill`,
`/channels checkpoint-drill`, `/channels rollback-lab`, and
`/channels checkpoint-lab` infer the current channel and thread id from the
issue marker when no explicit channel/thread target is provided. They create
or reuse one open GitHub issue carrying the existing
`gitclaw:checkpoint-rehearsal-issue` marker, label that issue for normal
GitClaw conversation, and queue a provider-facing rehearsal link back to the
mirrored channel thread. This is not a rollback: the channel action does not
call a model, print raw diffs, print file bodies, run restore, run `git reset`,
run `git clean`, run checkout mutations, call provider APIs, or mutate the
repository. The rehearsal issue stores the stable rehearsal id, readable target
ref, target ref hash, checkpoint status, inspect-only rollback preview
metadata, and dry-run checkpoint/rollback commands; the channel source receipt
remains body-free, reporting only rehearsal/thread/message/target hashes,
duplicate status, notification queue metadata, and disabled mutation gates. It
does not print raw rehearsal ids, raw target refs, raw provider thread/message
ids, raw channel message bodies, source bodies, raw diffs, or file bodies in
the source receipt. Duplicates are suppressed first by rehearsal id for the
GitHub checkpoint rehearsal issue and then by `channel + notify_message_id` for
the provider-facing rehearsal link. Changes to this surface require a live E2E
that records the checkpoint rehearsal, validates the metadata-only rehearsal
outbox, checks duplicate suppression, runs local checkpoint status/preview/risk
and rollback diff commands, and then continues on the rehearsal issue with a
normal GitHub Models repo-reader/search follow-up.
...
- create or reuse one `gitclaw:backup-restore-request-issue` issue per channel
  ...
  repository,
- create or reuse one `gitclaw:checkpoint-rehearsal-issue` issue per channel
  rehearsal id and queue one provider-facing checkpoint-rehearsal outbound
  comment per `channel + notify_message_id` without executing a model, printing
  raw diffs, printing file bodies, running restore/reset/clean/checkout, or
...
@gitclaw /channels propose-prompt --prompt-id channel-prompt-proposal-1 --message-id provider-msg-1
...
@gitclaw /channels cockpit research --cockpit-id channel-cockpit-1 --message-id provider-msg-1 --notify-message-id provider-cockpit-ack-1
...
@gitclaw /channels backup-spotlight deployment --spotlight-id channel-backup-spotlight-1 --message-id provider-msg-1 --notify-message-id provider-backup-spotlight-ack-1
...
@gitclaw /channels soul-info SOUL --message-id provider-msg-1 --notify-message-id provider-soul-info-ack-1
@gitclaw /channels soul-spotlight SOUL --spotlight-id channel-soul-spotlight-1 --message-id provider-msg-1 --notify-message-id provider-soul-spotlight-ack-1
@gitclaw /channels soul-risk --message-id provider-msg-1 --notify-message-id provider-soul-risk-ack-1
@gitclaw /channels soul-search operating --message-id provider-msg-1 --notify-message-id provider-soul-search-ack-1
...
- the channel backup-spotlight harness verifies a real channel-ingested issue
  is first present on the fetched `gitclaw-backups` branch, then a
  `/channels backup-spotlight` turn queues one deterministic body-free archive
  candidate through channel outbox, suppresses duplicate notifications, keeps
  raw backup payloads and paths out of the source receipt, and proves a normal
...
- A `gh`-driven channel-quick-replies-slash E2E harness ingests a real mirrored
  channel issue, replies with `@gitclaw /channels quick-replies ...`, verifies
  the provider-facing reply-chip card, body-free source receipt metadata,
  duplicate reply-chip notification suppression from a later issue comment with
  the same acknowledgement id, explicit no command/artifact/task/reminder/model/
  provider-API/workflow/skill/tool/repository mutation gates, and metadata-only
  outbox discovery for the acknowledgement. The same channel issue then gets a
  normal GitHub Models issue-comment follow-up that must select `repo-reader`,
  expose `gitclaw.search_files`, recover the channel-quick-replies fixture token,
  and avoid hidden channel/message/reply/lane/note sentinels.
- A `gh`-driven channel-status-wheel-slash E2E harness ingests a real mirrored
  channel issue, replies with `@gitclaw /channels status-wheel ...`, verifies
  the provider-facing deterministic status spin, body-free source receipt
  metadata, duplicate status-wheel notification suppression from a later issue
  comment with the same acknowledgement id, explicit no model/randomness/
  command/artifact/task/reminder/provider-API/workflow/status/skill/tool/
...
- A `gh`-driven channel-toast-slash E2E harness ingests a real mirrored channel
  issue, replies with `@gitclaw /channels toast ...`, verifies the
  provider-facing toast card, body-free source receipt metadata, duplicate
  toast notification suppression from a later issue comment with the same
  acknowledgement id, explicit no kudos issue/model/provider-API/workflow/
  repository mutation gates, and metadata-only outbox discovery for the
  acknowledgement. The same channel issue then gets a normal GitHub Models
  issue-comment follow-up that must select `repo-reader`, expose
...
- A `gh`-driven channel-timer-slash E2E harness ingests a real mirrored channel
  ...
  timer notification suppression from a later issue comment with the same
  acknowledgement id, explicit no reminder issue/scheduled workflow/provider
  timer/model/provider-API/workflow/repository mutation gates, and
  metadata-only outbox discovery for the acknowledgement. The same channel
  issue then gets a normal GitHub Models issue-comment follow-up that must
  select `repo-reader`, expose `gitclaw.search_files`, recover the
  channel-timer fixture token, and avoid hidden channel/message/timer
  sentinels.
...
- A `gh`-driven channel-cockpit-slash E2E harness ingests a real mirrored
  channel issue, replies with `@gitclaw /channels cockpit research ...`,
  verifies the provider-facing bounded status-board card, body-free source
  receipt metadata, duplicate cockpit notification suppression from a later
  issue comment with the same acknowledgement id, explicit no dynamic-cockpit-
  generation/external-randomness/command-execution/skill-install/tool-
  execution/backup-payload-read/soul-body-read/memory-write/source-fetch/live-
  browse/provider-API/model/workflow/policy/schedule/repository mutation gates,
  and metadata-only outbox discovery for the acknowledgement. The same channel
  issue then gets a normal GitHub Models issue-comment follow-up that must
  select `repo-reader`, expose `gitclaw.search_files`, recover the
  channel-cockpit fixture token, and avoid hidden channel/message/cockpit/lane/
  note/gauge/command sentinels.
...
- A `gh`-driven channel-compass-slash E2E harness ingests a real mirrored
  channel issue, replies with `@gitclaw /channels compass ...`, verifies the
  provider-facing focus/safe-step/note card, body-free source receipt metadata,
  duplicate compass notification suppression from a later issue comment with
  the same acknowledgement id, explicit no command-execution/skill-install/
  tool-execution/backup-payload-read/soul-body-read/provider-API/model/
  repository mutation gates, and metadata-only outbox discovery for the
  acknowledgement. The same channel issue then gets a normal GitHub Models
...
- A `gh`-driven channel-link-slash E2E harness creates a real channel-thread
  issue through `gitclaw-channel-ingest.yml`, posts `@gitclaw /channels link
  --link-id ... --url ... --message-id ...` on that mirrored thread, verifies
  GitHub link-card issue creation, body-free source receipt metadata,
  provider-facing link-card queueing, duplicate link and notification
  suppression, and metadata-only outbox discovery. The link-card issue then
  gets a normal GitHub Models issue-comment follow-up that must select
...
- A `gh`-driven channel-access-request-slash E2E harness creates a real
  ...
  ...` on that mirrored thread, verifies GitHub access-review issue creation,
  body-free source receipt metadata, provider-facing review-link queueing,
  duplicate access-request and notification suppression, metadata-only outbox
  discovery, and explicit no-grant/no-allowlist/no-pairing-code flags. The
  access-review issue then gets a normal GitHub Models issue-comment follow-up
  that must select `repo-reader`, expose `gitclaw.search_files`, recover the
  channel-access fixture token, and avoid hidden channel, account, provider,
  message, user, handle, reason, and access sentinels.
- A `gh`-driven channel-platform-slash E2E harness creates a real
  channel-thread issue through `gitclaw-channel-ingest.yml`, posts
  `@gitclaw /channels platform ... --state ... --message-id ...` on that
  mirrored thread, verifies body-free source receipt metadata, provider-facing
  platform-status queueing, duplicate notification suppression, metadata-only
  outbox discovery, and explicit no-pause/no-resume/no-breaker-mutation/
  no-home-mutation/no-gateway-start/no-provider-API flags. The channel-thread
  issue then gets a normal GitHub Models issue-comment follow-up that must
  select `repo-reader`, expose `gitclaw.search_files`, recover the
...
- A `gh`-driven channel-skill-status-slash E2E harness creates a real
  ...
  verifies body-free source receipt metadata, provider-facing skill-status
  queueing, duplicate notification suppression, metadata-only outbox
  discovery, and explicit no-model-call/no-skill-install/no-skill-update/
  no-registry-contact/no-installer-run/no-repository-mutation/no-provider-API
  flags. The channel-thread issue then gets a normal GitHub Models
  issue-comment follow-up that must select `repo-reader`, expose
...
- A `gh`-driven channel-skill-spotlight-slash E2E harness creates a real
  channel-thread issue through `gitclaw-channel-ingest.yml`, posts
  `@gitclaw /channels skill-spotlight ...` on that mirrored thread, verifies
  one provider-facing deterministic skill spotlight card, source receipt
  metadata without raw focus text, spotlight ids, notes, skill names, paths,
  descriptions, or bodies, duplicate notification suppression, metadata-only
  outbox discovery, deterministic/no-randomness markers, and explicit
  no-model-call/no-tool-execution/no-skill-install/no-skill-update/
  no-registry-contact/no-installer-run/no-repository-mutation/no-provider-API
  flags. The channel-thread issue then gets a normal GitHub Models
  issue-comment follow-up that must select `repo-reader`, expose
...
- A `gh`-driven channel-skill-map-slash E2E harness creates a real
  ...
  provider-facing safe skill sequence card, source receipt metadata without raw
  skill-map ids, requested skill names, notes, step text, skill names, paths,
  descriptions, or bodies, duplicate notification suppression, metadata-only
  outbox discovery, and explicit no-skill-install/no-skill-update/no-registry-
  contact/no-installer-run/no-proposal-issue/no-rehearsal-issue/no-skill-note-
  issue/no-model-call/no-provider-API/no-workflow-mutation/no-repository-
  mutation flags. The channel-thread issue then gets a normal GitHub Models
  issue-comment follow-up that must select `repo-reader`, expose
...
- A `gh`-driven channel-source-map-slash E2E harness creates a real
  channel-thread issue through `gitclaw-channel-ingest.yml`, posts
  `@gitclaw /channels source-map ...` on that mirrored thread, verifies one
  provider-facing safe skill-source sequence card, source receipt metadata
  without raw source-map ids, requested source names, source refs, notes, step
  text, source names, paths, bodies, skill paths, or skill bodies, duplicate
  notification suppression, metadata-only outbox discovery, and explicit
  no-registry-contact/no-remote-fetch/no-skill-install/no-skill-update/no-
  source-pin-write/no-installer-run/no-dependency-install/no-source-proposal-
  issue/no-model-call/no-provider-API/no-workflow-mutation/no-repository-
  mutation flags. The channel-thread issue then gets a normal GitHub Models
  issue-comment follow-up that must select `repo-reader`, expose
  `gitclaw.search_files`, recover the channel-source-map fixture token, and
  avoid hidden channel, account, message, map, note, source-ref, source-body,
  skill-body, and notification sentinels.
...
- A `gh`-driven channel-tool-info-slash E2E harness creates a real
  channel-thread issue through `gitclaw-channel-ingest.yml`, posts
  `@gitclaw /channels tool-info ...` on that mirrored thread, verifies one
  provider-facing focused tool card, source receipt metadata without raw tool
  names, triggers, inputs, outputs, schemas, or raw requested tool text,
  duplicate notification suppression, metadata-only outbox discovery, and
  ...
  channel-thread issue then gets a normal GitHub Models issue-comment follow-up
  that must select `repo-reader`, expose `gitclaw.search_files`, recover the
  channel-tool-info fixture token, and avoid hidden channel, account, message,
  info, tool-input/output, and notification sentinels.
- A `gh`-driven channel-tool-spotlight-slash E2E harness creates a real
  channel-thread issue through `gitclaw-channel-ingest.yml`, posts
  `@gitclaw /channels tool-spotlight ...` on that mirrored thread, verifies one
  provider-facing deterministic safe tool spotlight card, source receipt
  metadata without raw focus text, notes, spotlight ids, tool names, triggers,
  inputs, outputs, or schemas, duplicate notification suppression,
...
- A `gh`-driven channel-tool-map-slash E2E harness creates a real
  ...
  provider-facing safe tool sequence card, source receipt metadata without raw
  tool-map ids, requested tool names, notes, step text, schemas, inputs, or
  outputs, duplicate notification suppression, metadata-only outbox discovery,
  and explicit no-tool-execution/no-shell-execution/no-MCP-launch/no-toolset-
  activation/no-approval-issue/no-rehearsal-issue/no-run-request-issue/no-
  model-call/no-provider-API/no-workflow-mutation/no-repository-mutation
  flags. The channel-thread issue then gets a normal GitHub Models
  issue-comment follow-up that must select `repo-reader`, expose
  `gitclaw.search_files`, recover the channel-tool-map fixture token, and
  avoid hidden channel, account, message, map, note, tool-input/output, and
  notification sentinels.
...
- A `gh`-driven channel-soul-status-slash E2E harness creates a real
  channel-thread issue through `gitclaw-channel-ingest.yml`, posts
  `@gitclaw /channels soul-status --message-id ...` on that mirrored thread,
  verifies body-free source receipt metadata, provider-facing soul-status
  queueing, duplicate notification suppression, metadata-only outbox
  discovery, and explicit no-registry-contact/no-profile-export/no-soul-write/
  no-model-call/no-repository-mutation/no-provider-API flags. The
  channel-thread issue then gets a normal GitHub Models issue-comment
  follow-up that must select `repo-reader`, expose `gitclaw.search_files`,
  recover the channel-soul status fixture token, and avoid hidden channel,
  ...
  sentinels.
- A `gh`-driven channel-soul-info-slash E2E harness creates a real
  channel-thread issue through `gitclaw-channel-ingest.yml`, posts
  `@gitclaw /channels soul-info ...` on that mirrored thread, verifies one
  provider-facing high-authority context metadata card, source receipt
  metadata without raw context paths, raw ids, soul bodies, identity bodies,
  user bodies, memory bodies, tool guidance, or heartbeat bodies, duplicate
  notification suppression, metadata-only outbox discovery, and explicit
  no-registry-contact/no-profile-export/no-soul-write/no-memory-write/
  no-model-call/no-repository-mutation/no-provider-API flags. The
  channel-thread issue then gets a normal GitHub Models issue-comment
  follow-up that must select `repo-reader`, expose `gitclaw.search_files`,
  recover the channel-soul-info fixture token, and avoid hidden channel,
  account, message, info, soul-body, identity-body, user-body, memory-body,
  tool-guidance, heartbeat-body, prompt, session, backup, and notification
  sentinels.
- A `gh`-driven channel-soul-spotlight-slash E2E harness creates a real
  channel-thread issue through `gitclaw-channel-ingest.yml`, posts
  `@gitclaw /channels soul-spotlight ...` on that mirrored thread, verifies
  one provider-facing deterministic high-authority context spotlight card,
  source receipt metadata without raw focus text, notes, spotlight ids,
  selected context paths, soul bodies, identity bodies, user bodies, memory
  bodies, tool guidance, or heartbeat bodies, duplicate notification
  suppression, metadata-only outbox discovery, deterministic/no-randomness
  markers, and explicit no-registry-contact/no-profile-export/no-soul-write/
  no-memory-write/no-tool-execution/no-model-call/no-repository-mutation/
  no-provider-API flags. The channel-thread issue then gets a normal GitHub
  Models issue-comment follow-up that must select `repo-reader`, expose
  `gitclaw.search_files`, recover the channel-soul-spotlight fixture token,
...
- A `gh`-driven channel-soul-risk-slash E2E harness creates a real
  ...
  provider-facing high-authority persistent-state risk card, source receipt
  metadata without raw context paths, raw ids, soul bodies, identity bodies,
  user bodies, memory bodies, tool guidance, or heartbeat bodies, duplicate
  notification suppression, metadata-only outbox discovery, and explicit
  no-registry-contact/no-profile-export/no-soul-write/no-memory-write/
  ...
  channel-thread issue then gets a normal GitHub Models issue-comment
  follow-up that must select `repo-reader`, expose `gitclaw.search_files`,
  recover the channel-soul-risk fixture token, and avoid hidden channel,
  account, message, risk, soul-body, identity-body, user-body, memory-body,
  tool-guidance, heartbeat-body, prompt, session, backup, and notification
  sentinels.
- A `gh`-driven channel-soul-search-slash E2E harness creates a real
  channel-thread issue through `gitclaw-channel-ingest.yml`, posts
  `@gitclaw /channels soul-search ...` on that mirrored thread, verifies
  provider-facing high-authority context recall matches, source receipt
  metadata without raw queries, context paths, raw ids, soul bodies, identity
  bodies, user bodies, memory bodies, tool guidance, or heartbeat bodies,
  duplicate notification suppression, metadata-only outbox discovery, and
  explicit no-registry-contact/no-profile-export/no-soul-write/no-model-call/
  no-repository-mutation/no-provider-API flags. The channel-thread issue then
  gets a normal GitHub Models issue-comment follow-up that must select
  `repo-reader`, expose `gitclaw.search_files`, recover the
  channel-soul-search fixture token, and avoid hidden channel, account,
  message, search, soul-body, identity-body, user-body, memory-body,
  tool-guidance, heartbeat-body, prompt, session, backup, and notification
  sentinels.
...
- A `gh`-driven channel-prompt-proposal-slash E2E harness creates a real
  ...
  sentinels.
- A `gh`-driven channel-bundle-proposal-slash E2E harness creates a real
  channel-thread issue through `gitclaw-channel-ingest.yml`, posts
  `@gitclaw /channels propose-bundle --bundle-id ... --message-id ...` on
  that mirrored thread, verifies GitHub skill-bundle proposal issue creation,
  body-free source receipt metadata, provider-facing proposal-link queueing,
  duplicate proposal and notification suppression, explicit no-skill-
  installation/no-bundle-activation/no-bundle-yaml-write/no-repository-
  mutation gates, and metadata-only outbox discovery. The proposal issue then
  gets a normal GitHub Models issue-comment follow-up that must select
  `repo-reader`, expose `gitclaw.search_files`, recover the channel-bundle-
  proposal fixture token, and avoid hidden channel, account, provider, message,
  bundle id, skill, instruction, policy, and notes sentinels.
...
- A `gh`-driven channel-recovery-map-slash E2E harness creates a real
  ...
  source-body, and backup sentinels.
- A `gh`-driven channel-backup-spotlight-slash E2E harness creates a real
  ...
  `@gitclaw /channels backup-spotlight ... --spotlight-id ... --message-id ...
  --notify-message-id ...` on that mirrored thread, verifies one
  provider-facing archive spotlight card, source receipt metadata without raw
  backup paths, raw spotlight ids, focus text, titles, bodies, or transcript
  text, duplicate notification suppression, metadata-only outbox discovery,
  and explicit no-model-call/no-tool-execution/no-backup-branch-write/
  no-restore/no-GitHub-API-replay/no-repository-mutation/no-provider-API flags.
  The channel-thread issue then gets a normal GitHub Models issue-comment
  follow-up that must select `repo-reader`, expose `gitclaw.search_files`,
  recover the channel-backup-spotlight fixture token, and avoid hidden channel,
  account, message, spotlight, backup path, source-body, and archive sentinels.
...
- A `gh`-driven channel-checkpoint-rehearsal-slash E2E harness creates a real
  ...
  --message-id ...` on that mirrored thread, verifies GitHub checkpoint
  rehearsal issue creation, body-free source receipt metadata, provider-facing
  rehearsal-link queueing, duplicate rehearsal and notification suppression,
  inspect-only checkpoint status/preview/risk and rollback diff reports, and
  metadata-only outbox discovery. The rehearsal issue then gets a normal GitHub
  Models issue-comment follow-up that must select `repo-reader`, expose
  `gitclaw.search_files`, recover the channel-checkpoint-rehearsal fixture
  token, and avoid hidden channel, account, provider, message, rehearsal,
  target-ref, source-body, raw-diff, and file-body sentinels.
