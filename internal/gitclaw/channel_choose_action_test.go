...
func TestHandleChannelThisOrThatQueuesTwoOptionPromptWithoutLLM(t *testing.T) {
	root := t.TempDir()
	writeTestFile(t, root, "README.md", "fixture repo\n")

	threadBody := RenderChannelThreadBody(ChannelIngestOptions{
		Channel:  "telegram",
		ThreadID: "chat-wyr-123",
	})
	ev, err := ParseEvent("issue_comment", []byte(`{
		"action": "created",
		"repository": {"full_name": "owner/repo", "default_branch": "main"},
		"issue": {
			"number": 895,
			"title": "GitClaw telegram thread chat-wyr-123",
			"body": "<!-- gitclaw:channel-thread channel=\"telegram\" thread_id=\"chat-wyr-123\" -->\nGitClaw channel bridge thread.",
			"author_association": "MEMBER",
			"user": {"login": "alice", "type": "User"},
			"labels": [{"name": "gitclaw"}, {"name": "gitclaw:channel"}]
		},
		"comment": {
			"id": 89501,
			"body": "@gitclaw /channels this-or-that --message-id wyr-inbound-895 --notify-message-id wyr-notify-895 --wyr-id wyr-secret-895 --this postcard-scene --that story-dice\nQuestion: Which tiny channel ritual should we try?\nDo not include this command hidden token in the receipt: CHANNEL_WYR_COMMAND_MARKER.",
			"author_association": "MEMBER",
			"user": {"login": "alice", "type": "User"}
		},
		"sender": {"login": "alice", "type": "User"}
	...
	github := &FakeGitHub{
		...
		CommentsByIssue: map[int][]Comment{895: {{
			ID: 89500,
			Body: RenderChannelMessageComment(ChannelIngestOptions{
				Channel:   "telegram",
				ThreadID:  "chat-wyr-123",
				MessageID: "wyr-inbound-895",
				Author:    "telegram",
				Body:      "Original mirrored this-or-that command with CHANNEL_WYR_INGEST_MARKER.",
			}),
		}}},
		IssueLabels: map[int][]string{895: []string{"gitclaw", "gitclaw:channel"}},
	}
	llm := &FakeLLM{Response: "should not be called"}
	...
	if llm.Calls != 0 {
		t.Fatalf("LLM called %d times for channel this-or-that action", llm.Calls)
	}
	if len(github.Issues) != 1 {
		t.Fatalf("this-or-that action should not create artifact issues: %#v", github.Issues)
	}
	...
	outcome, err := BuildChannelChooseOutcome(ChannelChooseOptions{
		...
		Channel:         "telegram",
		ThreadID:        "chat-wyr-123",
		SourceMessageID: "wyr-inbound-895",
		NotifyMessageID: "wyr-notify-895",
		ChooseID:        "wyr-secret-895",
		Mode:            "this-or-that",
		Question:        "Which tiny channel ritual should we try?",
		Choices:         []string{"postcard-scene", "story-dice"},
	})
	if err != nil {
		t.Fatalf("BuildChannelChooseOutcome returned error: %v", err)
	}
	if outcome.SelectionMode != "deterministic-channel-this-or-that" || len(outcome.Choices) != 2 || outcome.Choice == "" {
		t.Fatalf("unexpected this-or-that outcome: %#v", outcome)
	}

	sourceComments := github.CommentsByIssue[895]
	if len(sourceComments) != 3 {
		t.Fatalf("source comments = %d, want message + outbound + receipt: %#v", len(sourceComments), sourceComments)
	}
	outbound := sourceComments[1].Body
	for _, want := range []string{
		"gitclaw:channel-outbound",
		`channel="telegram"`,
		`message_id="wyr-notify-895"`,
		"GitClaw channel this-or-that.",
		"Question: Which tiny channel ritual should we try?",
		"Option A: postcard-scene",
		"Option B: story-dice",
		"Nudge: Pick A or B and add one sentence why.",
		fmt.Sprintf("Deterministic lean: #%d", outcome.ChoiceIndex),
		"Lean hash: ",
		"Pair hash: ",
		"Seed hash: ",
		"Selection source: deterministic GitHub channel action seed.",
		...
		if !strings.Contains(outbound, want) {
			t.Fatalf("this-or-that notification missing %q:\n%s", want, outbound)
		}
	}
	for _, leaked := range []string{"CHANNEL_WYR_INGEST_MARKER", "CHANNEL_WYR_COMMAND_MARKER", "wyr-secret-895"} {
		if strings.Contains(outbound, leaked) {
			t.Fatalf("this-or-that notification leaked %q:\n%s", leaked, outbound)
		}
	}
	...
	for _, want := range []string{
		...
		if !strings.Contains(receipt, want) {
			t.Fatalf("channel this-or-that receipt missing %q:\n%s", want, receipt)
		}
	}
	for _, leaked := range []string{"CHANNEL_WYR_INGEST_MARKER", "CHANNEL_WYR_COMMAND_MARKER", "chat-wyr-123", "wyr-inbound-895", "wyr-notify-895", "wyr-secret-895", "Which tiny channel ritual", "postcard-scene", "story-dice"} {
		if strings.Contains(receipt, leaked) {
			t.Fatalf("channel this-or-that receipt leaked %q:\n%s", leaked, receipt)
		}
	}
	...
	duplicateEv, err := ParseEvent("issue_comment", []byte(`{
		...
		"comment": {
			"id": 89502,
			"body": "@gitclaw /channels wyr --message-id wyr-inbound-895 --notify-message-id wyr-notify-895 --wyr-id wyr-secret-895 --left postcard-scene --right story-dice\nQuestion: Which tiny channel ritual should we try?\nDo not leak duplicate token CHANNEL_WYR_DUPLICATE_MARKER.",
			"author_association": "MEMBER",
			"user": {"login": "alice", "type": "User"}
		},
		"sender": {"login": "alice", "type": "User"}
	...
	if len(github.Issues) != 1 {
		t.Fatalf("duplicate this-or-that created artifact issues: %#v", github.Issues)
	}
	if got := len(github.CommentsByIssue[895]); got != 4 {
		t.Fatalf("duplicate this-or-that posted another outbound comment: comments=%d %#v", got, github.CommentsByIssue[895])
	}
	duplicateReceipt := github.CommentsByIssue[895][3].Body
	for _, want := range []string{
		...
		if !strings.Contains(duplicateReceipt, want) {
			t.Fatalf("duplicate this-or-that receipt missing %q:\n%s", want, duplicateReceipt)
		}
	}
	for _, leaked := range []string{"CHANNEL_WYR_DUPLICATE_MARKER", "chat-wyr-123", "wyr-inbound-895", "wyr-notify-895", "wyr-secret-895", "Which tiny channel ritual", "postcard-scene", "story-dice"} {
		if strings.Contains(duplicateReceipt, leaked) {
			t.Fatalf("duplicate this-or-that receipt leaked %q:\n%s", leaked, duplicateReceipt)
		}
	}
}

func TestBuildChannelThisOrThatRejectsNonPairChoices(t *testing.T) {
	ev := Event{
		Kind:      EventIssueComment,
		EventName: "issue_comment",
		Repo:      "owner/repo",
		Issue: Issue{
			Number: 42,
			Title:  "Channel this-or-that",
			Body:   RenderChannelThreadBody(ChannelIngestOptions{Channel: "telegram", ThreadID: "thread-1"}),
		},
		Comment: &Comment{
			ID:   4201,
			Body: `@gitclaw /channels would-you-rather --message-id source-1 --notify-message-id notify-1 --wyr-id Fun.Pair --option alpha --option beta --option gamma`,
		},
	}
	_, err := BuildChannelChooseActionRequest(ev, DefaultConfig())
	if err == nil || !strings.Contains(err.Error(), "requires exactly 2 choices") {
		t.Fatalf("expected exactly-two choices error, got %v", err)
	}
}
