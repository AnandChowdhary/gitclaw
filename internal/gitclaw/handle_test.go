...
func TestHandleCommandsCommandPostsReportWithoutLLM(t *testing.T) {
	...
	body := github.Posted[0].Body
	for _, want := range []string{"GitClaw Commands Report", "Generated without a model call", "model=\"gitclaw/commands\"", "commands: `15`", "aliases: `7`", "local_cli_helpers: `21`", "/commands", "/backup", "/tools", "`gitclaw commands` command=`/help`", "`gitclaw backup search <query>` command=`/backup`", "`gitclaw backup retention-plan` command=`/backup`", "`gitclaw memory validate` command=`/memory`", "`gitclaw memory search <query>` command=`/memory`", "`gitclaw soul search <query>` command=`/soul`", "`gitclaw skills list` command=`/skills`", "`gitclaw skills info <name>` command=`/skills`", "`gitclaw skills search <query>` command=`/skills`", "`gitclaw tools validate` command=`/tools`", "`gitclaw tools search <query>` command=`/tools`"} {
		if !strings.Contains(body, want) {
