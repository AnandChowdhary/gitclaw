...
`)
	...
	body := github.Posted[0].Body
	for _, want := range []string{"GitClaw Config Report", "Generated without a model call", "model=\"gitclaw/config\"", "config_source: `defaults+repo`", "config_file_path: `.gitclaw/config.yml`", "config_file_present: `true`", "trigger_mode: `label-or-prefix`", "trigger_label: `gitclaw`", "trigger_prefix: `@gitclaw`", "model_fallbacks: `none`", "model_fallbacks_configured: `0`", "run_mode: `read-only`", "max_prompt_bytes: `60000`", "max_output_tokens: `4000`", "skills_allowed_configured: `0`", "skills_disabled_configured: `0`", "tools_allowed_configured: `0`", "tools_disabled_configured: `0`", "workflows_present: `2`", "slash_commands: `35`", "### Tool Gates", "OWNER", "COLLABORATOR", "gitclaw:disabled", "/agents", "/artifacts", "/approvals", "/bundles", "/channels", "/checkpoints", "/diffs", "/doctor", "/help", "/memory", "/models", "/nodes", "/profile", "/tasks", "/runs", "/sandbox", "/security", "/prompt", "/config", "/secrets", "/workspace", ".github/workflows/gitclaw.yml", ".github/workflows/gitclaw-heartbeat.yml", "sha256_12="} {
		if !strings.Contains(body, want) {
...
`)
	writeTestFile(t, root, ".github/workflows/gitclaw-channel-send.yml", `name: GitClaw Channel Send
on:
  workflow_dispatch:
    inputs:
      channel:
...
`)
	...
	body := github.Posted[0].Body
	for _, want := range []string{"GitClaw Channel Report", "Generated without a model call", "model=\"gitclaw/channels\"", "channel_label: `gitclaw:channel`", "trigger_label: `gitclaw`", "workflow_present: `true`", "workflow_dispatch_trigger: `true`", "permissions_actions_write: `true`", "permissions_issues_write: `true`", "workflow_inputs: `5`", "send_workflow_present: `true`", "send_workflow_inputs: `6`", "state_workflow_present: `true`", "gateway_workflow_present: `true`", "gateway_workflow_inputs: `6`", "delivery_workflow_present: `true`", "delivery_workflow_inputs: `6`", "outbox_workflow_present: `true`", "outbox_workflow_inputs: `5`", "channel_thread_issue: `true`", "channel_message_comments_now: `1`", "supported_providers: `telegram, slack, generic`", "wake_strategy: `workflow_dispatch`", "llm_e2e_required_after_channel_report_change: `true`", ".github/workflows/gitclaw-channel-ingest.yml", ".github/workflows/gitclaw-channel-send.yml", ".github/workflows/gitclaw-channel-outbox.yml", "telegram", "slack", "generic", "gitclaw channel-ingest", "gitclaw channel-send", "gitclaw channel-gateway", "gitclaw channel-outbox", "gitclaw channel-delivery", "dispatch id: `<channel>-<message_id>`", "sha256_12="} {
		if !strings.Contains(body, want) {
...
`)
	...
	body := github.Posted[0].Body
	for _, want := range []string{"GitClaw Channel Info Report", "Generated without a model call", "model=\"gitclaw/channels\"", "requested_provider: `telegram`", "channel_info_status: `ok`", "supported_providers: `telegram, slack, generic`", "wake_strategy: `workflow_dispatch`", "state_storage: `gitclaw:channel-state issue`", "gateway_runtime: `GitHub Actions workflow_dispatch`", "raw_bodies_included: `false`", "credential_values_included: `false`", "llm_e2e_required_after_channel_info_change: `true`", "required_secrets: `TELEGRAM_BOT_TOKEN`", "offset_key: `update_id`", "thread_key: `chat_id`", "message_key: `update_id or message_id`", "channel_thread_issue: `true`", "channel_message_comments_now: `1`", "getUpdates polling", "sendMessage then channel-delivery receipt", "`ingest` path=`.github/workflows/gitclaw-channel-ingest.yml` present=`true`", "`send` path=`.github/workflows/gitclaw-channel-send.yml` present=`true`", "`state` path=`.github/workflows/gitclaw-channel-state.yml` present=`true`", "`gateway` path=`.github/workflows/gitclaw-channel-gateway.yml` present=`true`", "`delivery` path=`.github/workflows/gitclaw-channel-delivery.yml` present=`true`", "`outbox` path=`.github/workflows/gitclaw-channel-outbox.yml` present=`true`", "gitclaw channel-ingest --channel telegram", "gitclaw channel-send --channel telegram", "gitclaw channel-gateway --channel telegram", "gitclaw channel-outbox --channel telegram", "gitclaw channel-delivery --channel telegram", "dispatch id: `telegram-<message_id>`"} {
		if !strings.Contains(body, want) {
...
func TestHandleToolsMapCommandPostsReportWithoutLLM(t *testing.T) {
	root := t.TempDir()
	writeTestFile(t, root, ".gitclaw/TOOLS.md", "TOOLS_MAP_HANDLER_SECRET: read-only tools only.")
	writeTestFile(t, root, "docs/search-fixture.md", "tools map unique search fixture phrase => GITCLAW_TOOLS_MAP_CONTEXT_V1\n")
	...
	ev, err := ParseEvent("issues", []byte(`{
		...
		"issue": {
			"number": 120,
			"title": "@gitclaw /tools map search_files",
			"body": "Search for \"tools map unique search fixture phrase\". Hidden tools map body token: TOOLS_MAP_HANDLER_BODY_SECRET.",
			"author_association": "MEMBER",
			"user": {"login": "alice", "type": "User"},
			"labels": [{"name": "gitclaw"}]
		},
	...
	if llm.Calls != 0 {
		t.Fatalf("LLM called %d times for deterministic tools map command", llm.Calls)
	}
	if len(github.Posted) != 1 {
		t.Fatalf("posted %d comments, want 1", len(github.Posted))
	}
	...
	for _, want := range []string{
		...
		"### Safe Tool Sequence",
		"issue=`/tools approval-plan gitclaw.search_files`",
		"issue=`/tools run-plan gitclaw.search_files`",
		...
		if !strings.Contains(body, want) {
			t.Fatalf("tools map report missing %q:\n%s", want, body)
		}
	}
	for _, leaked := range []string{"TOOLS_MAP_HANDLER_SECRET", "TOOLS_MAP_HANDLER_BODY_SECRET", "GITCLAW_TOOLS_MAP_CONTEXT_V1", "tools map unique search fixture phrase"} {
		if strings.Contains(body, leaked) {
			t.Fatalf("tools map report leaked body/input/output token %q:\n%s", leaked, body)
		}
	}
	if !hasLabel(github.IssueLabels[120], "gitclaw:done") || hasLabel(github.IssueLabels[120], "gitclaw:running") || hasLabel(github.IssueLabels[120], "gitclaw:error") {
		t.Fatalf("unexpected final labels: %#v", github.IssueLabels[120])
