...
func TestHandleSessionCatalogCommandPostsReportWithoutLLM(t *testing.T) {
	...
	body := github.Posted[0].Body
	for _, want := range []string{"GitClaw Session Catalog Report", "Generated without a model call", "model=\"gitclaw/session\"", "requested_session_command: `catalog`", "session_catalog_status: `ok`", "catalog_entries: `10`", "issue_side_commands: `10`", "local_backup_commands: `9`", "raw_bodies_included: `false`", "raw_tool_outputs_included: `false`", "session_deletion_allowed: `false`", "session_export_allowed_issue_side: `false`", "llm_e2e_required_after_session_catalog_change: `true`", "command=`provenance` issue_intent=`@gitclaw /session provenance`", "command=`tools` issue_intent=`@gitclaw /session tools`", "command=`skills` issue_intent=`@gitclaw /session skills`", "command=`coverage` issue_intent=`@gitclaw /session coverage`", "issue_thread_gate=`canonical-session-is-github-issue-thread`", "provenance_gate=`assistant-turn-marker-prompt-context`", "tools_gate=`assistant-turn-marker-tool-context`", "skills_gate=`assistant-turn-marker-skill-context`"} {
		if !strings.Contains(body, want) {
