...
func TestHandleBackupCatalogCommandPostsBodyFreeCatalogWithoutLLM(t *testing.T) {
	...
	body := github.Posted[0].Body
	for _, want := range []string{"GitClaw Backup Catalog Report", "Generated without a model call", "model=\"gitclaw/backup\"", "requested_backup_command: `catalog`", "backup_catalog_status: `ok`", "catalog_entries: `17`", "fetched_branch_required_commands: `16`", "requested_local_command: `gitclaw backup catalog --repo owner/repo`", "issue_side_execution: `deferred_to_post_turn_backup_branch`", "raw_bodies_included: `false`", "raw_backup_payloads_included: `false`", "llm_e2e_required_after_backup_catalog_change: `true`", "command=`verify` issue_intent=`@gitclaw /backup verify`", "backup_branch_gate=`fetched-before-local-inspection`"} {
		if !strings.Contains(body, want) {
...
func TestHandleSessionCatalogCommandPostsReportWithoutLLM(t *testing.T) {
	...
	body := github.Posted[0].Body
	for _, want := range []string{"GitClaw Session Catalog Report", "Generated without a model call", "model=\"gitclaw/session\"", "requested_session_command: `catalog`", "session_catalog_status: `ok`", "catalog_entries: `7`", "issue_side_commands: `7`", "local_backup_commands: `6`", "raw_bodies_included: `false`", "raw_tool_outputs_included: `false`", "session_deletion_allowed: `false`", "session_export_allowed_issue_side: `false`", "llm_e2e_required_after_session_catalog_change: `true`", "command=`coverage` issue_intent=`@gitclaw /session coverage`", "issue_thread_gate=`canonical-session-is-github-issue-thread`"} {
		if !strings.Contains(body, want) {
