...
type SoulProposalIssueResult struct {
	...
	Created             bool
	Duplicate           bool
	ChannelNotification SoulProposalChannelNotification
}

type SoulProposalChannelNotification struct {
	Requested           bool
	Routes              int
	Queued              int
	Duplicates          int
...
func BuildSoulProposalIssueRequest(ev Event, cfg Config, repoContext RepoContext) (SoulProposalIssueRequest, error) {
	...
	targetText, proposalID, notifyRoutes, err := parseSoulProposalIssueArgs(fields[2:], sourceText)
	if err != nil {
...
func RunSoulProposalChannelNotification(ctx context.Context, cfg Config, github ChannelSendGitHubClient, req SoulProposalIssueRequest, result SoulProposalIssueResult) (SoulProposalChannelNotification, error) {
	notification := SoulProposalChannelNotification{
	...
	if result.IssueNumber <= 0 {
		return notification, fmt.Errorf("missing soul proposal issue for channel notification")
	}
	body := RenderSoulProposalChannelNotificationBody(req, result)
	messageID := soulProposalChannelNotificationMessageID(req)
...
func parseSoulProposalIssueArgs(args []string, sourceText string) (string, string, []string, error) {
	target := "soul"
	targetSet := false
	proposalID := ""
	...
	for i := 0; i < len(args); i++ {
		...
		default:
			if strings.HasPrefix(field, "--") {
				return "", "", nil, fmt.Errorf("unknown soul proposal argument %q", field)
			}
			if !targetSet {
...
func RenderSoulProposalChannelNotificationBody(req SoulProposalIssueRequest, result SoulProposalIssueResult) string {
	var b strings.Builder
	b.WriteString("GitClaw soul proposal\n\n")
	...
	b.WriteString("\nReview the GitHub proposal issue before drafting a high-authority context diff on a normal code-review branch. This notification did not call a model, generate candidate soul text, write profile files, or mutate the repository.")
	return strings.TrimSpace(b.String())
}

func soulProposalChannelNotificationMessageID(req SoulProposalIssueRequest) string {
	return fmt.Sprintf("gitclaw-soul-proposal-%s", req.ProposalID)
}
