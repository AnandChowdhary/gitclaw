...
SKILL_CLI_PROFILE_CATALOG_SECRET`)
	...
	for _, want := range []string{"GitClaw Profile Catalog Report", "scope: `local-cli`", "Generated without a model call", "profile_catalog_status: `ok`", "catalog_strategy: `compact-repo-local-profile-discovery`", "profile_strategy: `repo-local-git-profile`", "profile_surface: `identity, user, soul, memory, skills, bundles, tools, models, proactive, hooks, channels, backups, sessions`", "provider: `github-models`", "model: `openai/gpt-5-nano`", "catalog_entries: `7`", "profile_layers: `13`", "profile_documents_loaded: `7`", "identity_policy_files: `6`", "memory_notes: `1`", "available_skills: `1`", "available_tools: `5`", "raw_bodies_included: `false`", "raw_profile_payloads_included: `false`", "raw_skill_bodies_included: `false`", "raw_tool_outputs_included: `false`", "profile_mutation_allowed: `false`", "profile_switching_supported: `false`", "profile_export_supported: `false`", "llm_e2e_required_after_profile_catalog_change: `true`", "### Catalog Entries", "command=`catalog` issue_intent=`@gitclaw /profile catalog` local_command=`gitclaw profile catalog`", "command=`snapshot` issue_intent=`@gitclaw /profile snapshot` local_command=`gitclaw profile snapshot`", "command=`manifest` issue_intent=`@gitclaw /profile manifest` local_command=`gitclaw profile manifest`", "### Profile Layers", "layer=`memory` store=`.gitclaw/MEMORY.md + .gitclaw/memory/*.md`", "layer=`proactive` store=`.gitclaw/proactive + .github/workflows`", "### Catalog Gates", "profile_store_gate=`repo-local-reviewed-files`", "session_gate=`github-issue-thread-plus-backup-json`"} {
		if !strings.Contains(output, want) {
