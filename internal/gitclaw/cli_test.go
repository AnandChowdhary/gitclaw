...
func TestSkillsRefreshPlanCommandReportsPerTurnDiscoveryWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/SKILLS/repo-reader/SKILL.md", `---
name: repo-reader
description: Use read-only repository context.
...
SECRET_SKILLS_REFRESH_PLAN_CLI_BODY
`)
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"skills", "refresh-plan"}); err != nil {
			t.Fatalf("skills refresh-plan returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Skill Refresh Plan Report", "scope: `local-cli`", "skill_refresh_plan_status: `needs_review`", "refresh_strategy: `github-actions-per-turn-discovery`", "refresh_trigger: `next-issue-comment-or-workflow-dispatch-run`", "current_snapshot_scope: `current-actions-checkout`", "resident_skill_watcher: `false`", "mid_run_hot_reload_supported: `false`", "session_snapshot_reused_across_runs: `false`", "skill_snapshot_persisted: `false`", "remote_node_refresh_supported: `false`", "remote_registry_polling_allowed: `false`", "workflow_dispatch_refresh_supported: `true`", "model_call_required: `false`", "repository_mutation_allowed: `false`", "skill_install_allowed: `false`", "skill_update_allowed: `false`", "raw_skill_bodies_included: `false`", "raw_issue_bodies_included: `false`", "raw_comment_bodies_included: `false`", "raw_prompt_included: `false`", "available_skills: `1`", "enabled_skills: `1`", "selected_skills: `0`", "skill_hashes: `1`", "skill_index_sha256_12:", "llm_e2e_required_after_skill_refresh_change: `true`", "skill_validation_status: `ok`", "### Refresh Boundary", "kind=`runtime`", "kind=`source`", "kind=`prompt`", "### Current Skill Snapshot", "name=`repo-reader`", "selected_for_this_turn=`false`", "### Refresh Steps", "Start a new issue/comment turn or dispatch the workflow", "code=`per_turn_discovery`", "code=`resident_watcher_disabled`", "code=`live_llm_e2e_required`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("skills refresh-plan output missing %q:\n%s", want, output)
		}
	}
	if strings.Contains(output, "SECRET_SKILLS_REFRESH_PLAN_CLI_BODY") {
		t.Fatalf("skills refresh-plan leaked skill body:\n%s", output)
	}
}
...
func TestSkillsProposalPlanCommandReportsReviewPlan(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/SKILLS/repo-reader/SKILL.md", `---
name: repo-reader
description: Use read-only repository context.
...
SECRET_SKILLS_PROPOSAL_PLAN_CLI_BODY
`)
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"skills", "proposal-plan", "repo-reader"}); err != nil {
			t.Fatalf("skills proposal-plan returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Skill Proposal Plan Report", "scope: `local-cli`", "proposal_plan_status: `needs_review`", "operation: `proposal-plan`", "requested_action: `auto`", "planned_proposal_action: `propose-update`", "target_type: `registry-name`", "safe_name_candidate: `repo-reader`", "proposal_path: `.gitclaw/skill-proposals/repo-reader/PROPOSAL.md`", "destination_path: `.gitclaw/SKILLS/repo-reader/SKILL.md`", "existing_skill_matches: `1`", "review_pr_required: `true`", "proposal_mutation_allowed: `false`", "active_skill_write_allowed: `false`", "autonomous_skill_creation: `false`", "autonomous_skill_improvement: `false`", "llm_e2e_required_after_change: `true`", "raw_proposal_body_included: `false`", "raw_existing_skill_body_included: `false`", "skill_name=`repo-reader`", "code=`existing_skill_update_review`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("skills proposal-plan output missing %q:\n%s", want, output)
		}
	}
	if strings.Contains(output, "SECRET_SKILLS_PROPOSAL_PLAN_CLI_BODY") {
		t.Fatalf("skills proposal-plan leaked skill body:\n%s", output)
	}
}

func TestSkillsProposalsCommandReportsStoreMetadata(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/skill-proposals/repo-reader/PROPOSAL.md", `---
name: repo-reader
status: pending
...
SECRET_SKILLS_PROPOSALS_CLI_BODY
`)
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"skills", "proposals", "risk"}); err != nil {
			t.Fatalf("skills proposals returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Skill Proposals Report", "scope: `local-cli`", "proposal_store_status: `ok`", "proposal_files: `1`", "proposal_status_pending: `1`", "proposal_risk_findings: `0`", "proposal_apply_supported: `false`", "proposal_mutation_allowed: `false`", "active_skill_write_allowed: `false`", "autonomous_skill_creation: `false`", "autonomous_skill_improvement: `false`", "llm_e2e_required_after_proposal_change: `true`", "raw_proposal_bodies_included: `false`", "proposal_name=`repo-reader`", "path=`.gitclaw/skill-proposals/repo-reader/PROPOSAL.md`", "risk_codes=`none`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("skills proposals output missing %q:\n%s", want, output)
		}
	}
	if strings.Contains(output, "SECRET_SKILLS_PROPOSALS_CLI_BODY") || strings.Contains(output, "Improve repo reader") {
		t.Fatalf("skills proposals leaked proposal body:\n%s", output)
	}
}
...
func TestSkillsCatalogCommandReportsEligibilityWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/SKILLS/repo-reader/SKILL.md", `---
name: repo-reader
description: Use read-only repository context.
...
SECRET_SKILLS_CATALOG_CLI_BODY
`)
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"skills", "catalog"}); err != nil {
			t.Fatalf("skills catalog returned error: %v", err)
		}
	})
	for _, want := range []string{
		...
		if !strings.Contains(output, want) {
			t.Fatalf("skills catalog output missing %q:\n%s", want, output)
		}
	}
	for _, leaked := range []string{"SECRET_SKILLS_CATALOG_CLI_BODY", "Use read-only repository context."} {
		if strings.Contains(output, leaked) {
			t.Fatalf("skills catalog leaked %q:\n%s", leaked, output)
		}
	}
}
...
`)
	...
	searchOutput := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"bundles", "search", "workflow"}); err != nil {
			t.Fatalf("bundles search returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Skill Bundle Search Report", "scope: `local-cli`", "bundle_search_status: `ok`", "query_terms: `1`", "available_bundles: `1`", "matched_bundles: `1`", "available_skills: `1`", "raw_bodies_included: `false`", "raw_queries_included: `false`", "llm_e2e_required_after_bundle_search_change: `true`", "bundle_name=`repo-context`", "path=`.gitclaw/skill-bundles/repo-context.yaml`", "match_fields=`description`", "skills=`repo-reader`", "resolved_skills=`repo-reader`", "missing_skills=`none`", "instruction=`true`", "sha256_12=", "query_sha256_12:"} {
		if !strings.Contains(searchOutput, want) {
			t.Fatalf("bundles search output missing %q:\n%s", want, searchOutput)
		}
	}
	for _, leaked := range []string{"SECRET_BUNDLE_CLI_SKILL_BODY", "SECRET_BUNDLE_CLI_INSTRUCTION", "workflow"} {
		if strings.Contains(searchOutput, leaked) {
			t.Fatalf("bundles search leaked %q:\n%s", leaked, searchOutput)
		}
	}

	riskOutput := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"bundles", "risk"}); err != nil {
			t.Fatalf("bundles risk returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Skill Bundle Risk Report", "scope: `local-cli`", "bundle_risk_status: `ok`", "available_bundles: `1`", "scanned_bundles: `1`", "bundle_risk_findings: `0`", "raw_bundle_bodies_included: `false`", "raw_bundle_instructions_included: `false`", "raw_skill_bodies_included: `false`", "bundle_name=`repo-context`", "risk_findings=`0`", "risk_codes=`none`"} {
		if !strings.Contains(riskOutput, want) {
			t.Fatalf("bundles risk output missing %q:\n%s", want, riskOutput)
		}
	}
	for _, leaked := range []string{"SECRET_BUNDLE_CLI_SKILL_BODY", "SECRET_BUNDLE_CLI_INSTRUCTION"} {
		if strings.Contains(riskOutput, leaked) {
			t.Fatalf("bundles risk leaked %q:\n%s", leaked, riskOutput)
		}
	}
}
...
func TestSoulAnchorsCommandReportsAuthorityMapWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/SOUL.md", "---\ndescription: CLI soul.\n---\nSOUL_ANCHORS_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/IDENTITY.md", "IDENTITY_ANCHORS_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/USER.md", "USER_ANCHORS_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/TOOLS.md", "TOOLS_ANCHORS_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/MEMORY.md", "MEMORY_ANCHORS_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/HEARTBEAT.md", "HEARTBEAT_ANCHORS_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/STANDING_ORDERS.md", "STANDING_ORDERS_ANCHORS_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/memory/2026-05-29.md", "DATED_MEMORY_ANCHORS_CLI_BODY_TOKEN")
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"soul", "anchors"}); err != nil {
			t.Fatalf("soul anchors returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Soul Anchors Report", "scope: `local-cli`", "soul_anchors_status: `ok`", "authority_model: `repo-local-workspace-files`", "loaded_anchors: `8`", "required_anchors_loaded: `6`", "required_anchors_missing: `0`", "optional_anchors_loaded: `2`", "memory_note_anchors: `1`", "prompt_visible_anchors: `8`", "raw_bodies_included: `false`", "soul_writes_allowed: `false`", "llm_e2e_required_after_soul_anchors_change: `true`", "soul_validation_status: `ok`", "soul_risk_status: `ok`", "### Authority Anchors", "name=`soul` path=`.gitclaw/SOUL.md`", "role=`persona-boundaries`", "authority=`core`", "frontmatter=`true`", "description=`true`", "name=`memory-note` path=`.gitclaw/memory/2026-05-29.md`", "### Anchor Gates", "validation_gate=`pass`", "risk_gate=`pass`", "mutation_gate=`disabled`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("soul anchors output missing %q:\n%s", want, output)
		}
	}
	for _, leaked := range []string{"SOUL_ANCHORS_CLI_BODY_TOKEN", "USER_ANCHORS_CLI_BODY_TOKEN", "MEMORY_ANCHORS_CLI_BODY_TOKEN", "DATED_MEMORY_ANCHORS_CLI_BODY_TOKEN", "CLI soul"} {
		if strings.Contains(output, leaked) {
			t.Fatalf("soul anchors leaked body token %q:\n%s", leaked, output)
		}
	}
}

func TestSoulSnapshotCommandReportsCompositeFingerprintWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/SOUL.md", "---\ndescription: CLI soul snapshot.\n---\nSOUL_SNAPSHOT_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/IDENTITY.md", "IDENTITY_SNAPSHOT_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/USER.md", "USER_SNAPSHOT_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/TOOLS.md", "TOOLS_SNAPSHOT_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/MEMORY.md", "MEMORY_SNAPSHOT_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/HEARTBEAT.md", "HEARTBEAT_SNAPSHOT_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/STANDING_ORDERS.md", "STANDING_ORDERS_SNAPSHOT_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/memory/2026-05-29.md", "DATED_MEMORY_SNAPSHOT_CLI_BODY_TOKEN")
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"soul", "snapshot"}); err != nil {
			t.Fatalf("soul snapshot returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Soul Snapshot Report", "scope: `local-cli`", "soul_snapshot_status: `ok`", "snapshot_version: `gitclaw-soul-snapshot-v1`", "snapshot_scope: `repo-local-high-authority-context`", "snapshot_sha256_12:", "snapshot_entries: `17`", "loaded_snapshot_entries: `8`", "required_loaded_entries: `6`", "missing_required_entries: `0`", "optional_loaded_entries: `2`", "memory_note_entries: `1`", "prompt_visible_entries: `8`", "registry_contact_allowed: `false`", "profile_export_allowed: `false`", "soul_writes_allowed: `false`", "repository_mutation_allowed: `false`", "raw_bodies_included: `false`", "raw_descriptions_included: `false`", "llm_e2e_required_after_soul_snapshot_change: `true`", "soul_validation_status: `ok`", "soul_risk_status: `ok`", "### Snapshot Entries", "name=`agent-instructions` path=`AGENTS.md`", "load_state=`optional-missing`", "name=`soul` path=`.gitclaw/SOUL.md`", "load_state=`required-loaded`", "name=`memory-note` path=`.gitclaw/memory/2026-05-29.md`", "latest_memory_note=`true`", "### Snapshot Gates", "validation_gate=`pass`", "risk_gate=`pass`", "snapshot_hash_gate=`composite-sha256_12`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("soul snapshot output missing %q:\n%s", want, output)
		}
	}
	for _, leaked := range []string{"SOUL_SNAPSHOT_CLI_BODY_TOKEN", "USER_SNAPSHOT_CLI_BODY_TOKEN", "MEMORY_SNAPSHOT_CLI_BODY_TOKEN", "DATED_MEMORY_SNAPSHOT_CLI_BODY_TOKEN", "CLI soul snapshot"} {
		if strings.Contains(output, leaked) {
			t.Fatalf("soul snapshot leaked body token %q:\n%s", leaked, output)
		}
	}
}

func TestSoulCatalogCommandReportsCompactAuthorityCatalogWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/SOUL.md", "---\ndescription: CLI soul catalog.\n---\nSOUL_CATALOG_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/IDENTITY.md", "IDENTITY_CATALOG_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/USER.md", "USER_CATALOG_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/TOOLS.md", "TOOLS_CATALOG_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/MEMORY.md", "MEMORY_CATALOG_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/HEARTBEAT.md", "HEARTBEAT_CATALOG_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/STANDING_ORDERS.md", "STANDING_ORDERS_CATALOG_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/memory/2026-05-29.md", "DATED_MEMORY_CATALOG_CLI_BODY_TOKEN")
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"soul", "catalog"}); err != nil {
			t.Fatalf("soul catalog returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Soul Catalog Report", "scope: `local-cli`", "soul_catalog_status: `ok`", "catalog_strategy: `compact-authority-discovery`", "cataloged_anchors: `17`", "loaded_anchors: `8`", "required_anchors_loaded: `6`", "optional_anchors_loaded: `2`", "memory_note_anchors: `1`", "authority_layer_names:", "raw_bodies_included: `false`", "raw_descriptions_included: `false`", "profile_export_allowed: `false`", "llm_e2e_required_after_soul_catalog_change: `true`", "### Authority Catalog", "name=`soul` path=`.gitclaw/SOUL.md`", "load_mode=`required-loaded`", "reason_codes=`canonical, loaded, prompt_visible, required`", "name=`memory-note` path=`.gitclaw/memory/2026-05-29.md`", "load_mode=`optional-loaded`", "### Catalog Gates", "validation_gate=`pass`", "risk_gate=`pass`", "body_hash_gate=`sha256_12`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("soul catalog output missing %q:\n%s", want, output)
		}
	}
	for _, leaked := range []string{"SOUL_CATALOG_CLI_BODY_TOKEN", "USER_CATALOG_CLI_BODY_TOKEN", "MEMORY_CATALOG_CLI_BODY_TOKEN", "DATED_MEMORY_CATALOG_CLI_BODY_TOKEN", "CLI soul catalog"} {
		if strings.Contains(output, leaked) {
			t.Fatalf("soul catalog leaked body token %q:\n%s", leaked, output)
		}
	}
}

func TestSoulProvenanceCommandReportsGitHistoryWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/SOUL.md", "---\ndescription: CLI soul.\n---\nSOUL_PROVENANCE_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/IDENTITY.md", "IDENTITY_PROVENANCE_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/USER.md", "USER_PROVENANCE_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/TOOLS.md", "TOOLS_PROVENANCE_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/MEMORY.md", "MEMORY_PROVENANCE_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/HEARTBEAT.md", "HEARTBEAT_PROVENANCE_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/memory/2026-05-29.md", "DATED_MEMORY_PROVENANCE_CLI_BODY_TOKEN")
	...
	runTestGit(t, dir, "-c", "commit.gpgsign=false", "commit", "-m", "Add soul provenance fixture CLI_COMMIT_SUBJECT_TOKEN")
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"soul", "provenance"}); err != nil {
			t.Fatalf("soul provenance returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Soul Provenance Report", "scope: `local-cli`", "soul_provenance_status: `ok`", "provenance_scope: `repo-local-git-history`", "context_documents: `7`", "identity_policy_files: `6`", "memory_notes: `1`", "repo_local_documents: `7`", "git_tracked_documents: `7`", "untracked_documents: `0`", "working_tree_dirty_documents: `0`", "documents_with_commits: `7`", "documents_without_commits: `0`", "git_available: `true`", "git_history_available: `true`", "raw_bodies_included: `false`", "raw_git_subjects_included: `false`", "author_identities_included: `false`", "soul_writes_allowed: `false`", "llm_e2e_required_after_soul_provenance_change: `true`", "soul_validation_status: `ok`", "soul_risk_status: `ok`", "### Provenance Cards", "path=`.gitclaw/SOUL.md` category=`soul`", "git_tracked=`true`", "working_tree_dirty=`false`", "commit_available=`true`", "last_commit_sha256_12=", "last_commit_short=", "last_commit_date=", "subject_sha256_12=", "path=`.gitclaw/memory/2026-05-29.md` category=`memory-note`", "### Provenance Gates", "validation_gate=`pass`", "risk_gate=`pass`", "git_history_gate=`pass`", "mutation_gate=`disabled`", "### Findings", "- none"} {
		if !strings.Contains(output, want) {
			t.Fatalf("soul provenance output missing %q:\n%s", want, output)
		}
	}
	for _, leaked := range []string{"SOUL_PROVENANCE_CLI_BODY_TOKEN", "IDENTITY_PROVENANCE_CLI_BODY_TOKEN", "USER_PROVENANCE_CLI_BODY_TOKEN", "TOOLS_PROVENANCE_CLI_BODY_TOKEN", "MEMORY_PROVENANCE_CLI_BODY_TOKEN", "HEARTBEAT_PROVENANCE_CLI_BODY_TOKEN", "DATED_MEMORY_PROVENANCE_CLI_BODY_TOKEN", "CLI_COMMIT_SUBJECT_TOKEN", "test@example.com", "Test User"} {
		if strings.Contains(output, leaked) {
			t.Fatalf("soul provenance leaked %q:\n%s", leaked, output)
		}
	}
}
...
func TestMemoryCatalogCommandReportsCompactCatalogWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/MEMORY.md", "MEMORY_CATALOG_CLI_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/memory/2026-05-29.md", "DATED_MEMORY_CATALOG_CLI_BODY_TOKEN")
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"memory", "catalog"}); err != nil {
			t.Fatalf("memory catalog returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Memory Catalog Report", "scope: `local-cli`", "memory_catalog_status: `ok`", "catalog_strategy: `compact-durable-memory-discovery`", "catalog_scope: `repo-local-memory-notes-session-search`", "memory_model: `repo-local-reviewed-markdown`", "hermes_memory_layers: `durable-memory, procedural-skills, session-search`", "cataloged_entries: `2`", "long_term_entries: `1`", "dated_note_entries: `1`", "prompt_visible_entries: `2`", "loaded_memory_entries: `2`", "memory_files: `2`", "long_term_memory_loaded: `true`", "dated_memory_notes: `1`", "latest_memory_note: `.gitclaw/memory/2026-05-29.md`", "raw_memory_bodies_included: `false`", "raw_session_bodies_included: `false`", "embedding_vectors_included: `false`", "external_provider_accessed: `false`", "memory_writes_allowed: `false`", "llm_e2e_required_after_memory_catalog_change: `true`", "memory_validation_status: `ok`", "memory_risk_status: `ok`", "### Memory Catalog Entries", "kind=`long-term` path=`.gitclaw/MEMORY.md` memory_layer=`durable-memory`", "role=`stable-summary`", "kind=`dated-note` path=`.gitclaw/memory/2026-05-29.md`", "role=`latest-daily-note`", "load_mode=`prompt-visible`", "reason_codes=", "### Catalog Gates", "validation_gate=`pass`", "risk_gate=`pass`", "memory_write_gate=`disabled`", "external_provider_gate=`not_configured`", "session_search_gate=`github-issues-and-backups`", "background_promotion_gate=`disabled`", "body_hash_gate=`sha256_12`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("memory catalog output missing %q:\n%s", want, output)
		}
	}
	for _, leaked := range []string{"MEMORY_CATALOG_CLI_BODY_TOKEN", "DATED_MEMORY_CATALOG_CLI_BODY_TOKEN"} {
		if strings.Contains(output, leaked) {
			t.Fatalf("memory catalog leaked body token %q:\n%s", leaked, output)
		}
	}
}

func TestMemoryTimelineCommandReportsChronologyWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/MEMORY.md", "MEMORY_TIMELINE_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/memory/2026-05-27.md", "OLDER_MEMORY_TIMELINE_BODY_TOKEN")
	writeTestFile(t, dir, ".gitclaw/memory/2026-05-29.md", "LATEST_MEMORY_TIMELINE_BODY_TOKEN")
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"memory", "timeline"}); err != nil {
			t.Fatalf("memory timeline returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Memory Timeline Report", "scope: `local-cli`", "memory_timeline_status: `ok`", "memory_mode: `read-only`", "authority_model: `repo-local-reviewed-markdown`", "memory_files: `3`", "long_term_memory_present: `true`", "long_term_memory_loaded: `true`", "dated_memory_notes: `2`", "canonical_dated_memory_notes: `2`", "noncanonical_dated_memory_notes: `0`", "loaded_memory_notes: `2`", "omitted_memory_notes: `0`", "first_memory_note: `.gitclaw/memory/2026-05-27.md`", "latest_memory_note: `.gitclaw/memory/2026-05-29.md`", "timeline_entries: `3`", "timeline_span_days: `2`", "largest_gap_days: `2`", "gaps_over_one_day: `1`", "raw_bodies_included: `false`", "memory_writes_allowed: `false`", "llm_e2e_required_after_memory_timeline_change: `true`", "memory_validation_status: `ok`", "memory_risk_status: `ok`", "### Timeline Entries", "position=`1` kind=`long-term` path=`.gitclaw/MEMORY.md`", "date=`long-term`", "position=`2` kind=`dated-note` path=`.gitclaw/memory/2026-05-27.md`", "gap_days_since_previous_note=`first`", "position=`3` kind=`dated-note` path=`.gitclaw/memory/2026-05-29.md`", "gap_days_since_previous_note=`2`", "risk_codes=`none`", "### Timeline Gates", "validation_gate=`pass`", "risk_gate=`pass`", "mutation_gate=`disabled`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("memory timeline output missing %q:\n%s", want, output)
		}
	}
	for _, leaked := range []string{"MEMORY_TIMELINE_BODY_TOKEN", "OLDER_MEMORY_TIMELINE_BODY_TOKEN", "LATEST_MEMORY_TIMELINE_BODY_TOKEN"} {
		if strings.Contains(output, leaked) {
			t.Fatalf("memory timeline leaked body token %q:\n%s", leaked, output)
		}
	}
}
...
func TestToolsCatalogCommandReportsCompactProgressiveDisclosureWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/TOOLS.md", "TOOLS_CATALOG_BODY_TOKEN")
	writeTestFile(t, dir, "go.mod", "module github.com/AnandChowdhary/gitclaw\n")
	writeTestFile(t, dir, "docs/search-fixture.md", "tools catalog unique search fixture phrase => GITCLAW_TOOLS_CATALOG_CONTEXT_V1\n")
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"tools", "catalog"}); err != nil {
			t.Fatalf("tools catalog returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Tools Catalog Report", "scope: `local-cli`", "tool_catalog_status: `ok`", "catalog_strategy: `compact-progressive-disclosure`", "catalog_scope: `deterministic-tools-toolsets-mcp`", "cataloged_entries: `5`", "direct_core_entries: `5`", "enabled_core_entries: `5`", "deferrable_candidate_entries: `0`", "toolset_catalog_entries: `0`", "mcp_catalog_entries: `0`", "planned_direct_entries: `5`", "planned_deferred_entries: `0`", "candidate_bridge_tools: `3`", "planned_bridge_tools: `0`", "activation_decision: `direct`", "activation_reason: `no_deferrable_catalog_entries`", "available_tools: `5`", "enabled_tools: `5`", "active_tool_outputs: `1`", "model_callable_structured_tools: `false`", "tool_search_bridge_runtime_enabled: `false`", "raw_tool_schemas_included: `false`", "raw_toolset_bodies_included: `false`", "raw_mcp_bodies_included: `false`", "raw_inputs_included: `false`", "raw_outputs_included: `false`", "llm_e2e_required_after_tool_catalog_change: `true`", "tool_validation_status: `ok`", "tool_risk_status: `ok`", "### Catalog Entries", "kind=`builtin-contract` name=`gitclaw.list_files`", "catalog_mode=`direct-core`", "schema_visibility=`direct-contract` active_outputs=`1` tool_refs_count=`0`", "kind=`builtin-contract` name=`gitclaw.policy`", "reason_codes=`builtin_contract, direct_core, enabled, no_active_outputs, not_deferrable, planned_direct`", "### Catalog Gates", "validation_gate=`pass`", "risk_gate=`pass`", "activation_gate=`direct`", "tool_search_bridge_gate=`disabled`", "structured_tool_gate=`disabled`", "mcp_runtime_gate=`disabled`", "toolset_activation_gate=`disabled`", "schema_body_gate=`sha256_12`", "mutation_gate=`disabled`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("tools catalog output missing %q:\n%s", want, output)
		}
	}
	for _, leaked := range []string{"TOOLS_CATALOG_BODY_TOKEN", "GITCLAW_TOOLS_CATALOG_CONTEXT_V1", "tools catalog unique search fixture phrase", "module github.com/AnandChowdhary/gitclaw", "go.mod"} {
		if strings.Contains(output, leaked) {
			t.Fatalf("tools catalog leaked %q:\n%s", leaked, output)
		}
	}
}
...
func TestToolsMapCommandReportsSafeSequenceWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/TOOLS.md", "TOOLS_MAP_BODY_TOKEN")
	writeTestFile(t, dir, "docs/search-fixture.md", "tools map unique search fixture phrase => GITCLAW_TOOLS_MAP_CONTEXT_V1\n")
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"tools", "map", "search_files"}); err != nil {
			t.Fatalf("tools map returned error: %v", err)
		}
	})
	for _, want := range []string{
		"GitClaw Tool Map Report",
		"scope: `local-cli`",
		"tool_map_status: `ok`",
		"tool_map_mode: `issue-native-safe-tool-sequence`",
		...
		"run_request_issue_created: `false`",
		...
		"tool_risk_status: `ok`",
		"llm_e2e_required_after_tool_map_change: `true`",
		"### Matched Tool",
		"name=`gitclaw.search_files`",
		"### Safe Tool Sequence",
		"issue=`/tools list`",
		"issue=`/tools search gitclaw.search_files`",
		"issue=`/tools request-run gitclaw.search_files --id <request-id>`",
		"### Safety Gates",
		"gate=`tool_execution` status=`not_performed`",
		"### Findings",
		"code=`openclaw_tool_sequence_mapped`",
		"code=`hermes_issue_native_review_boundary`",
		"code=`read_only_or_metadata_only`",
	} {
		if !strings.Contains(output, want) {
			t.Fatalf("tools map output missing %q:\n%s", want, output)
		}
	}
	for _, leaked := range []string{"TOOLS_MAP_BODY_TOKEN", "GITCLAW_TOOLS_MAP_CONTEXT_V1", "tools map unique search fixture phrase"} {
		if strings.Contains(output, leaked) {
			t.Fatalf("tools map leaked %q:\n%s", leaked, output)
		}
	}
}

func TestToolsReadinessCommandReportsGateChecklistWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/TOOLS.md", "TOOLS_READINESS_BODY_TOKEN")
	writeTestFile(t, dir, "docs/search-fixture.md", "tools readiness unique search fixture phrase => GITCLAW_TOOLS_READINESS_CONTEXT_V1\n")
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"tools", "readiness", "search_files"}); err != nil {
			t.Fatalf("tools readiness returned error: %v", err)
		}
	})
	for _, want := range []string{
		...
		"code=`prompt_visible_read_only_or_metadata_only`",
	} {
		if !strings.Contains(output, want) {
			t.Fatalf("tools readiness output missing %q:\n%s", want, output)
		}
	}
	for _, leaked := range []string{"TOOLS_READINESS_BODY_TOKEN", "GITCLAW_TOOLS_READINESS_CONTEXT_V1", "tools readiness unique search fixture phrase"} {
		if strings.Contains(output, leaked) {
			t.Fatalf("tools readiness leaked %q:\n%s", leaked, output)
		}
	}
}
...
`)
	writeTestFile(t, dir, ".github/workflows/gitclaw-channel-send.yml", `name: GitClaw Channel Send
on:
  workflow_dispatch:
    inputs:
      channel:
...
permissions:
  issues: write
`)
	writeTestFile(t, dir, ".github/workflows/gitclaw-channel-outbox.yml", `name: GitClaw Channel Outbox
on:
  workflow_dispatch:
    inputs:
      channel:
...
`)
	...
	for _, want := range []string{"GitClaw Channel Report", "scope: `local-cli`", "channel_label: `gitclaw:channel`", "trigger_label: `gitclaw`", "workflow_path: `.github/workflows/gitclaw-channel-ingest.yml`", "workflow_present: `true`", "workflow_dispatch_trigger: `true`", "permissions_actions_write: `true`", "permissions_issues_write: `true`", "workflow_inputs: `5`", "send_workflow_present: `true`", "send_workflow_inputs: `6`", "state_workflow_present: `true`", "state_workflow_inputs: `4`", "gateway_workflow_present: `true`", "gateway_workflow_inputs: `6`", "delivery_workflow_present: `true`", "delivery_workflow_inputs: `6`", "outbox_workflow_present: `true`", "outbox_workflow_permissions_issues_read: `true`", "outbox_workflow_inputs: `5`", "supported_providers: `telegram, slack, generic`", "wake_strategy: `workflow_dispatch`", "llm_e2e_required_after_channel_report_change: `true`", "telegram", "slack", "generic", "gitclaw channel-ingest", "gitclaw channel-send", "gitclaw channel-state", "gitclaw channel-gateway", "gitclaw channel-outbox", "gitclaw channel-delivery", "dispatch id: `<channel>-<message_id>`"} {
		if !strings.Contains(output, want) {
...
`)
	...
	for _, want := range []string{"GitClaw Channel Verify Report", "scope: `local-cli`", "channel_verify_status: `ok`", "verification_scope: `workflow_dispatch_channel_bridge`", "workflow_present: `true`", "workflow_dispatch_trigger: `true`", "permissions_actions_write: `true`", "permissions_issues_write: `true`", "workflow_inputs: `5`", "required_workflow_inputs: `5`", "send_workflow_present: `true`", "send_workflow_permissions_issues_write: `true`", "send_workflow_inputs: `6`", "state_workflow_present: `true`", "state_workflow_inputs: `4`", "gateway_workflow_present: `true`", "gateway_workflow_permissions_actions_write: `true`", "gateway_workflow_inputs: `6`", "delivery_workflow_present: `true`", "delivery_workflow_inputs: `6`", "outbox_workflow_present: `true`", "outbox_workflow_permissions_issues_read: `true`", "outbox_workflow_inputs: `5`", "supported_providers: `telegram, slack, generic`", "wake_strategy: `workflow_dispatch`", "llm_e2e_required_after_channel_verify_change: `true`", "raw_bodies_included: `false`", "### Verification Findings", "- none", "workflow has `workflow_dispatch`", "channel-send workflow can queue GitHub-originated outbound messages", "channel state and gateway workflows are callable", "outbox workflow can read pending assistant replies", "delivery workflow records outbound receipts", "dispatch id `<channel>-<message_id>`"} {
		if !strings.Contains(output, want) {
...
`)
	...
	for _, want := range []string{"GitClaw Channel Info Report", "scope: `local-cli`", "Generated without a model call", "requested_provider: `telegram`", "channel_info_status: `ok`", "supported_providers: `telegram, slack, generic`", "wake_strategy: `workflow_dispatch`", "state_storage: `gitclaw:channel-state issue`", "gateway_runtime: `GitHub Actions workflow_dispatch`", "raw_bodies_included: `false`", "credential_values_included: `false`", "llm_e2e_required_after_channel_info_change: `true`", "required_secrets: `TELEGRAM_BOT_TOKEN`", "offset_key: `update_id`", "thread_key: `chat_id`", "message_key: `update_id or message_id`", "### Provider Contract", "getUpdates polling", "sendMessage then channel-delivery receipt", "required_secret_names=`TELEGRAM_BOT_TOKEN`", "### Workflow Surface", "`ingest` path=`.github/workflows/gitclaw-channel-ingest.yml` present=`true`", "`send` path=`.github/workflows/gitclaw-channel-send.yml` present=`true`", "`state` path=`.github/workflows/gitclaw-channel-state.yml` present=`true`", "`gateway` path=`.github/workflows/gitclaw-channel-gateway.yml` present=`true`", "`delivery` path=`.github/workflows/gitclaw-channel-delivery.yml` present=`true`", "`outbox` path=`.github/workflows/gitclaw-channel-outbox.yml` present=`true`", "sha256_12=", "### Commands", "gitclaw channel-ingest --channel telegram", "gitclaw channel-send --channel telegram", "gitclaw channel-state --channel telegram", "gitclaw channel-gateway --channel telegram", "gitclaw channel-outbox --channel telegram", "gitclaw channel-delivery --channel telegram", "dispatch id: `telegram-<message_id>`"} {
		if !strings.Contains(output, want) {
...
func TestSecretsRiskCommandReportsBoundaryWithoutValues(t *testing.T) {
	dir := t.TempDir()
	secret := "ghp_abcdefghijklmnopqrstuvwxyz123456"
	writeTestFile(t, dir, ".github/workflows/example.yml", "env:\n  API_TOKEN: ${{ secrets.MY_API_TOKEN }}\n")
	writeTestFile(t, dir, "config.env", "GITHUB_TOKEN="+secret+"\n")
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"secrets", "risk"}); err != nil {
			t.Fatalf("secrets risk returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Secrets Risk Report", "scope: `local-cli`", "Generated without a model call", "secrets_risk_status: `high_risk`", "verification_scope: `repo_secret_exposure`", "plaintext_secret_findings: `1`", "known_token_findings: `1`", "plaintext_assignment_findings: `0`", "github_actions_secret_references: `1`", "raw_values_included: `false`", "raw_lines_included: `false`", "environment_values_loaded: `false`", "github_secret_values_resolved: `false`", "secret_configure_apply_supported: `false`", "secret_reload_supported: `false`", "llm_e2e_required_after_secrets_risk_change: `true`", "kind=`plaintext-residue` status=`high_risk`", "kind=`secret-reference` status=`review`", "code=`github_token`", "syntax=`github-actions`", "value_sha256_12=", "name_sha256_12="} {
		if !strings.Contains(output, want) {
			t.Fatalf("secrets risk output missing %q:\n%s", want, output)
		}
	}
	for _, notWant := range []string{secret, "GITHUB_TOKEN=", "MY_API_TOKEN", "repository:", "issue:"} {
		if strings.Contains(output, notWant) {
			t.Fatalf("secrets risk output unexpectedly included %q:\n%s", notWant, output)
		}
	}
}
...
func TestApprovalsRiskCommandReportsStaticBoundary(t *testing.T) {
	t.Setenv("GITCLAW_WORKDIR", t.TempDir())
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"approvals", "risk"}); err != nil {
			t.Fatalf("approvals risk returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Approvals Risk Report", "scope: `local-cli`", "Generated without a model call", "approval_risk_status: `ok`", "verification_scope: `approval-gates-labels-and-read-only-boundary`", "approval_store: `github-issue-labels`", "approval_scope: `per-issue`", "trusted_associations: `3`", "broad_trusted_associations: `0`", "approval_labels_configured: `3`", "duplicate_approval_labels: `0`", "approval_managed_label_collisions: `0`", "managed_labels_configured: `9`", "approval_risk_findings: `0`", "write_actions_supported: `false`", "write_actions_enabled: `false`", "repository_mutation_allowed: `false`", "host_exec_allowed: `false`", "approval_payloads_included: `false`", "raw_bodies_included: `false`", "llm_e2e_required_after_approval_risk_change: `true`", "### Approval Gate Risk Card", "kind=`approval-gates`", "### Trusted Association Risk Cards", "association=`OWNER` trusted=`true` broad=`false`", "association=`MEMBER` trusted=`true` broad=`false`", "association=`COLLABORATOR` trusted=`true` broad=`false`", "### Approval Label Risk Cards", "role=`write-requested` label=`gitclaw:write-requested` managed=`true`", "role=`approved` label=`gitclaw:approved` managed=`false`", "role=`needs-human` label=`gitclaw:needs-human` managed=`false`", "### Runtime Boundary Risk Card", "kind=`runtime-boundary` write_actions_supported=`false`", "### Risk Findings", "- none"} {
		if !strings.Contains(output, want) {
			t.Fatalf("approvals risk output missing %q:\n%s", want, output)
		}
	}
	for _, notWant := range []string{"repository:", "issue:", "event_kind:", "preflight_allowed:", "actor_association:", "write_request_detected:"} {
		if strings.Contains(output, notWant) {
			t.Fatalf("approvals risk output unexpectedly included %q:\n%s", notWant, output)
		}
	}
}

func TestApprovalsProvenanceCommandReportsStaticEvidenceBoundary(t *testing.T) {
	t.Setenv("GITCLAW_WORKDIR", t.TempDir())
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"approvals", "provenance"}); err != nil {
			t.Fatalf("approvals provenance returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Approvals Provenance Report", "scope: `local-cli`", "Generated without a model call", "current_issue_labels_available: `false`", "comments_available: `false`", "approval_provenance_status: `static_policy`", "verification_scope: `current-issue-labels-transcript-and-assistant-markers`", "approval_status: `static_policy`", "approval_decision: `no_write_requested`", "approval_store: `github-issue-labels`", "approval_scope: `per-issue`", "label_source: `current-github-issue-labels`", "write_request_source: `transcript-heuristic-or-label`", "assistant_marker_source: `issue-comments`", "write_actions_enabled: `false`", "repository_mutation_allowed: `false`", "host_exec_allowed: `false`", "raw_bodies_included: `false`", "raw_comments_included: `false`", "raw_prompts_included: `false`", "raw_approval_payloads_included: `false`", "run_urls_included: `false`", "llm_e2e_required_after_approval_provenance_change: `true`", "### Provenance Chain", "source=`local-config`", "### Managed Label Evidence", "role=`approved` label=`gitclaw:approved` present=`false`", "### Assistant Marker Evidence", "- none", "### Findings", "code=`local_static_policy_only`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("approvals provenance output missing %q:\n%s", want, output)
		}
	}
	for _, notWant := range []string{"issue: `#0`", "run_url=\""} {
		if strings.Contains(output, notWant) {
			t.Fatalf("approvals provenance output unexpectedly included %q:\n%s", notWant, output)
		}
	}
}
...
func TestProfileCatalogCommandReportsProfileSurfaceWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/SOUL.md", "SOUL_CLI_PROFILE_CATALOG_SECRET")
	writeTestFile(t, dir, ".gitclaw/IDENTITY.md", "IDENTITY_CLI_PROFILE_CATALOG_SECRET")
	writeTestFile(t, dir, ".gitclaw/USER.md", "USER_CLI_PROFILE_CATALOG_SECRET")
	writeTestFile(t, dir, ".gitclaw/TOOLS.md", "TOOLS_CLI_PROFILE_CATALOG_SECRET")
	writeTestFile(t, dir, ".gitclaw/MEMORY.md", "MEMORY_CLI_PROFILE_CATALOG_SECRET")
	writeTestFile(t, dir, ".gitclaw/HEARTBEAT.md", "HEARTBEAT_CLI_PROFILE_CATALOG_SECRET")
	writeTestFile(t, dir, ".gitclaw/memory/2026-05-30.md", "MEMORY_NOTE_CLI_PROFILE_CATALOG_SECRET")
	writeTestFile(t, dir, ".gitclaw/SKILLS/repo-reader/SKILL.md", `---
name: repo-reader
description: Use read-only repository context.
---

SKILL_CLI_PROFILE_CATALOG_SECRET`)
	writeTestFile(t, dir, ".gitclaw/proactive/repo-hygiene.md", "PROACTIVE_CLI_PROFILE_CATALOG_SECRET")
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"profile", "catalog"}); err != nil {
			t.Fatalf("profile catalog returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Profile Catalog Report", "scope: `local-cli`", "Generated without a model call", "profile_catalog_status: `ok`", "catalog_strategy: `compact-repo-local-profile-discovery`", "profile_strategy: `repo-local-git-profile`", "profile_surface: `identity, user, soul, memory, skills, bundles, tools, models, proactive, hooks, channels, backups, sessions`", "provider: `github-models`", "model: `openai/gpt-5-nano`", "catalog_entries: `10`", "profile_layers: `13`", "profile_documents_loaded: `7`", "identity_policy_files: `6`", "memory_notes: `1`", "available_skills: `1`", "available_tools: `5`", "raw_bodies_included: `false`", "raw_profile_payloads_included: `false`", "raw_skill_bodies_included: `false`", "raw_tool_outputs_included: `false`", "profile_mutation_allowed: `false`", "profile_switching_supported: `false`", "profile_export_supported: `false`", "llm_e2e_required_after_profile_catalog_change: `true`", "### Catalog Entries", "command=`catalog` issue_intent=`@gitclaw /profile catalog` local_command=`gitclaw profile catalog`", "command=`provenance` issue_intent=`@gitclaw /profile provenance` local_command=`gitclaw profile provenance`", "command=`search` issue_intent=`@gitclaw /profile search <query>` local_command=`gitclaw profile search <query>`", "command=`diff` issue_intent=`@gitclaw /profile diff [base-ref]` local_command=`gitclaw profile diff [base-ref]`", "command=`snapshot` issue_intent=`@gitclaw /profile snapshot` local_command=`gitclaw profile snapshot`", "command=`manifest` issue_intent=`@gitclaw /profile manifest` local_command=`gitclaw profile manifest`", "### Profile Layers", "layer=`memory` store=`.gitclaw/MEMORY.md + .gitclaw/memory/*.md`", "layer=`proactive` store=`.gitclaw/proactive + .github/workflows`", "### Catalog Gates", "profile_store_gate=`repo-local-reviewed-files`", "session_gate=`github-issue-thread-plus-backup-json`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("profile catalog output missing %q:\n%s", want, output)
		}
	}
	for _, notWant := range []string{"repository:", "issue:", "SOUL_CLI_PROFILE_CATALOG_SECRET", "IDENTITY_CLI_PROFILE_CATALOG_SECRET", "USER_CLI_PROFILE_CATALOG_SECRET", "TOOLS_CLI_PROFILE_CATALOG_SECRET", "MEMORY_CLI_PROFILE_CATALOG_SECRET", "HEARTBEAT_CLI_PROFILE_CATALOG_SECRET", "MEMORY_NOTE_CLI_PROFILE_CATALOG_SECRET", "SKILL_CLI_PROFILE_CATALOG_SECRET", "PROACTIVE_CLI_PROFILE_CATALOG_SECRET"} {
		if strings.Contains(output, notWant) {
			t.Fatalf("profile catalog output unexpectedly included %q:\n%s", notWant, output)
		}
	}
}

func TestProfileManifestCommandReportsPortabilityPlanWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/config.yml", "trigger:\n  label: gitclaw\n")
	writeTestFile(t, dir, ".gitclaw/SOUL.md", "SOUL_CLI_PROFILE_MANIFEST_SECRET")
	writeTestFile(t, dir, ".gitclaw/IDENTITY.md", "IDENTITY_CLI_PROFILE_MANIFEST_SECRET")
	writeTestFile(t, dir, ".gitclaw/USER.md", "USER_CLI_PROFILE_MANIFEST_SECRET")
	writeTestFile(t, dir, ".gitclaw/TOOLS.md", "TOOLS_CLI_PROFILE_MANIFEST_SECRET")
	writeTestFile(t, dir, ".gitclaw/MEMORY.md", "MEMORY_CLI_PROFILE_MANIFEST_SECRET")
	writeTestFile(t, dir, ".gitclaw/HEARTBEAT.md", "HEARTBEAT_CLI_PROFILE_MANIFEST_SECRET")
	writeTestFile(t, dir, ".gitclaw/SKILLS/repo-reader/SKILL.md", `---
name: repo-reader
description: Use read-only repository context.
---

SKILL_CLI_PROFILE_MANIFEST_SECRET`)
	writeTestFile(t, dir, ".gitclaw/proactive/repo-hygiene.md", "PROACTIVE_CLI_PROFILE_MANIFEST_SECRET")
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"profile", "export-plan"}); err != nil {
			t.Fatalf("profile export-plan returned error: %v", err)
		}
	})
	for _, want := range []string{
		...
		"kind=`tool-contract` name=`gitclaw.list_files`",
		"### Excluded State",
		"kind=`credentials`",
		"kind=`profile-mutation`",
	} {
		if !strings.Contains(output, want) {
			t.Fatalf("profile manifest output missing %q:\n%s", want, output)
		}
	}
	for _, notWant := range []string{"repository:", "issue:", "SOUL_CLI_PROFILE_MANIFEST_SECRET", "IDENTITY_CLI_PROFILE_MANIFEST_SECRET", "USER_CLI_PROFILE_MANIFEST_SECRET", "TOOLS_CLI_PROFILE_MANIFEST_SECRET", "MEMORY_CLI_PROFILE_MANIFEST_SECRET", "HEARTBEAT_CLI_PROFILE_MANIFEST_SECRET", "SKILL_CLI_PROFILE_MANIFEST_SECRET", "PROACTIVE_CLI_PROFILE_MANIFEST_SECRET"} {
		if strings.Contains(output, notWant) {
			t.Fatalf("profile manifest output unexpectedly included %q:\n%s", notWant, output)
		}
	}
}
...
func TestRunsHistoryCommandReportsBackupWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	backup := IssueBackup{
		Version:   1,
		Repo:      "owner/repo",
		...
		Issue: IssueBackupIssue{
			...
			Body:              "CLI_RUN_HISTORY_ISSUE_BODY_SECRET",
			Author:            "alice",
		...
		Comments: []IssueBackupComment{
			{
				...
				Body: RenderAssistantComment(Marker{
					RunID:               "backup-model-run",
					EventID:             "issue-141",
					Model:               "openai/gpt-5-nano",
					IdempotencyKey:      "CLI_RUN_HISTORY_IDEMPOTENCY_SECRET",
					RunURL:              "https://github.com/owner/repo/actions/runs/CLI_RUN_HISTORY_URL_SECRET",
					PromptContextSHA:    "def456abc123",
					ContextDocuments:    3,
					SelectedSkills:      1,
					ToolOutputs:         2,
					PromptVisibleSkills: []string{"repo-reader"},
					PromptVisibleTools:  []string{"gitclaw.search_files", "gitclaw.read_file"},
				}, "CLI_RUN_HISTORY_ASSISTANT_BODY_SECRET"),
				Author:            "github-actions[bot]",
				AuthorAssociation: "MEMBER",
			},
			{ID: 32, Body: "CLI_RUN_HISTORY_USER_COMMENT_SECRET", Author: "alice", AuthorAssociation: "MEMBER"},
		},
	}
	writeBackupFixture(t, dir, backup)
	backupPath := issueBackupPath(dir, backup.Repo, backup.Issue.Number)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"runs", "history", "--backup", backupPath}); err != nil {
			t.Fatalf("runs history returned error: %v", err)
		}
	})
	for _, want := range []string{
		...
		"index=`1` source=`comment:31` run_id=`backup-model-run` event_id=`issue-141` model=`openai/gpt-5-nano`",
		"prompt_context_sha256_12=`def456abc123`",
		"idempotency_key_sha256_12=`",
		"run_url_sha256_12=`",
		"comment_sha256_12=`",
	} {
		if !strings.Contains(output, want) {
			t.Fatalf("runs history output missing %q:\n%s", want, output)
		}
	}
	for _, notWant := range []string{"- repository:", "- issue:", "CLI_RUN_HISTORY_ISSUE_BODY_SECRET", "CLI_RUN_HISTORY_ASSISTANT_BODY_SECRET", "CLI_RUN_HISTORY_USER_COMMENT_SECRET", "CLI_RUN_HISTORY_IDEMPOTENCY_SECRET", "CLI_RUN_HISTORY_URL_SECRET"} {
		if strings.Contains(output, notWant) {
			t.Fatalf("runs history output unexpectedly included %q:\n%s", notWant, output)
		}
	}
}
...
func TestPolicyRiskCommandReportsPolicyBoundaryWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".github/workflows/gitclaw.yml", policyRiskWorkflowBody+"POLICY_RISK_CLI_WORKFLOW_TOKEN\n")
	writeTestFile(t, dir, "go.mod", "module example.com/policy-risk\nPOLICY_RISK_CLI_REPO_TOKEN\n")
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"policy", "risk"}); err != nil {
			t.Fatalf("policy risk returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Policy Risk Report", "scope: `local-cli`", "Generated without a model call", "policy_risk_status: `ok`", "verification_scope: `policy-trust-labels-workflow-permissions-and-write-boundary`", "run_mode: `read-only`", "model: `openai/gpt-5-nano`", "trusted_associations: `3`", "broad_trusted_associations: `0`", "managed_labels_configured: `9`", "duplicate_managed_labels: `0`", "workflow_path: `.github/workflows/gitclaw.yml`", "workflow_present: `true`", "workflow_sha256_12:", "workflow_verify_status: `ok`", "expected_jobs: `3`", "jobs_present: `3`", "expected_permissions: `7`", "expected_write_permissions: `2`", "permissions_present: `7`", "missing_permissions: `0`", "unexpected_write_permissions: `0`", "backup_concurrency_group: `true`", "backup_concurrency_cancel_safe: `true`", "policy_outputs_hashed: `0`", "policy_output_present: `false`", "policy_risk_findings: `0`", "write_request_policy_output_body_included: `false`", "write_actions_supported: `false`", "write_actions_enabled: `false`", "repository_mutation_allowed: `false`", "host_exec_allowed: `false`", "raw_bodies_included: `false`", "raw_inputs_included: `false`", "llm_e2e_required_after_policy_risk_change: `true`", "### Trust Boundary Risk Card", "### Managed Label Risk Card", "### Workflow Permission Risk Cards", "job=`handle` present=`true`", "### Policy Output Risk Cards", "kind=`policy-output` none", "### Runtime Boundary Risk Card", "### Risk Findings", "- none"} {
		if !strings.Contains(output, want) {
			t.Fatalf("policy risk output missing %q:\n%s", want, output)
		}
	}
	for _, notWant := range []string{"repository:", "issue:", "event_kind:", "preflight_allowed:", "actor_association:", "write_request_detected:", "POLICY_RISK_CLI_WORKFLOW_TOKEN", "POLICY_RISK_CLI_REPO_TOKEN"} {
		if strings.Contains(output, notWant) {
			t.Fatalf("policy risk output unexpectedly included %q:\n%s", notWant, output)
		}
	}
}
...
func TestContextRiskCommandReportsContextBoundaryWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/SOUL.md", "CONTEXT_RISK_CLI_SOUL_BODY")
	writeTestFile(t, dir, ".gitclaw/MEMORY.md", "CONTEXT_RISK_CLI_MEMORY_BODY")
	writeTestFile(t, dir, ".gitclaw/TOOLS.md", "CONTEXT_RISK_CLI_TOOLS_BODY")
	writeTestFile(t, dir, ".gitclaw/SKILLS/repo-reader/SKILL.md", `---
name: repo-reader
description: Use read-only repository context.
---

CONTEXT_RISK_CLI_SKILL_BODY
`)
	writeTestFile(t, dir, "go.mod", "module example.com/context-risk\nCONTEXT_RISK_CLI_REPO_TOKEN\n")
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"context", "risk"}); err != nil {
			t.Fatalf("context risk returned error: %v", err)
		}
	})
	for _, want := range []string{
		...
		if !strings.Contains(output, want) {
			t.Fatalf("context risk output missing %q:\n%s", want, output)
		}
	}
	for _, notWant := range []string{"repository:", "issue:", "CONTEXT_RISK_CLI_SOUL_BODY", "CONTEXT_RISK_CLI_MEMORY_BODY", "CONTEXT_RISK_CLI_TOOLS_BODY", "CONTEXT_RISK_CLI_SKILL_BODY", "CONTEXT_RISK_CLI_REPO_TOKEN", "module example.com/context-risk"} {
		if strings.Contains(output, notWant) {
			t.Fatalf("context risk output unexpectedly included %q:\n%s", notWant, output)
		}
	}
}
...
`)
	...
	for _, want := range []string{"GitClaw Doctor Report", "scope: `local-cli`", "Generated without a model call", "health_status: `ok`", "config_source: `defaults+repo+environment`", "config_valid: `true`", "config_file_present: `true`", "model: `openai/gpt-5-nano`", "run_mode: `read-only`", "workflows_present: `9`", "context_files_present: `6`", "memory_notes: `1`", "skill_files: `1`", "e2e_scripts: `1`", "e2e_live_issue_scripts: `1`", "e2e_cleanup_scripts: `1`", "e2e_model_coverage_scripts: `1`", "e2e_model_followup_scripts: `1`", "e2e_session_coverage_scripts: `1`", "e2e_backup_gate_scripts: `1`", "e2e_workflow_dispatch_scripts: `1`", "enabled_skills: `1`", "disabled_skills: `0`", "allowlist_blocked_skills: `0`", "enabled_tools: `5`", "disabled_tools: `0`", "allowlist_blocked_tools: `0`", "proactive_prompt_files: `1`", "managed_labels: `9`", "validation_errors: `0`", "validation_warnings: `0`", "skill_validation_status: `ok`", "soul_validation_status: `ok`", "memory_validation_status: `ok`", "tool_validation_status: `ok`", "`config_validation`: `ok`", "`workflow_set`: `ok`", "`identity_context`: `ok`", "`local_skills`: `ok`", "`proactive_prompt`: `ok`", "`e2e_harnesses`: `ok`", "### E2E Harnesses", "e2e_coverage_status=`ok`", "model_followup_scripts=`1`", "path=`scripts/e2e/github-session-coverage.sh`", "live_issue=`true`", "cleanup=`true`", "model_coverage=`true`", "model_followup=`true`", "session_coverage=`true`", "backup_gate=`true`", "workflow_dispatch=`true`", ".gitclaw/config.yml", ".github/workflows/gitclaw.yml", ".github/workflows/gitclaw-channel-send.yml", ".github/workflows/gitclaw-channel-outbox.yml", ".gitclaw/SOUL.md", ".gitclaw/SKILLS/repo-reader/SKILL.md", ".gitclaw/proactive/repo-hygiene.md", "sha256_12="} {
		if !strings.Contains(output, want) {
...
func TestSessionProvenanceCommandReportsBackupPromptEvidenceWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	backup := IssueBackup{
		Version:   1,
		Repo:      "owner/repo",
		...
		Issue: IssueBackupIssue{
			Number: 9,
			Title:  "@gitclaw session provenance",
			Body:   "SESSION_PROVENANCE_CLI_ISSUE_BODY_TOKEN",
		},
		Transcript: []TranscriptMessage{
			{Role: "user", Body: "SESSION_PROVENANCE_CLI_USER_TRANSCRIPT_TOKEN", Actor: "alice", AuthorAssociation: "MEMBER", Trusted: true},
			{Role: "assistant", Body: "SESSION_PROVENANCE_CLI_ASSISTANT_TRANSCRIPT_TOKEN", Actor: "github-actions[bot]", AuthorAssociation: "NONE", CommentID: 41, Trusted: true},
		},
		Comments: []IssueBackupComment{{
			ID:                41,
			Body:              "<!-- gitclaw:assistant-turn model=\"openai/gpt-4.1-nano\" prompt_context_sha256_12=\"abc123abc123\" context_documents=\"2\" selected_skills=\"1\" tool_outputs=\"2\" skills=\"repo-reader\" tools=\"gitclaw.search_files,gitclaw.read_file\" usage_prompt_tokens=\"100\" usage_completion_tokens=\"9\" usage_total_tokens=\"109\" -->\nSESSION_PROVENANCE_CLI_ASSISTANT_COMMENT_TOKEN",
			Author:            "github-actions[bot]",
			AuthorAssociation: "NONE",
		}},
	}
	writeBackupFixture(t, dir, backup)
	backupPath := issueBackupPath(dir, "owner/repo", 9)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"session", "provenance", "--backup", backupPath}); err != nil {
			t.Fatalf("session provenance returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Session Provenance Report", "scope: `local-backup`", "backup_file:", "repository: `owner/repo`", "issue: `#9`", "event_kind: `issue_comment`", "session_provenance_status: `ok`", "provenance_scope: `assistant-turn-marker-prompt-context`", "local_backup_store: `gitclaw-backups issue JSON`", "raw_comments: `1`", "transcript_messages: `2`", "assistant_turn_comments: `1`", "assistant_turns_with_prompt_provenance: `1`", "assistant_turns_missing_prompt_provenance: `0`", "model_backed_assistant_turns: `1`", "model_names: `openai/gpt-4.1-nano`", "prompt_visible_skill_names: `repo-reader`", "prompt_visible_tool_names: `gitclaw.search_files, gitclaw.read_file`", "usage_total_tokens: `109`", "raw_bodies_included: `false`", "raw_prompts_included: `false`", "raw_tool_outputs_included: `false`", "llm_e2e_required_after_session_provenance_change: `true`", "prompt_provenance_gate=`pass`", "model_backed_gate=`pass`", "usage_telemetry_gate=`pass`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("session provenance output missing %q:\n%s", want, output)
		}
	}
	for _, notWant := range []string{"SESSION_PROVENANCE_CLI_ISSUE_BODY_TOKEN", "SESSION_PROVENANCE_CLI_USER_TRANSCRIPT_TOKEN", "SESSION_PROVENANCE_CLI_ASSISTANT_TRANSCRIPT_TOKEN", "SESSION_PROVENANCE_CLI_ASSISTANT_COMMENT_TOKEN"} {
		if strings.Contains(output, notWant) {
			t.Fatalf("session provenance output unexpectedly included %q:\n%s", notWant, output)
		}
	}
}

func TestSessionToolsCommandReportsBackupToolLedgerWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	backup := IssueBackup{
		Version:   1,
		Repo:      "owner/repo",
		...
		Issue: IssueBackupIssue{
			Number: 10,
			Title:  "@gitclaw session tools",
			Body:   "SESSION_TOOLS_CLI_ISSUE_BODY_TOKEN",
		},
		Transcript: []TranscriptMessage{
			{Role: "user", Body: "SESSION_TOOLS_CLI_USER_TRANSCRIPT_TOKEN", Actor: "alice", AuthorAssociation: "MEMBER", Trusted: true},
			{Role: "assistant", Body: "SESSION_TOOLS_CLI_ASSISTANT_TRANSCRIPT_TOKEN", Actor: "github-actions[bot]", AuthorAssociation: "NONE", CommentID: 42, Trusted: true},
		},
		Comments: []IssueBackupComment{{
			ID:                42,
			Body:              "<!-- gitclaw:assistant-turn model=\"openai/gpt-4.1-nano\" prompt_context_sha256_12=\"abc123abc123\" context_documents=\"2\" selected_skills=\"1\" tool_outputs=\"2\" skills=\"repo-reader\" tools=\"gitclaw.search_files,gitclaw.read_file\" usage_prompt_tokens=\"100\" usage_completion_tokens=\"9\" usage_total_tokens=\"109\" -->\nSESSION_TOOLS_CLI_ASSISTANT_COMMENT_TOKEN",
			Author:            "github-actions[bot]",
			AuthorAssociation: "NONE",
		}},
	}
	writeBackupFixture(t, dir, backup)
	backupPath := issueBackupPath(dir, "owner/repo", 10)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"session", "tools", "--backup", backupPath}); err != nil {
			t.Fatalf("session tools returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Session Tools Report", "scope: `local-backup`", "backup_file:", "repository: `owner/repo`", "issue: `#10`", "event_kind: `issue_comment`", "session_tools_status: `ok`", "provenance_scope: `assistant-turn-marker-tool-context`", "local_backup_store: `gitclaw-backups issue JSON`", "raw_comments: `1`", "transcript_messages: `2`", "assistant_turn_comments: `1`", "tool_backed_assistant_turns: `1`", "unique_prompt_visible_tools: `2`", "prompt_visible_tool_names: `gitclaw.search_files, gitclaw.read_file`", "prompt_visible_tool_output_markers: `2`", "model_backed_tool_turns: `1`", "model_names: `openai/gpt-4.1-nano`", "usage_total_tokens: `109`", "raw_bodies_included: `false`", "raw_tool_inputs_included: `false`", "raw_tool_outputs_included: `false`", "llm_e2e_required_after_session_tools_change: `true`", "tool=`gitclaw.search_files` prompt_visible_turns=`1` model_backed_turns=`1`", "tool_context_gate=`pass`", "model_backed_tool_gate=`pass`", "usage_telemetry_gate=`pass`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("session tools output missing %q:\n%s", want, output)
		}
	}
	for _, notWant := range []string{"SESSION_TOOLS_CLI_ISSUE_BODY_TOKEN", "SESSION_TOOLS_CLI_USER_TRANSCRIPT_TOKEN", "SESSION_TOOLS_CLI_ASSISTANT_TRANSCRIPT_TOKEN", "SESSION_TOOLS_CLI_ASSISTANT_COMMENT_TOKEN"} {
		if strings.Contains(output, notWant) {
			t.Fatalf("session tools output unexpectedly included %q:\n%s", notWant, output)
		}
	}
}

func TestSessionSkillsCommandReportsBackupSkillLedgerWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	backup := IssueBackup{
		Version:   1,
		Repo:      "owner/repo",
		...
		Issue: IssueBackupIssue{
			Number: 11,
			Title:  "@gitclaw session skills",
			Body:   "SESSION_SKILLS_CLI_ISSUE_BODY_TOKEN",
		},
		Transcript: []TranscriptMessage{
			{Role: "user", Body: "SESSION_SKILLS_CLI_USER_TRANSCRIPT_TOKEN", Actor: "alice", AuthorAssociation: "MEMBER", Trusted: true},
			{Role: "assistant", Body: "SESSION_SKILLS_CLI_ASSISTANT_TRANSCRIPT_TOKEN", Actor: "github-actions[bot]", AuthorAssociation: "NONE", CommentID: 43, Trusted: true},
		},
		Comments: []IssueBackupComment{{
			ID:                43,
			Body:              "<!-- gitclaw:assistant-turn model=\"openai/gpt-4.1-nano\" prompt_context_sha256_12=\"abc123abc123\" context_documents=\"2\" selected_skills=\"1\" tool_outputs=\"2\" skills=\"repo-reader\" tools=\"gitclaw.search_files,gitclaw.read_file\" usage_prompt_tokens=\"100\" usage_completion_tokens=\"9\" usage_total_tokens=\"109\" -->\nSESSION_SKILLS_CLI_ASSISTANT_COMMENT_TOKEN",
			Author:            "github-actions[bot]",
			AuthorAssociation: "NONE",
		}},
	}
	...
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"session", "skills", "--backup", backupPath}); err != nil {
			t.Fatalf("session skills returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Session Skills Report", "scope: `local-backup`", "backup_file:", "repository: `owner/repo`", "issue: `#11`", "event_kind: `issue_comment`", "session_skills_status: `ok`", "provenance_scope: `assistant-turn-marker-skill-context`", "local_backup_store: `gitclaw-backups issue JSON`", "raw_comments: `1`", "transcript_messages: `2`", "assistant_turn_comments: `1`", "skill_backed_assistant_turns: `1`", "assistant_turns_missing_skill_context: `0`", "unique_prompt_visible_skills: `1`", "prompt_visible_skill_names: `repo-reader`", "selected_skill_markers: `1`", "model_backed_skill_turns: `1`", "deterministic_skill_turns: `0`", "model_names: `openai/gpt-4.1-nano`", "usage_total_tokens: `109`", "raw_bodies_included: `false`", "raw_issue_bodies_included: `false`", "raw_comment_bodies_included: `false`", "raw_assistant_replies_included: `false`", "raw_prompts_included: `false`", "raw_skill_bodies_included: `false`", "raw_tool_outputs_included: `false`", "raw_search_queries_included: `false`", "repository_mutation_allowed: `false`", "llm_e2e_required_after_session_skills_change: `true`", "skill=`repo-reader` prompt_visible_turns=`1` model_backed_turns=`1`", "skill_context_gate=`pass`", "model_backed_skill_gate=`pass`", "usage_telemetry_gate=`pass`", "raw_skill_body_gate=`marker-attributes-only`", "raw_tool_output_gate=`marker-attributes-only`", "mutation_gate=`disabled`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("session skills output missing %q:\n%s", want, output)
		}
	}
	for _, notWant := range []string{"SESSION_SKILLS_CLI_ISSUE_BODY_TOKEN", "SESSION_SKILLS_CLI_USER_TRANSCRIPT_TOKEN", "SESSION_SKILLS_CLI_ASSISTANT_TRANSCRIPT_TOKEN", "SESSION_SKILLS_CLI_ASSISTANT_COMMENT_TOKEN"} {
		if strings.Contains(output, notWant) {
			t.Fatalf("session skills output unexpectedly included %q:\n%s", notWant, output)
		}
	}
}

func TestSessionUsageCommandReportsBackupUsageLedgerWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	backup := IssueBackup{
		Version:   1,
		Repo:      "owner/repo",
		...
		Issue: IssueBackupIssue{
			Number: 12,
			Title:  "@gitclaw session usage",
			Body:   "SESSION_USAGE_CLI_ISSUE_BODY_TOKEN",
		},
		Transcript: []TranscriptMessage{
			{Role: "user", Body: "SESSION_USAGE_CLI_USER_TRANSCRIPT_TOKEN", Actor: "alice", AuthorAssociation: "MEMBER", Trusted: true},
			{Role: "assistant", Body: "SESSION_USAGE_CLI_ASSISTANT_TRANSCRIPT_TOKEN", Actor: "github-actions[bot]", AuthorAssociation: "NONE", CommentID: 44, Trusted: true},
		},
		Comments: []IssueBackupComment{{
			ID:                44,
			Body:              "<!-- gitclaw:assistant-turn model=\"openai/gpt-4.1-nano\" prompt_context_sha256_12=\"abc123abc123\" context_documents=\"2\" selected_skills=\"1\" tool_outputs=\"2\" skills=\"repo-reader\" tools=\"gitclaw.search_files,gitclaw.read_file\" usage_prompt_tokens=\"100\" usage_completion_tokens=\"9\" usage_total_tokens=\"109\" usage_cache_read_tokens=\"7\" usage_cache_write_tokens=\"2\" -->\nSESSION_USAGE_CLI_ASSISTANT_COMMENT_TOKEN",
			Author:            "github-actions[bot]",
			AuthorAssociation: "NONE",
		}},
	}
	writeBackupFixture(t, dir, backup)
	backupPath := issueBackupPath(dir, "owner/repo", 12)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"session", "usage", "--backup", backupPath}); err != nil {
			t.Fatalf("session usage returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Session Usage Report", "scope: `local-backup`", "backup_file:", "repository: `owner/repo`", "issue: `#12`", "event_kind: `issue_comment`", "session_usage_status: `ok`", "usage_scope: `assistant-turn-marker-token-telemetry`", "local_backup_store: `gitclaw-backups issue JSON`", "raw_comments: `1`", "transcript_messages: `2`", "assistant_turn_comments: `1`", "usage_bearing_assistant_turns: `1`", "assistant_turns_missing_usage_telemetry: `0`", "model_backed_usage_turns: `1`", "deterministic_usage_turns: `0`", "model_names: `openai/gpt-4.1-nano`", "usage_prompt_tokens: `100`", "usage_completion_tokens: `9`", "usage_total_tokens: `109`", "usage_cache_read_tokens: `7`", "usage_cache_write_tokens: `2`", "latest_usage_model: `openai/gpt-4.1-nano`", "raw_bodies_included: `false`", "raw_issue_bodies_included: `false`", "raw_comment_bodies_included: `false`", "raw_assistant_replies_included: `false`", "raw_prompts_included: `false`", "raw_provider_usage_included: `false`", "raw_provider_responses_included: `false`", "raw_tool_outputs_included: `false`", "raw_search_queries_included: `false`", "repository_mutation_allowed: `false`", "llm_e2e_required_after_session_usage_change: `true`", "model=`openai/gpt-4.1-nano` assistant_turns=`1` usage_turns=`1` model_backed_turns=`1` deterministic_turns=`0` prompt_tokens=`100` completion_tokens=`9` total_tokens=`109` cache_read_tokens=`7` cache_write_tokens=`2`", "usage_telemetry_gate=`pass`", "model_backed_usage_gate=`pass`", "raw_provider_usage_gate=`marker-attributes-only`", "raw_provider_response_gate=`disabled`", "raw_prompt_body_gate=`hashes-and-marker-attributes-only`", "mutation_gate=`disabled`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("session usage output missing %q:\n%s", want, output)
		}
	}
	for _, notWant := range []string{"SESSION_USAGE_CLI_ISSUE_BODY_TOKEN", "SESSION_USAGE_CLI_USER_TRANSCRIPT_TOKEN", "SESSION_USAGE_CLI_ASSISTANT_TRANSCRIPT_TOKEN", "SESSION_USAGE_CLI_ASSISTANT_COMMENT_TOKEN"} {
		if strings.Contains(output, notWant) {
			t.Fatalf("session usage output unexpectedly included %q:\n%s", notWant, output)
		}
	}
}

func TestSessionTrajectoryCommandReportsBackupManifestWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	backup := IssueBackup{
		Version:   1,
		Repo:      "owner/repo",
		...
		Issue: IssueBackupIssue{
			Number: 13,
			Title:  "@gitclaw session trajectory",
			Body:   "SESSION_TRAJECTORY_CLI_ISSUE_BODY_TOKEN",
		},
		Transcript: []TranscriptMessage{
			{Role: "user", Body: "SESSION_TRAJECTORY_CLI_USER_TRANSCRIPT_TOKEN", Actor: "alice", AuthorAssociation: "MEMBER", Trusted: true},
			{Role: "assistant", Body: "SESSION_TRAJECTORY_CLI_ASSISTANT_TRANSCRIPT_TOKEN", Actor: "github-actions[bot]", AuthorAssociation: "NONE", CommentID: 45, Trusted: true},
		},
		Comments: []IssueBackupComment{{
			ID:                45,
			Body:              "<!-- gitclaw:assistant-turn run_id=\"run-1\" event_id=\"issue-13\" model=\"openai/gpt-4.1-nano\" idempotency_key=\"idem-1\" run_url=\"https://github.com/owner/repo/actions/runs/123\" prompt_context_sha256_12=\"abc123abc123\" context_documents=\"2\" selected_skills=\"1\" tool_outputs=\"2\" skills=\"repo-reader\" tools=\"gitclaw.search_files,gitclaw.read_file\" usage_prompt_tokens=\"100\" usage_completion_tokens=\"9\" usage_total_tokens=\"109\" usage_cache_read_tokens=\"7\" usage_cache_write_tokens=\"2\" -->\nSESSION_TRAJECTORY_CLI_ASSISTANT_COMMENT_TOKEN",
			Author:            "github-actions[bot]",
			AuthorAssociation: "NONE",
		}},
	}
	writeBackupFixture(t, dir, backup)
	backupPath := issueBackupPath(dir, "owner/repo", 13)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"session", "trajectory", "--backup", backupPath}); err != nil {
			t.Fatalf("session trajectory returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Session Trajectory Report", "scope: `local-backup`", "backup_file:", "repository: `owner/repo`", "issue: `#13`", "event_kind: `issue_comment`", "session_trajectory_status: `ok`", "trajectory_scope: `body-free-assistant-turn-manifest`", "export_format: `gitclaw.session-trajectory.v1`", "local_backup_store: `gitclaw-backups issue JSON`", "raw_comments: `1`", "transcript_messages: `2`", "assistant_turn_comments: `1`", "trajectory_turns: `1`", "model_backed_assistant_turns: `1`", "deterministic_assistant_turns: `0`", "model_names: `openai/gpt-4.1-nano`", "prompt_visible_skill_names: `repo-reader`", "prompt_visible_tool_names: `gitclaw.search_files, gitclaw.read_file`", "run_metadata_turns: `1`", "unique_run_id_hashes: `1`", "usage_prompt_tokens: `100`", "usage_completion_tokens: `9`", "usage_total_tokens: `109`", "usage_cache_read_tokens: `7`", "usage_cache_write_tokens: `2`", "raw_bodies_included: `false`", "raw_issue_bodies_included: `false`", "raw_comment_bodies_included: `false`", "raw_assistant_replies_included: `false`", "raw_prompts_included: `false`", "raw_provider_responses_included: `false`", "raw_tool_outputs_included: `false`", "raw_search_queries_included: `false`", "repository_mutation_allowed: `false`", "llm_e2e_required_after_session_trajectory_change: `true`", "turn=`01` source=`comment:45` model=`openai/gpt-4.1-nano` deterministic=`false`", "run_id_sha256_12=", "event_id_sha256_12=", "idempotency_key_sha256_12=", "run_url_sha256_12=", "prompt_context_sha256_12=`abc123abc123`", "usage_present=`true`", "usage_total_tokens=`109`", "assistant_comment_sha256_12=", "prompt_provenance_gate=`pass`", "model_backed_gate=`pass`", "run_metadata_gate=`pass`", "usage_telemetry_gate=`pass`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("session trajectory output missing %q:\n%s", want, output)
		}
	}
	for _, notWant := range []string{"SESSION_TRAJECTORY_CLI_ISSUE_BODY_TOKEN", "SESSION_TRAJECTORY_CLI_USER_TRANSCRIPT_TOKEN", "SESSION_TRAJECTORY_CLI_ASSISTANT_TRANSCRIPT_TOKEN", "SESSION_TRAJECTORY_CLI_ASSISTANT_COMMENT_TOKEN", "https://github.com/owner/repo/actions/runs/123"} {
		if strings.Contains(output, notWant) {
			t.Fatalf("session trajectory output unexpectedly included %q:\n%s", notWant, output)
		}
	}
}

func TestSessionCompactionCommandReportsBackupReadinessWithoutBodies(t *testing.T) {
	t.Setenv("GITCLAW_WORKDIR", t.TempDir())
	dir := t.TempDir()
	backup := IssueBackup{
		Version:   1,
		...
		Issue: IssueBackupIssue{
			Number: 14,
			Title:  "@gitclaw session compaction",
			Body:   "SESSION_COMPACTION_CLI_ISSUE_BODY_TOKEN",
		},
		Transcript: []TranscriptMessage{
			{Role: "user", Body: "SESSION_COMPACTION_CLI_USER_TRANSCRIPT_TOKEN", Actor: "alice", AuthorAssociation: "MEMBER", Trusted: true},
			{Role: "assistant", Body: "SESSION_COMPACTION_CLI_ASSISTANT_TRANSCRIPT_TOKEN", Actor: "github-actions[bot]", AuthorAssociation: "NONE", CommentID: 46, Trusted: true},
		},
		Comments: []IssueBackupComment{{
			ID:                46,
			Body:              "<!-- gitclaw:assistant-turn run_id=\"run-1\" model=\"openai/gpt-4.1-nano\" prompt_context_sha256_12=\"abc123abc123\" context_documents=\"2\" selected_skills=\"1\" tool_outputs=\"2\" skills=\"repo-reader\" tools=\"gitclaw.search_files,gitclaw.read_file\" usage_prompt_tokens=\"100\" usage_completion_tokens=\"9\" usage_total_tokens=\"109\" usage_cache_read_tokens=\"7\" usage_cache_write_tokens=\"2\" -->\nSESSION_COMPACTION_CLI_ASSISTANT_COMMENT_TOKEN",
			Author:            "github-actions[bot]",
			AuthorAssociation: "NONE",
		}},
	}
	writeBackupFixture(t, dir, backup)
	backupPath := issueBackupPath(dir, "owner/repo", 14)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"session", "compaction", "--backup", backupPath}); err != nil {
			t.Fatalf("session compaction returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Session Compaction Report", "scope: `local-backup`", "backup_file:", "repository: `owner/repo`", "issue: `#14`", "event_kind: `issue_comment`", "session_compaction_status: `ok`", "compaction_scope: `body-free-session-compaction-readiness`", "compaction_strategy: `github-issue-thread-body-free-compaction-readiness`", "compression_model: `hermes-dual-thresholds+openclaw-trajectory-pruning`", "local_backup_store: `gitclaw-backups issue JSON`", "raw_comments: `1`", "transcript_messages: `2`", "assistant_turn_comments: `1`", "model_backed_assistant_turns: `1`", "deterministic_assistant_turns: `0`", "model_names: `openai/gpt-4.1-nano`", "prompt_visible_skill_names: `repo-reader`", "prompt_visible_tool_names: `gitclaw.search_files, gitclaw.read_file`", "usage_prompt_tokens: `100`", "usage_completion_tokens: `9`", "usage_total_tokens: `109`", "usage_cache_read_tokens: `7`", "usage_cache_write_tokens: `2`", "agent_compression_threshold_percent: `50`", "gateway_hygiene_threshold_percent: `85`", "agent_compaction_recommended: `false`", "gateway_hygiene_recommended: `false`", "lossy_summary_supported: `false`", "lossless_session_search_supported: `true`", "raw_bodies_included: `false`", "raw_issue_bodies_included: `false`", "raw_comment_bodies_included: `false`", "raw_assistant_replies_included: `false`", "raw_prompts_included: `false`", "raw_provider_usage_included: `false`", "raw_provider_responses_included: `false`", "raw_tool_outputs_included: `false`", "raw_search_queries_included: `false`", "repository_mutation_allowed: `false`", "compaction_mutation_allowed: `false`", "compression_writes_memory_allowed: `false`", "llm_e2e_required_after_session_compaction_change: `true`", "message=`01` role=`user`", "message=`02` role=`assistant`", "body_included=`false`", "agent_compaction_gate=`pass`", "gateway_hygiene_gate=`pass`", "model_backed_gate=`pass`", "lossless_recall_gate=`backup-json-and-session-search`", "raw_body_gate=`hashes-counts-and-metadata-only`", "mutation_gate=`disabled`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("session compaction output missing %q:\n%s", want, output)
		}
	}
	for _, notWant := range []string{"SESSION_COMPACTION_CLI_ISSUE_BODY_TOKEN", "SESSION_COMPACTION_CLI_USER_TRANSCRIPT_TOKEN", "SESSION_COMPACTION_CLI_ASSISTANT_TRANSCRIPT_TOKEN", "SESSION_COMPACTION_CLI_ASSISTANT_COMMENT_TOKEN", "run-1"} {
		if strings.Contains(output, notWant) {
			t.Fatalf("session compaction output unexpectedly included %q:\n%s", notWant, output)
		}
	}
}
...
func TestSessionCatalogCommandReportsBodyFreeCommandSurface(t *testing.T) {
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"session", "catalog"}); err != nil {
			t.Fatalf("session catalog returned error: %v", err)
		}
	})
	for _, want := range []string{
		...
		"command=`handoff` issue_intent=`@gitclaw /session handoff --id <id>` local_command=``",
		...
		if !strings.Contains(output, want) {
			t.Fatalf("session catalog output missing %q:\n%s", want, output)
		}
	}
	if strings.Contains(output, "issue: `#0`") {
		t.Fatalf("session catalog output should not include synthetic issue metadata:\n%s", output)
	}
}

func TestBackupCatalogCommandReportsBodyFreeCommandSurface(t *testing.T) {
	t.Setenv("GITHUB_REPOSITORY", "owner/repo")
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"backup", "catalog"}); err != nil {
			t.Fatalf("backup catalog returned error: %v", err)
		}
	})
	for _, want := range []string{
		...
		"repo_backup_dir: `.gitclaw/backups/owner__repo`",
		"catalog_entries: `18`",
		...
		"command=`verify` issue_intent=`@gitclaw /backup verify` local_command=`gitclaw backup verify --root .gitclaw/backups --repo owner/repo`",
		...
		if !strings.Contains(output, want) {
			t.Fatalf("backup catalog output missing %q:\n%s", want, output)
		}
	}
	for _, notWant := range []string{"issue: `#0`", "issue_backup_path:"} {
		if strings.Contains(output, notWant) {
			t.Fatalf("backup catalog output unexpectedly included %q:\n%s", notWant, output)
		}
	}
}
...
func TestMigrateRiskCommandReportsBoundaryWithoutBodies(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, dir, ".gitclaw/SOUL.md", "Soul policy.\n")
	writeTestFile(t, dir, ".gitclaw/IDENTITY.md", "Identity policy.\n")
	writeTestFile(t, dir, ".gitclaw/USER.md", "User profile.\n")
...
MIGRATE_RISK_CLI_SKILL_SECRET
`)
	t.Setenv("GITCLAW_WORKDIR", dir)
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"migrate", "risk", "hermes"}); err != nil {
			t.Fatalf("migrate risk returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Migration Risk Report", "scope: `local-cli`", "migration_risk_status: `needs_review`", "verification_scope: `agent_state_migration_boundary`", "normalized_source: `hermes`", "supported_source: `true`", "provider_import_items: `10`", "manual_review_items: `1`", "archive_only_items: `1`", "skipped_items: `1`", "credential_items: `1`", "executable_state_items: `2`", "memory_items: `2`", "skill_items: `1`", "session_archive_items: `1`", "source_scan_allowed: `false`", "source_home_read: `false`", "source_paths_printed: `false`", "apply_supported: `false`", "model_call_required: `false`", "repository_mutation_allowed: `false`", "credentials_import_allowed: `false`", "executable_state_import_allowed: `false`", "installer_execution_allowed: `false`", "mcp_autoload_allowed: `false`", "raw_source_body_included: `false`", "raw_secret_values_included: `false`", "backup_required_before_apply: `true`", "human_review_required: `true`", "quarantine_required: `true`", "soul_validation_status: `ok`", "skill_validation_status: `ok`", "tool_validation_status: `ok`", "llm_e2e_required_after_migration_risk_change: `true`", "source_kind=`mcp_servers` target=`.gitclaw/TOOLS.md` action=`manual-review`", "source_kind=`auth.json/.env` target=`manual secret setup` action=`skip`", "code=`credential_import_disabled`", "code=`executable_state_quarantined`", "code=`raw_state_archive_only`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("migrate risk output missing %q:\n%s", want, output)
		}
	}
	if strings.Contains(output, "MIGRATE_RISK_CLI_SKILL_SECRET") {
		t.Fatalf("migrate risk leaked skill body:\n%s", output)
	}
}
...
func TestBackupTimelineCommandReportsFetchedBackupChronology(t *testing.T) {
	...
	writeBackupFixture(t, dir, IssueBackup{
		...
		Issue: IssueBackupIssue{
			Number: 7,
			Title:  "@gitclaw cli backup timeline old CLI_BACKUP_TIMELINE_OLD_TITLE",
			Body:   "CLI_BACKUP_TIMELINE_OLD_BODY",
			Labels: []string{"gitclaw"},
		},
		Transcript: []TranscriptMessage{{Role: "user", Body: "CLI_BACKUP_TIMELINE_OLD_TRANSCRIPT"}},
	})
	writeBackupFixture(t, dir, IssueBackup{
		Version:     1,
		GeneratedAt: "2026-05-29T13:00:00Z",
		...
		Issue: IssueBackupIssue{
			Number: 8,
			Title:  "@gitclaw cli backup timeline new CLI_BACKUP_TIMELINE_NEW_TITLE",
			Body:   "CLI_BACKUP_TIMELINE_NEW_BODY",
			Labels: []string{"gitclaw", "gitclaw:e2e"},
		},
		Transcript: []TranscriptMessage{{Role: "user", Body: "CLI_BACKUP_TIMELINE_NEW_TRANSCRIPT"}, {Role: "assistant", Body: "CLI_BACKUP_TIMELINE_ASSISTANT"}},
		Comments:   []IssueBackupComment{{ID: 12, Body: "<!-- gitclaw:assistant-turn -->\nCLI_BACKUP_TIMELINE_COMMENT"}},
	})
	if _, err := WriteBackupIndex(dir, "owner/repo", time.Date(2026, 5, 29, 14, 0, 0, 0, time.UTC)); err != nil {
		t.Fatalf("WriteBackupIndex returned error: %v", err)
	}
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"backup", "timeline", "--root", dir, "--repo", "owner/repo", "--limit", "2"}); err != nil {
			t.Fatalf("backup timeline returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Backup Timeline Report", "backup_timeline_status: `ok`", "backup_verify_status: `ok`", "issue_count: `2`", "limit: `2`", "timeline_points: `2`", "timeline_order: `chronological`", "timeline_window: `latest_by_backup_generated_at`", "first_issue: `#7`", "latest_issue: `#8`", "total_span_seconds: `3600`", "raw_bodies_included: `false`", "llm_e2e_required_after_backup_timeline_change: `true`", "issue=#7 path=`issues/000007.json`", "gap_seconds_since_previous=`0`", "issue=#8 path=`issues/000008.json`", "gap_seconds_since_previous=`3600`", "assistant_turn_comments=`1`", "payload_sha256_12=", "title_sha256_12="} {
		if !strings.Contains(output, want) {
			t.Fatalf("backup timeline output missing %q:\n%s", want, output)
		}
	}
	for _, leaked := range []string{"CLI_BACKUP_TIMELINE_OLD_TITLE", "CLI_BACKUP_TIMELINE_OLD_BODY", "CLI_BACKUP_TIMELINE_OLD_TRANSCRIPT", "CLI_BACKUP_TIMELINE_NEW_TITLE", "CLI_BACKUP_TIMELINE_NEW_BODY", "CLI_BACKUP_TIMELINE_NEW_TRANSCRIPT", "CLI_BACKUP_TIMELINE_ASSISTANT", "CLI_BACKUP_TIMELINE_COMMENT", "@gitclaw cli backup timeline"} {
		if strings.Contains(output, leaked) {
...
func TestBackupCoverageCommandReportsFetchedIssueBackup(t *testing.T) {
	dir := t.TempDir()
	writeBackupFixture(t, dir, IssueBackup{
		Version:     1,
		GeneratedAt: "2026-05-29T13:00:00Z",
		...
		Issue: IssueBackupIssue{
			Number: 8,
			Title:  "@gitclaw cli backup coverage CLI_BACKUP_COVERAGE_TITLE",
			Body:   "CLI_BACKUP_COVERAGE_BODY",
			Labels: []string{"gitclaw", "gitclaw:e2e"},
		},
		Transcript: []TranscriptMessage{{Role: "user", Body: "CLI_BACKUP_COVERAGE_TRANSCRIPT"}, {Role: "assistant", Body: "CLI_BACKUP_COVERAGE_ASSISTANT"}},
		Comments:   []IssueBackupComment{{ID: 12, Body: "<!-- gitclaw:assistant-turn -->\nCLI_BACKUP_COVERAGE_COMMENT"}},
	})
	if _, err := WriteBackupIndex(dir, "owner/repo", time.Date(2026, 5, 29, 14, 0, 0, 0, time.UTC)); err != nil {
		t.Fatalf("WriteBackupIndex returned error: %v", err)
	}
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"backup", "coverage", "--root", dir, "--repo", "owner/repo", "--issue", "8"}); err != nil {
			t.Fatalf("backup coverage returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Backup Coverage Report", "backup_coverage_status: `ok`", "backup_verify_status: `ok`", "issue: `#8`", "issue_indexed: `true`", "expected_issue_backup_path: `issues/000008.json`", "issue_backup_path: `issues/000008.json`", "issue_backup_path_canonical: `true`", "issue_backup_payload_readable: `true`", "backup_event_name: `issue_comment`", "labels: `2`", "comments: `1`", "transcript_messages: `2`", "assistant_turn_comments: `1`", "raw_bodies_included: `false`", "llm_e2e_required_after_backup_coverage_change: `true`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("backup coverage output missing %q:\n%s", want, output)
		}
	}
	for _, leaked := range []string{"CLI_BACKUP_COVERAGE_TITLE", "CLI_BACKUP_COVERAGE_BODY", "CLI_BACKUP_COVERAGE_TRANSCRIPT", "CLI_BACKUP_COVERAGE_ASSISTANT", "CLI_BACKUP_COVERAGE_COMMENT", "@gitclaw cli backup coverage"} {
		if strings.Contains(output, leaked) {
...
func TestBackupDrillCommandReportsRestoreReadiness(t *testing.T) {
	dir := t.TempDir()
	writeBackupFixture(t, dir, IssueBackup{
		Version:     1,
		GeneratedAt: "2026-05-29T13:00:00Z",
		...
		Issue: IssueBackupIssue{
			Number: 8,
			Title:  "@gitclaw cli backup drill CLI_BACKUP_DRILL_TITLE",
			Body:   "CLI_BACKUP_DRILL_BODY",
			Labels: []string{"gitclaw", "gitclaw:e2e"},
		},
		Transcript: []TranscriptMessage{{Role: "user", Body: "CLI_BACKUP_DRILL_TRANSCRIPT"}, {Role: "assistant", Body: "CLI_BACKUP_DRILL_ASSISTANT"}},
		Comments:   []IssueBackupComment{{ID: 12, Body: "<!-- gitclaw:assistant-turn -->\nCLI_BACKUP_DRILL_COMMENT"}},
	})
	if _, err := WriteBackupIndex(dir, "owner/repo", time.Date(2026, 5, 29, 14, 0, 0, 0, time.UTC)); err != nil {
		t.Fatalf("WriteBackupIndex returned error: %v", err)
	}
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"backup", "drill", "--root", dir, "--repo", "owner/repo", "--target-repo", "owner/restored", "--issue", "8"}); err != nil {
			t.Fatalf("backup drill returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Backup Drill Report", "target_repository: `owner/restored`", "backup_drill_status: `ok`", "backup_verify_status: `ok`", "backup_coverage_status: `ok`", "restore_plan_status: `ok`", "restore_mode: `dry-run`", "issue: `#8`", "issue_indexed: `true`", "expected_issue_backup_path: `issues/000008.json`", "issue_backup_path: `issues/000008.json`", "issue_backup_payload_readable: `true`", "backup_event_name: `issue_comment`", "labels: `2`", "comments: `1`", "transcript_messages: `2`", "assistant_turn_comments: `1`", "restore_plan_available: `true`", "raw_bodies_included: `false`", "mutation_performed: `false`", "github_api_calls_performed: `false`", "llm_e2e_required_after_backup_drill_change: `true`", "verify_gate=`pass`", "coverage_gate=`pass`", "restore_plan_gate=`pass`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("backup drill output missing %q:\n%s", want, output)
		}
	}
	for _, leaked := range []string{"CLI_BACKUP_DRILL_TITLE", "CLI_BACKUP_DRILL_BODY", "CLI_BACKUP_DRILL_TRANSCRIPT", "CLI_BACKUP_DRILL_ASSISTANT", "CLI_BACKUP_DRILL_COMMENT", "@gitclaw cli backup drill"} {
		if strings.Contains(output, leaked) {
...
func TestBackupStatsCommandReportsFetchedBackupTree(t *testing.T) {
	...
	for _, want := range []string{"GitClaw Backup Stats Report", "backup_stats_status: `ok`", "backup_verify_status: `ok`", "issue_count: `1`", "transcript_messages: `1`", "latest_issue: `#7`", "raw_bodies_included: `false`", "llm_e2e_required_after_backup_stats_change: `true`"} {
		if !strings.Contains(output, want) {
...
func TestBackupSnapshotCommandReportsFetchedBackupFingerprint(t *testing.T) {
	dir := t.TempDir()
	writeBackupFixture(t, dir, IssueBackup{
		Version:     1,
		GeneratedAt: "2026-05-29T12:00:00Z",
		Repo:        "owner/repo",
		EventName:   "issues",
		Issue:       IssueBackupIssue{Number: 7, Title: "@gitclaw cli snapshot CLI_SNAPSHOT_TITLE_TOKEN", Body: "CLI_SNAPSHOT_BODY_TOKEN"},
		Transcript:  []TranscriptMessage{{Role: "user", Body: "CLI_SNAPSHOT_TRANSCRIPT_TOKEN"}, {Role: "assistant", Body: "CLI_SNAPSHOT_ASSISTANT_TOKEN"}},
		Comments:    []IssueBackupComment{{ID: 12, Body: "<!-- gitclaw:assistant-turn -->\nCLI_SNAPSHOT_COMMENT_TOKEN"}},
	})
	if _, err := WriteBackupIndex(dir, "owner/repo", time.Date(2026, 5, 29, 13, 0, 0, 0, time.UTC)); err != nil {
		t.Fatalf("WriteBackupIndex returned error: %v", err)
	}
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"backup", "snapshot", "--root", dir, "--repo", "owner/repo"}); err != nil {
			t.Fatalf("backup snapshot returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Backup Snapshot Report", "backup_snapshot_status: `ok`", "backup_verify_status: `ok`", "snapshot_version: `gitclaw-backup-snapshot-v1`", "snapshot_sha256_12:", "snapshot_entries: `3`", "control_file_entries: `2`", "issue_payload_entries: `1`", "issue_count: `1`", "transcript_messages: `2`", "assistant_turn_comments: `1`", "raw_bodies_included: `false`", "llm_e2e_required_after_backup_snapshot_change: `true`", "kind=`control-file` path=`index.json`", "kind=`control-file` path=`README.md`", "kind=`issue-payload` issue=#7 path=`issues/000007.json`", "verify_gate=`pass`", "snapshot_hash_gate=`composite-sha256_12:"} {
		if !strings.Contains(output, want) {
			t.Fatalf("backup snapshot output missing %q:\n%s", want, output)
		}
	}
	for _, leaked := range []string{"CLI_SNAPSHOT_TITLE_TOKEN", "CLI_SNAPSHOT_BODY_TOKEN", "CLI_SNAPSHOT_TRANSCRIPT_TOKEN", "CLI_SNAPSHOT_ASSISTANT_TOKEN", "CLI_SNAPSHOT_COMMENT_TOKEN", "@gitclaw cli snapshot"} {
		if strings.Contains(output, leaked) {
...
func TestBackupFreshnessCommandReportsFetchedBackupTree(t *testing.T) {
	dir := t.TempDir()
	writeBackupFixture(t, dir, IssueBackup{
		Version:     1,
		GeneratedAt: "2026-05-29T12:00:00Z",
		Repo:        "owner/repo",
		EventName:   "issues",
		Issue:       IssueBackupIssue{Number: 7, Title: "@gitclaw cli freshness", Body: "CLI_FRESHNESS_BODY_TOKEN"},
		Transcript:  []TranscriptMessage{{Role: "user", Body: "CLI_FRESHNESS_TRANSCRIPT_TOKEN"}},
	})
	if _, err := WriteBackupIndex(dir, "owner/repo", time.Date(2026, 5, 29, 13, 0, 0, 0, time.UTC)); err != nil {
		t.Fatalf("WriteBackupIndex returned error: %v", err)
	}
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"backup", "freshness", "--root", dir, "--repo", "owner/repo", "--max-age-hours", "24", "--now", "2026-05-29T13:00:00Z"}); err != nil {
			t.Fatalf("backup freshness returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Backup Freshness Report", "backup_freshness_status: `ok`", "backup_verify_status: `ok`", "issue_count: `1`", "max_age_seconds: `86400`", "as_of: `2026-05-29T13:00:00Z`", "latest_issue: `#7`", "latest_age_seconds: `3600`", "freshness_gate: `pass`", "raw_bodies_included: `false`", "llm_e2e_required_after_backup_freshness_change: `true`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("backup freshness output missing %q:\n%s", want, output)
		}
	}
	if strings.Contains(output, "CLI_FRESHNESS_BODY_TOKEN") || strings.Contains(output, "CLI_FRESHNESS_TRANSCRIPT_TOKEN") || strings.Contains(output, "@gitclaw cli freshness") {
		t.Fatalf("backup freshness leaked body/title token:\n%s", output)
	}
}

func TestBackupContinuityCommandReportsFetchedBackupTree(t *testing.T) {
	dir := t.TempDir()
	writeBackupFixture(t, dir, IssueBackup{
		Version:     1,
		GeneratedAt: "2026-05-29T12:00:00Z",
		Repo:        "owner/repo",
		EventName:   "issues",
		Issue:       IssueBackupIssue{Number: 7, Title: "@gitclaw cli continuity old", Body: "CLI_CONTINUITY_OLD_BODY_TOKEN"},
		Transcript:  []TranscriptMessage{{Role: "user", Body: "CLI_CONTINUITY_OLD_TRANSCRIPT_TOKEN"}},
	})
	writeBackupFixture(t, dir, IssueBackup{
		Version:     1,
		...
		Repo:        "owner/repo",
		EventName:   "issue_comment",
		Issue:       IssueBackupIssue{Number: 8, Title: "@gitclaw cli continuity new", Body: "CLI_CONTINUITY_NEW_BODY_TOKEN"},
		Transcript:  []TranscriptMessage{{Role: "user", Body: "CLI_CONTINUITY_NEW_TRANSCRIPT_TOKEN"}},
	})
	if _, err := WriteBackupIndex(dir, "owner/repo", time.Date(2026, 5, 29, 15, 0, 0, 0, time.UTC)); err != nil {
		t.Fatalf("WriteBackupIndex returned error: %v", err)
	}
	output := captureStdout(t, func() {
		if err := RunCLI(context.Background(), []string{"backup", "continuity", "--root", dir, "--repo", "owner/repo", "--max-gap-hours", "24"}); err != nil {
			t.Fatalf("backup continuity returned error: %v", err)
		}
	})
	for _, want := range []string{"GitClaw Backup Continuity Report", "backup_continuity_status: `ok`", "backup_verify_status: `ok`", "issue_count: `2`", "points_scanned: `2`", "max_gap_seconds: `86400`", "gaps_over_max: `0`", "longest_gap_seconds: `7200`", "continuity_gate: `pass`", "raw_bodies_included: `false`", "llm_e2e_required_after_backup_continuity_change: `true`"} {
		if !strings.Contains(output, want) {
			t.Fatalf("backup continuity output missing %q:\n%s", want, output)
		}
	}
	if strings.Contains(output, "CLI_CONTINUITY_OLD_BODY_TOKEN") || strings.Contains(output, "CLI_CONTINUITY_OLD_TRANSCRIPT_TOKEN") || strings.Contains(output, "CLI_CONTINUITY_NEW_BODY_TOKEN") || strings.Contains(output, "CLI_CONTINUITY_NEW_TRANSCRIPT_TOKEN") || strings.Contains(output, "@gitclaw cli continuity") {
		t.Fatalf("backup continuity leaked body/title token:\n%s", output)
	}
}
