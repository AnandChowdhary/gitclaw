...
SKILL_PROFILE_CATALOG_SECRET`)
	...
	for _, want := range []string{
		...
		"command=`diff` issue_intent=`@gitclaw /profile diff [base-ref]` local_command=`gitclaw profile diff [base-ref]` execution=`body-free-profile-git-diff` gate=`numstat-and-status-only`",
