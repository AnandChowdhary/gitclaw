...
func Handle(ctx context.Context, ev Event, cfg Config, github GitHubClient, llm LLMClient) error {
	...
	if IsChannelResearchMapActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel research map"))
		}
		req, err := BuildChannelResearchMapActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel research map action: %w", err))
		}
		result, err := RunChannelResearchMap(ctx, cfg, channelClient, req)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel research map action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelResearchMapActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel research map action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	...
	if IsChannelToolSpotlightActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel tool spotlight"))
		}
		req, err := BuildChannelToolSpotlightActionRequest(ev, cfg, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel tool spotlight action: %w", err))
		}
		result, err := RunChannelToolSpotlight(ctx, cfg, channelClient, req, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel tool spotlight action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelToolSpotlightActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel tool spotlight action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	...
	if IsChannelToolMapActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel tool map"))
		}
		req, err := BuildChannelToolMapActionRequest(ev, cfg, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel tool map action: %w", err))
		}
		result, err := RunChannelToolMap(ctx, cfg, channelClient, req.Options, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel tool map action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelToolMapActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel tool map action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	...
	if IsChannelSoulSpotlightActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel soul spotlight"))
		}
		req, err := BuildChannelSoulSpotlightActionRequest(ev, cfg, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel soul spotlight action: %w", err))
		}
		result, err := RunChannelSoulSpotlight(ctx, cfg, channelClient, req, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel soul spotlight action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelSoulSpotlightActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel soul spotlight action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	...
	if IsChannelRoomPulseActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel room pulse cards"))
		}
		req, err := BuildChannelRoomPulseActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel room pulse action: %w", err))
		}
		result, err := RunChannelRoomPulse(ctx, cfg, channelClient, req)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel room pulse action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelRoomPulseActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel room pulse action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelQuickRepliesActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel quick replies"))
		}
		req, err := BuildChannelQuickRepliesActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel quick replies action: %w", err))
		}
		result, err := RunChannelQuickReplies(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel quick replies action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelQuickRepliesActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel quick replies action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelStatusWheelActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel status wheels"))
		}
		req, err := BuildChannelStatusWheelActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel status wheel action: %w", err))
		}
		result, err := RunChannelStatusWheel(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel status wheel action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelStatusWheelActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel status wheel action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	...
	if IsChannelRiddleActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel riddles"))
		}
		req, err := BuildChannelRiddleActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel riddle action: %w", err))
		}
		result, err := RunChannelRiddle(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel riddle action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelRiddleActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel riddle action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelStickerActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel stickers"))
		}
		req, err := BuildChannelStickerActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel sticker action: %w", err))
		}
		result, err := RunChannelSticker(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel sticker action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelStickerActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel sticker action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelToastActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel toasts"))
		}
		req, err := BuildChannelToastActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel toast action: %w", err))
		}
		result, err := RunChannelToast(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel toast action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelToastActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel toast action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelTimerActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel timers"))
		}
		req, err := BuildChannelTimerActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel timer action: %w", err))
		}
		result, err := RunChannelTimer(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel timer action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelTimerActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel timer action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelBingoActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel bingos"))
		}
		req, err := BuildChannelBingoActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel bingo action: %w", err))
		}
		result, err := RunChannelBingo(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel bingo action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelBingoActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel bingo action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelHaikuActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel haikus"))
		}
		req, err := BuildChannelHaikuActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel haiku action: %w", err))
		}
		result, err := RunChannelHaiku(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel haiku action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelHaikuActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel haiku action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelSoundtrackActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel soundtracks"))
		}
		req, err := BuildChannelSoundtrackActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel soundtrack action: %w", err))
		}
		result, err := RunChannelSoundtrack(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel soundtrack action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelSoundtrackActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel soundtrack action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelStoryDiceActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel story dice"))
		}
		req, err := BuildChannelStoryDiceActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel story dice action: %w", err))
		}
		result, err := RunChannelStoryDice(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel story dice action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelStoryDiceActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel story dice action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelMadLibsActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel mad libs"))
		}
		req, err := BuildChannelMadLibsActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel mad libs action: %w", err))
		}
		result, err := RunChannelMadLibs(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel mad libs action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelMadLibsActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel mad libs action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelArcadeActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel arcades"))
		}
		req, err := BuildChannelArcadeActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel arcade action: %w", err))
		}
		result, err := RunChannelArcade(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel arcade action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelArcadeActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel arcade action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	...
	if IsChannelCoachActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel coaches"))
		}
		req, err := BuildChannelCoachActionRequest(ev, cfg, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel coach action: %w", err))
		}
		result, err := RunChannelCoach(ctx, cfg, channelClient, req.Options, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel coach action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelCoachActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel coach action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelRecoveryMapActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel recovery maps"))
		}
		req, err := BuildChannelRecoveryMapActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel recovery map action: %w", err))
		}
		result, err := RunChannelRecoveryMap(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel recovery map action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelRecoveryMapActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel recovery map action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelNudgeActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel nudges"))
		}
		req, err := BuildChannelNudgeActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel nudge action: %w", err))
		}
		result, err := RunChannelNudge(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel nudge action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelNudgeActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel nudge action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelConstellationActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel constellations"))
		}
		req, err := BuildChannelConstellationActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel constellation action: %w", err))
		}
		result, err := RunChannelConstellation(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel constellation action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelConstellationActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel constellation action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelMissionControlActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel mission-control cards"))
		}
		req, err := BuildChannelMissionControlActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel mission-control action: %w", err))
		}
		result, err := RunChannelMissionControl(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel mission-control action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelMissionControlActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel mission-control action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelCockpitActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel cockpit cards"))
		}
		req, err := BuildChannelCockpitActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel cockpit action: %w", err))
		}
		result, err := RunChannelCockpit(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel cockpit action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelCockpitActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel cockpit action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelPaletteActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel palettes"))
		}
		req, err := BuildChannelPaletteActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel palette action: %w", err))
		}
		result, err := RunChannelPalette(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel palette action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelPaletteActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel palette action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelCompassActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel compasses"))
		}
		req, err := BuildChannelCompassActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel compass action: %w", err))
		}
		result, err := RunChannelCompass(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel compass action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelCompassActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel compass action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelModeActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel modes"))
		}
		req, err := BuildChannelModeActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel mode action: %w", err))
		}
		result, err := RunChannelMode(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel mode action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelModeActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel mode action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelWarmupActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel warmups"))
		}
		req, err := BuildChannelWarmupActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel warmup action: %w", err))
		}
		result, err := RunChannelWarmup(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel warmup action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelWarmupActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel warmup action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	...
	if IsChannelSkillSpotlightActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel skill spotlight"))
		}
		req, err := BuildChannelSkillSpotlightActionRequest(ev, cfg, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel skill spotlight action: %w", err))
		}
		result, err := RunChannelSkillSpotlight(ctx, cfg, channelClient, req, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel skill spotlight action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelSkillSpotlightActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel skill spotlight action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	...
	if IsChannelSkillMapActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel skill map"))
		}
		req, err := BuildChannelSkillMapActionRequest(ev, cfg, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel skill map action: %w", err))
		}
		result, err := RunChannelSkillMap(ctx, cfg, channelClient, req.Options, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel skill map action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelSkillMapActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel skill map action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelBundleMapActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel bundle map"))
		}
		req, err := BuildChannelBundleMapActionRequest(ev, cfg, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel bundle map action: %w", err))
		}
		result, err := RunChannelBundleMap(ctx, cfg, channelClient, req.Options, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel bundle map action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelBundleMapActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel bundle map action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelSourceMapActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel source map"))
		}
		req, err := BuildChannelSourceMapActionRequest(ev, cfg, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel source map action: %w", err))
		}
		result, err := RunChannelSourceMap(ctx, cfg, channelClient, req.Options, repoContext)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel source map action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelSourceMapActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel source map action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	...
	if IsChannelBackupSpotlightActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel backup spotlight"))
		}
		req, err := BuildChannelBackupSpotlightActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel backup spotlight action: %w", err))
		}
		result, err := RunChannelBackupSpotlight(ctx, cfg, channelClient, req)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel backup spotlight action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelBackupSpotlightActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel backup spotlight action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	...
	if IsChannelBackupTimelineActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel backup timeline"))
		}
		req, err := BuildChannelBackupTimelineActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel backup timeline action: %w", err))
		}
		result, err := RunChannelBackupTimeline(ctx, cfg, channelClient, req)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel backup timeline action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelBackupTimelineActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel backup timeline action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelBackupFreshnessActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel backup freshness"))
		}
		req, err := BuildChannelBackupFreshnessActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel backup freshness action: %w", err))
		}
		result, err := RunChannelBackupFreshness(ctx, cfg, channelClient, req)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel backup freshness action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelBackupFreshnessActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel backup freshness action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	if IsChannelBackupContinuityActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel backup continuity"))
		}
		req, err := BuildChannelBackupContinuityActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel backup continuity action: %w", err))
		}
		result, err := RunChannelBackupContinuity(ctx, cfg, channelClient, req)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel backup continuity action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelBackupContinuityActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel backup continuity action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	...
	if IsChannelBoundaryActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot capture channel boundaries"))
		}
		req, err := BuildChannelBoundaryActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel boundary action: %w", err))
		}
		result, err := RunChannelBoundary(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel boundary action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelBoundaryActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel boundary action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	...
	if IsChannelDockActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot create channel dock requests"))
		}
		req, err := BuildChannelDockActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel dock action: %w", err))
		}
		result, err := RunChannelDock(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel dock action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelDockActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel dock action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
	...
	if IsChannelBrowserStatusActionRequest(ev, cfg) {
		channelClient, ok := github.(ChannelSendGitHubClient)
		if !ok {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("github client cannot queue channel browser status"))
		}
		req, err := BuildChannelBrowserStatusActionRequest(ev, cfg)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("build channel browser status action: %w", err))
		}
		result, err := RunChannelBrowserStatus(ctx, cfg, channelClient, req.Options)
		if err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "channel", fmt.Errorf("run channel browser status action: %w", err))
		}
		body := RenderAssistantComment(Marker{
			RunID:          envFirst("GITHUB_RUN_ID", "local"),
			EventID:        eventID(ev),
			...
			IdempotencyKey: key,
			RunURL:         actionRunURL(ev),
		}, RenderChannelBrowserStatusActionReport(ev, req, result))
		if _, err := github.PostIssueComment(ctx, ev.Repo, ev.Issue.Number, body); err != nil {
			return failStartedTurn(ctx, cfg, github, ev, status, "comment", fmt.Errorf("post channel browser status action comment: %w", err))
		}
		status.SetDone()
		return nil
	}
