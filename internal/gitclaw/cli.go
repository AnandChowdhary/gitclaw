...
func runToolsMapCommand(args []string) error {
	if len(args) != 1 {
		return fmt.Errorf("usage: gitclaw tools map <name>")
	}
	cfg, err := LoadEffectiveConfig()
	if err != nil {
		return err
	}
	repoContext, err := LoadRepoContextWithConfig(cfg.Workdir, []TranscriptMessage{{Role: "user", Body: "tools map " + args[0]}}, cfg)
	if err != nil {
		return err
	}
...
func runChannelOutboxCommand(ctx context.Context, args []string) error {
	...
	fmt.Printf(
		"channel_outbox issue=%d state_issue=%d assistant_comments=%d outbound_comments=%d reaction_comments=%d status_comments=%d edit_comments=%d topic_comments=%d activity_comments=%d deliverable_comments=%d delivered=%d pending=%d returned=%d body_included=%t account_sha256_12=%s out=%s\n",
		result.IssueNumber,
		result.StateIssueNumber,
