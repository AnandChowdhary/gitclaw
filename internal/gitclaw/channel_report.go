...
func renderChannelReport(ev Event, cfg Config, comments []Comment, includeIssue bool) string {
	...
	b.WriteString("- `/channels icebreaker --icebreaker-id <id> --message-id <message>` queues a provider-facing low-pressure thread starter with the default icebreaker theme without calling a model, creating polls, creating rollcalls, creating tasks, creating schedules, provider API calls, or mutating the repository\n")
	...
	b.WriteString("- `/channels skill-spotlight <focus> --spotlight-id <id> --message-id <message>` queues one deterministic provider-facing skill spotlight card without installing skills or exposing skill bodies\n")
	...
	b.WriteString("- `/channels bundle-map <bundle> --map-id <id> --message-id <message>` queues a provider-facing safe skill-bundle sequence without installing skills, enabling bundles, writing bundle YAML, creating proposal/rehearsal issues, calling models, mutating workflows, or exposing raw bundle bodies\n")
	b.WriteString("- `/channels source-map <source> --map-id <id> --message-id <message>` queues a provider-facing safe skill-source sequence without registry contact, remote fetches, installs, source-pin writes, proposal issues, model calls, workflow edits, or repo mutation\n")
	b.WriteString("- `/channels research-spotlight <focus> --spotlight-id <id> --message-id <message>` queues one deterministic provider-facing research spotlight card from the reviewed static catalog without fetching sources or exposing raw research bodies\n")
	b.WriteString("- `/channels research-map <focus> --map-id <id> --message-id <message>` queues a provider-facing research-to-GitClaw sequence from the reviewed static catalog without fetching sources, live browsing, executing tools, calling models, or mutating workflows\n")
	...
	b.WriteString("- `/channels tool-spotlight <focus> --spotlight-id <id> --message-id <message>` queues one deterministic provider-facing safe tool spotlight card without executing tools or exposing raw triggers/schemas\n")
	b.WriteString("- `/channels tool-drill <focus> --drill-id <id> --message-id <message>` queues one deterministic provider-facing safe tool practice card without executing tools, launching MCP servers, activating toolsets, or exposing raw triggers/schemas\n")
	...
	b.WriteString("- `/channels backup-spotlight <focus> --spotlight-id <id> --message-id <message>` queues one deterministic provider-facing backup spotlight card from local or read-only fetched gitclaw-backups metadata without restoring files or exposing raw payloads\n")
	b.WriteString("- `/channels backup-timeline --timeline-id <id> --message-id <message>` queues a provider-facing body-free backup chronology card from local or read-only fetched gitclaw-backups metadata\n")
	b.WriteString("- `/channels backup-freshness --freshness-id <id> --message-id <message>` queues a provider-facing backup freshness gate card from local or read-only fetched gitclaw-backups metadata\n")
	b.WriteString("- `/channels backup-continuity --continuity-id <id> --message-id <message>` queues a provider-facing backup continuity/gap card from local or read-only fetched gitclaw-backups metadata\n")
	...
	b.WriteString("- `/channels soul-spotlight <focus> --spotlight-id <id> --message-id <message>` queues one deterministic provider-facing high-authority context spotlight card without registry contact, profile export, soul writes, model calls, or raw bodies\n")
	...
	b.WriteString("- `/channels riddle <theme> --riddle-id <id> --message-id <message>` queues a provider-facing deterministic riddle card from bounded static decks without model calls, external randomness, game-state persistence, score tracking, provider API calls, workflow edits, or repo mutation\n")
	...
	b.WriteString("- `/channels postcard <title> --postcard-id <id> --message-id <message>` queues a provider-facing place/caption scene card without calling a model, generating images, fetching media, calling provider APIs, or mutating the repository\n")
	b.WriteString("- `/channels timer <duration> --timer-id <id> --message-id <message>` queues a provider-facing timebox cue without creating reminder issues, scheduling workflows, starting provider timers, calling a model, calling provider APIs, or mutating the repository\n")
	b.WriteString("- `/channels bingo <theme> --bingo-id <id> --message-id <message>` queues a provider-facing deterministic mini-game card without model calls, external randomness, game-state persistence, score tracking, provider API calls, workflow edits, or repo mutation\n")
	...
	b.WriteString("- `/channels soundtrack <theme> --soundtrack-id <id> --message-id <message>` queues a provider-facing deterministic three-track soundtrack card from bounded static decks without calling a model, using external randomness, generating or fetching media, calling provider APIs, or mutating the repository\n")
	b.WriteString("- `/channels story-dice <theme> --story-dice-id <id> --message-id <message>` queues a provider-facing deterministic prompt-dice card from bounded static decks without calling a model, using external randomness, generating media, calling provider APIs, or mutating the repository\n")
	b.WriteString("- `/channels mad-libs <theme> --mad-libs-id <id> --message-id <message>` queues a provider-facing deterministic fill-in card from bounded static templates without calling a model, dynamically generating text, using external randomness, persisting game state, tracking scores, calling provider APIs, or mutating the repository\n")
	b.WriteString("- `/channels arcade <mode> --arcade-id <id> --message-id <message>` queues a provider-facing bounded play-menu card without calling a model, dynamically generating play text, using external randomness, persisting game state, tracking scores, calling provider APIs, or mutating the repository\n")
	...
	b.WriteString("- `/channels constellation <lane> --constellation-id <id> --message-id <message>` queues a provider-facing capability star-map card across skills, tools, soul, memory, backups, research, channels, or fun without dynamic generation, execution, provider APIs, or repo mutation\n")
	b.WriteString("- `/channels mission-control <lane> --mission-id <id> --message-id <message>` queues a provider-facing bounded operating-loop card across skills, tools, soul, memory, backups, research, channels, launch, or fun without dynamic generation, execution, provider APIs, schedules, workflow edits, or repo mutation\n")
	b.WriteString("- `/channels cockpit <lane> --cockpit-id <id> --message-id <message>` queues a provider-facing bounded status-board card across skills, tools, soul, memory, backups, research, channels, launch, or fun without dynamic generation, execution, provider APIs, schedules, workflow edits, or repo mutation\n")
...
func renderChannelVerifyReport(ev Event, cfg Config, comments []Comment, includeIssue bool) string {
	...
	b.WriteString("- `/channels icebreaker` can queue the warmup conversation-starter card as a deterministic low-pressure opener from a mirrored thread without creating polls, rollcalls, tasks, schedules, provider API calls, model calls, workflow edits, or repository mutations\n")
	...
	b.WriteString("- `/channels skill-spotlight` can queue one deterministic provider-facing skill spotlight card from a mirrored channel thread without calling a model, installing skills, contacting registries, exposing raw skill bodies, or mutating the repository\n")
	...
	b.WriteString("- `/channels backup-timeline` can queue a provider-facing backup chronology card from a mirrored channel thread by fetching `gitclaw-backups` read-only when needed, without writing the backup branch, restoring files, replaying GitHub APIs, calling a model, or mutating the repository\n")
	b.WriteString("- `/channels backup-freshness` can queue a provider-facing latest-backup age and freshness-gate card from a mirrored channel thread by fetching `gitclaw-backups` read-only when needed, without writing the backup branch, restoring files, replaying GitHub APIs, calling a model, or mutating the repository\n")
	b.WriteString("- `/channels backup-continuity` can queue a provider-facing backup continuity and longest-gap card from a mirrored channel thread by fetching `gitclaw-backups` read-only when needed, without writing the backup branch, restoring files, replaying GitHub APIs, calling a model, or mutating the repository\n")
	...
	b.WriteString("- `/channels riddle` can queue a provider-facing deterministic riddle card from static channel-native decks without external randomness, executing commands, creating artifacts/tasks/reminders, installing skills, executing tools, calling models/provider APIs, editing workflows, persisting game state, tracking scores, or mutating the repository\n")
	...
	b.WriteString("- `/channels postcard` can queue a provider-facing place/caption scene card from a mirrored channel thread without calling a model, generating images, fetching media, calling provider APIs, mutating the repo, or exposing raw postcard ids/titles/captions in the source receipt\n")
	b.WriteString("- `/channels bingo` can queue a provider-facing deterministic mini-game card from bounded static decks without calling a model, using external randomness, persisting game state, tracking scores, calling provider APIs, mutating the repo, or exposing raw bingo ids/themes/notes/card cells in the source receipt\n")
	...
	b.WriteString("- `/channels soundtrack` can queue a provider-facing deterministic three-track mix card from bounded static decks without calling a model, using external randomness, generating or fetching media, calling provider APIs, mutating the repo, or exposing raw soundtrack ids/themes/notes/tracks in the source receipt\n")
	b.WriteString("- `/channels story-dice` can queue a provider-facing deterministic prompt-dice card from bounded static decks without calling a model, using external randomness, generating media, calling provider APIs, mutating the repo, or exposing raw story-dice ids/themes/notes/rolls in the source receipt\n")
	b.WriteString("- `/channels mad-libs` can queue a provider-facing deterministic fill-in card from bounded static templates without calling a model, dynamically generating text, using external randomness, persisting game state, tracking scores, calling provider APIs, mutating the repo, or exposing raw mad-libs ids/themes/notes/templates/blanks/prompts in the source receipt\n")
	b.WriteString("- `/channels arcade` can queue a provider-facing bounded play-menu card from a mirrored channel thread without calling a model, dynamically generating play text, using external randomness, persisting game state, tracking scores, executing commands, installing skills, executing tools, reading backup payloads, reading soul bodies, calling provider APIs, mutating the repo, or exposing raw arcade ids/modes/notes/move text/command text in the source receipt\n")
	...
	b.WriteString("- `/channels constellation` can queue a provider-facing bounded capability star-map card from a mirrored channel thread without dynamic star generation, external randomness, command execution, skill installs, tool execution, backup payload reads, soul body reads, memory writes, source fetches, live browsing, provider APIs, model calls, workflow mutations, policy changes, schedules, repository mutations, or exposing raw constellation ids/lanes/notes/star text/command text in the source receipt\n")
	b.WriteString("- `/channels mission-control` can queue a provider-facing bounded operating-loop card from a mirrored channel thread without dynamic loop generation, external randomness, command execution, skill installs, tool execution, backup payload reads, soul body reads, memory writes, source fetches, live browsing, provider APIs, model calls, workflow mutations, policy changes, schedules, repository mutations, or exposing raw mission ids/lanes/notes/step text/command text in the source receipt\n")
	b.WriteString("- `/channels cockpit` can queue a provider-facing bounded status-board card from a mirrored channel thread without dynamic cockpit generation, external randomness, command execution, skill installs, tool execution, backup payload reads, soul body reads, memory writes, source fetches, live browsing, provider APIs, model calls, workflow mutations, policy changes, schedules, repository mutations, or exposing raw cockpit ids/lanes/notes/gauge text/command text in the source receipt\n")
	...
	b.WriteString("- `/channels tool-spotlight` can queue a deterministic provider-facing safe tool spotlight card from built-in tool contracts without executing tools, launching MCP servers, activating toolsets, calling provider APIs, model calls, workflow mutations, repository mutations, or exposing raw focus/notes/spotlight ids/tool triggers/tool schemas/tool inputs/tool outputs in the source receipt\n")
	b.WriteString("- `/channels tool-drill` can queue a deterministic provider-facing safe tool practice card from built-in tool contracts without executing tools, launching MCP servers, activating toolsets, calling provider APIs, model calls, workflow mutations, repository mutations, or exposing raw focus/notes/drill ids/tool triggers/tool schemas/tool inputs/tool outputs in the source receipt\n")
	b.WriteString("- `/channels bundle-map` can queue a provider-facing safe skill-bundle sequence from a mirrored channel thread without installing skills, enabling bundles, writing bundle YAML, creating bundle proposal/rehearsal issues, calling provider APIs, model calls, workflow mutations, repository mutations, or exposing raw bundle-map ids/bundle names/notes/step text in the source receipt\n")
	b.WriteString("- `/channels source-map` can queue a provider-facing safe skill-source sequence from a mirrored channel thread without contacting registries, fetching remote sources, installing skills, writing source pins, creating source proposal issues, calling provider APIs, model calls, workflow mutations, repository mutations, or exposing raw source-map ids/source names/source refs/notes/step text in the source receipt\n")
	b.WriteString("- `/channels research-spotlight` can queue a deterministic provider-facing research spotlight card from static source, pattern, and rejection metadata without fetching sources, live browsing, model calls, provider API calls, workflow mutations, repository mutations, or exposing raw focus/notes/spotlight ids/source ids/URLs/patterns/surfaces/research bodies in the source receipt\n")
	b.WriteString("- `/channels research-map` can queue a provider-facing research-to-GitClaw command sequence from static source, pattern, and rejection metadata without fetching sources, live browsing, executing tools, calling provider APIs, model calls, workflow mutations, repository mutations, or exposing raw focus/notes/map ids/source ids/URLs/patterns/surfaces/step text/research bodies in the source receipt\n")
	b.WriteString("- `/channels soul-spotlight` can queue a deterministic provider-facing high-authority context spotlight card from repo-local soul metadata without registry contact, profile export, soul writes, memory writes, provider APIs, model calls, workflow mutations, repository mutations, or exposing raw focus/notes/spotlight ids/context paths/context bodies in the source receipt\n")
